	...
	<tr bgcolor="silver">
...

		{
...

			{
				...
				echo '<td width="110px" height="90px" onclick="selecciona_dia_general('.$comillas.$year.$comillas.','.$comillas.$mes_valor.$comillas.','.$comillas.$dia_valor.$comillas.')"; style="cursor:pointer; font-size:9px" >';
				...
				if($rs_citas)

                   {
	                  ...
	                  while (!$rs_citas->EOF) {	
					   ...
					   echo $nombres_p[0].' '.$apellido_p[0].'('.$rs_nmedico->fields["usua_codigoiniciales"].')';
					   echo '<br>';
				...
				$busca_terapias="select * from faesa_terapiasregistro inner join app_cliente on  faesa_terapiasregistro.clie_id=app_cliente.clie_id  where terap_fecha='".$fecha_bu."' order by terap_hora asc";
				...
				if($rs_terapia)
                   {
	                  ...
	                  while (!$rs_terapia->EOF) {	
					   ...
					   $apellido_p=explode(" ",$rs_terapia->fields["clie_apellido"]);  
						...
						echo $nombres_p[0].' '.$apellido_p[0].'(TERAPIA-'.$rs_terapia->fields["terap_hora"].')';
					    echo '<br>';
