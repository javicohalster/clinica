...
';
  ...
  //busca cruce
  $busca_cruse = "select * from lpin_crucedocumentos inner join pichinchahumana_extension.lpin_crucedetalle on lpin_crucedocumentos.crudoc_enlace=pichinchahumana_extension.lpin_crucedetalle.crudoc_enlace where crudet_documento='" . $xml . "'";
  $lista_cruce = $DB_gogess->executec($busca_cruse, array());

  if ($lista_cruce) {
    while (!$lista_cruce->EOF) {

      $crudet_id = $lista_cruce->fields["crudet_id"];

      //==============================================================================================


      $busca_asiento = "select * from lpin_comprobantecontable inner join lpin_detallecomprobantecontable on lpin_comprobantecontable.comcont_enlace=lpin_detallecomprobantecontable.comcont_enlace where crudet_id='" . $crudet_id . "'";
...
';
      ...
      $lista_data = "select * from  lpin_comprobantecontable where comcont_tabla='lpin_crucedocumentos' and comcont_idtabla='" . $comcont_idtabla . "' and crudet_id='" . $crudet_id . "'";
      $registros_data = $DB_gogess->executec($lista_data, array());
