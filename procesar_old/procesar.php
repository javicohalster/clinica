...
$url = "importar_COMPLETO_2018.xlsx";
	 ...
	 // print_r($array_letra);
	  ...
	  for ($row = 1; $row <= $lastRow; $row++) {
		  ...
		  for ($ilet=0;$ilet<count($array_letra);$ilet++)
			...
			if($datos[14]=='Control interno')
			{
			  $asign_id=4;
			}	
			$categ_id=$asign_id;
			
			if($datos[14]=='Equipo de oficina')
			{
			  $asign_id=5;
			}	
			$categ_id=$asign_id;

			if($datos[14]=='Equipo de computo')
			{
			  $asign_id=2;
			}	
			$categ_id=$asign_id;

			if($datos[14]==utf8_encode('Material didáctico'))
			{
			  $asign_id=3;
			}	
			$categ_id=$asign_id;	
			
			if($datos[14]=='Equipo telefonico')
			{
			  $asign_id=6;
			}	
			$categ_id=$asign_id;
			
			if($datos[14]=='Otros activos')
			{
			  $asign_id=7;
			}	
			$categ_id=$asign_id;
			...
			for($i=0;$i<count($array_campos);$i++)
			{
			   ...
			   if($array_campos[$i]=='invenm_fechaadquisicion' or $array_campos[$i]=='invenm_fechamantenimiento')
			   {
					if($$array_campos[$i])
					 {
					 $concatena_campos_valores.="'".utf8_decode($$array_campos[$i])."',";
					 }
					 else
					 {
					 $concatena_campos_valores.="'0000-00-00',";
					 }
			   }
			   else
			   {
