...
$lista_d="select distinct dns_cuadrobasicomedicamentos.cuadrobm_id,cuadrobm_codigoatc,concat(cuadrobm_principioactivo,' ',cuadrobm_nombredispositivo) as nombre_generico,cuadrobm_primerniveldesagregcion as forma,concat(cuadrobm_concentracion,' ',cuadrobm_presentacion) as concentracion,cuadrobm_valorplanilla from dns_stockactual inner join dns_cuadrobasicomedicamentos on dns_stockactual.cuadrobm_id=dns_cuadrobasicomedicamentos.cuadrobm_id where stock_fechaureg>='".$_POST["fecha_inicio"]."' and stock_fechaureg<='".$_POST["fecha_fin"]."' and centro_id='".$_POST["centro_id"]."' and categ_id=1 order by dns_cuadrobasicomedicamentos.cuadrobm_id asc";
