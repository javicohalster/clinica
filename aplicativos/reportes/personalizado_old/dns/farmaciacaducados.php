...
$lista_d="select distinct dns_cuadrobasicomedicamentos.cuadrobm_id,cuadrobm_codigoatc,concat(cuadrobm_principioactivo,' ',cuadrobm_nombredispositivo) as nombre_generico,cuadrobm_primerniveldesagregcion as forma,cuadrobm_concentracion as concentracion,cuadrobm_presentacion as presentacion,cuadrobm_valorplanilla,cuadrobm_stockminimo,cuadrobm_nombrecomercial,moviin_nombrefabricante,moviin_nombreproveedor,moviin_fechadeelaboracion,moviin_nlote,moviin_fechadecaducidad,cuadrobm_valorplanilla from dns_stockactual inner join dns_cuadrobasicomedicamentos on dns_stockactual.cuadrobm_id=dns_cuadrobasicomedicamentos.cuadrobm_id inner join dns_movimientoinventario on dns_cuadrobasicomedicamentos.cuadrobm_id=dns_movimientoinventario.cuadrobm_id where stock_fechaureg>='".$_POST["fecha_inicio"]."' and stock_fechaureg<='".$_POST["fecha_fin"]."' and dns_stockactual.centro_id='".$_POST["centro_id"]."' and categ_id=1 and tipom_id=1 order by dns_cuadrobasicomedicamentos.cuadrobm_id asc";
...
<table width="950" border="1" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td class="css_listat">NOMBRE GENERICO (PRINCIPIO ACTIVO)</td>
	<td class="css_listat">NOMBRE COMERCIAL DEL MEDICAMENTO</td>
	...
	<td class="css_listat">NOMBRE DEL FABRICANTE</td>
	<td class="css_listat">NOMBRE DEL PROVEEDOR</td>	
	<td class="css_listat">FECHA DE ELABORACI&Oacute;N</td>
	<td class="css_listat">LOTE PRODUCTO CADUCADO/ POR CADUCAR</td>
	<td class="css_listat">FECHA DE CADUCIDAD PRODUCTO CADUCADO/POR CADUCAR</td>
	<td class="css_listat">ESTADO DEL PRODUCTO</td>
...
?>  
  <tr>
	...
	<td class="css_lista"><?php echo utf8_encode($rs_tblpla->fields["cuadrobm_nombrecomercial"]); ?></td>
	...
	<td class="css_lista"><?php echo utf8_encode($rs_tblpla->fields["moviin_nombrefabricante"]); ?></td>
	<td class="css_lista"><?php echo utf8_encode($rs_tblpla->fields["moviin_nombreproveedor"]); ?></td>
	<td class="css_lista"><?php echo utf8_encode($rs_tblpla->fields["moviin_fechadeelaboracion"]); ?></td>
	<td class="css_lista"><?php echo utf8_encode($rs_tblpla->fields["moviin_nlote"]); ?></td>
	<td class="css_lista"><?php echo utf8_encode($rs_tblpla->fields["moviin_fechadecaducidad"]); ?></td>
	<td class="css_lista"  nowrap="nowrap" ><?php 
	
	if($rs_tblpla->fields["moviin_fechadecaducidad"]>date("Y-m-d"))
	{
	
	$date1 = new DateTime(date("Y-m-d"));
	$date2 = new DateTime($rs_tblpla->fields["moviin_fechadecaducidad"]);
	$diff = $date1->diff($date2);
	// will output 2 days
	if($diff->days<=60)
	{
	
	 echo '<div><b>POR CADUCADO</b></div>';
	}
	...
	{
	  echo '<div style="color:#FF0000"><b>CADUCADO</b></div>';
	}
	
	//echo utf8_encode($rs_tblpla->fields["moviin_fechadecaducidad"]); 
