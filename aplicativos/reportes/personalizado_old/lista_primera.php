...
<table border="0" align="center" cellpadding="2" cellspacing="2">
  ...
  if($_POST["even_id"])
  {
 $listacampos1="select * from app_cliente c inner join app_asistencia a on c.clie_id=a.clie_id inner join app_eventos ev on a.even_id=ev.even_id where (asis_fecharegistro>='".$_POST["fecha_inicio"]."' and asis_fecharegistro<='".$_POST["fecha_fin"]."') and a.even_id=".$_POST["even_id"]." order by asis_fecharegistro asc";
 }
 else
 {
  $listacampos1="select * from app_cliente c inner join app_asistencia a on c.clie_id=a.clie_id inner join app_eventos ev on a.even_id=ev.even_id where (asis_fecharegistro>='".$_POST["fecha_inicio"]."' and asis_fecharegistro<='".$_POST["fecha_fin"]."') "."order by asis_fecharegistro asc";;
 ...
 if($rs_gogessform)
 {
		...
		$desplegar=1;
		//reglas
		$fechas_menores="select asis_id from app_asistencia where clie_id=".$rs_gogessform->fields["clie_id"]." and asis_fecharegistro < '".$_POST["fecha_inicio"]."'";
