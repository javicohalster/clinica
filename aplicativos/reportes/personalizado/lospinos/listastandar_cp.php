...
{
   $sql3=" DATE_FORMAT(crb_fecha,'%Y-%m-%d')>='".$fecha_i."' and DATE_FORMAT(crb_fecha,'%Y-%m-%d')<='".$fecha_f."' and ";
}  
else
{
  ...
  if($fecha_i!='' and $fecha_f=='')
  {  
    $sql3=" DATE_FORMAT(crb_fecha,'%Y-%m-%d')>='".$fecha_i."' and ";
  }
  else
  {
    if($fecha_i=='' and $fecha_f!='')
	{
	   $sql3=" DATE_FORMAT(crb_fecha,'%Y-%m-%d')<='".$fecha_f."' and ";  
    }
  }
...
$documento.='<table border="1" cellpadding="0" cellspacing="0">
  <tr>
	...
	<td>Persona</td>
	<td>Cuenta Pago/Cobro</td>
	<td># de Comprobante</td>
	<td>Lote:</td>
	<td>N&uacute;mero de cheque</td>
...
$lista_doc="select * from lpin_cobropago left join lpin_ttransac on lpin_cobropago.ttra_id=lpin_ttransac.ttra_id left join app_proveedor on lpin_cobropago.proveep_id=app_proveedor.provee_id  left join lpin_lote on lpin_cobropago.lote_id=lpin_lote.lottar_id left join lpin_cuentasbancarias_vista on lpin_cobropago.cuentb_id=lpin_cuentasbancarias_vista.cuentb_id left join lpin_formadecobro on lpin_cobropago.frocob_id=lpin_formadecobro.frocob_id inner join lpin_cobropagodetalle on lpin_cobropago.crb_enlace=lpin_cobropagodetalle.crb_enlace where ".$concatena_sql." order by crb_fecha asc";
...
$lista_doc="select * from lpin_cobropago left join lpin_ttransac on lpin_cobropago.ttra_id=lpin_ttransac.ttra_id left join app_proveedor on lpin_cobropago.proveep_id=app_proveedor.provee_id  left join lpin_lote on lpin_cobropago.lote_id=lpin_lote.lottar_id left join lpin_cuentasbancarias_vista on lpin_cobropago.cuentb_id=lpin_cuentasbancarias_vista.cuentb_id left join lpin_formadecobro on lpin_cobropago.frocob_id=lpin_formadecobro.frocob_id inner join lpin_cobropagodetalle on lpin_cobropago.crb_enlace=lpin_cobropagodetalle.crb_enlace  order by crb_fecha asc";
...
{
$Nfactura=$objformulario->replace_cmb("beko_documentocabecera","doccab_id,doccab_ndocumento"," where doccab_id =",$rs_data->fields["doccabcp_id"],$DB_gogess);
}

$Ncompra='';
if($rs_data->fields["compracp_id"]>0)
{
$Ncompra=$objformulario->replace_cmb("dns_compras","compra_id,compra_nfactura"," where compra_id =",$rs_data->fields["compracp_id"],$DB_gogess);
}

$documento.='<tr>';
$documento.='<td  nowrap="nowrap">'.$contador_t.'</td>';
$documento.='<td >'.$rs_data->fields["ttra_nombre"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["frocob_nombre"].'</td>';
$documento.='<td  style=mso-number-format:"@"  nowrap="nowrap" >'.$rs_data->fields["crb_fecha"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["provee_nombre"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["crb_cuenta"].'</td>';
...
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["crb_ncheque"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["crb_fechacheque"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["cuentb_cuentacontable"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["crb_descripcion"].'</td>';

$documento.='<td  style=mso-number-format:"@" nowrap="nowrap" >'.$Ncompra.'</td>';
$documento.='<td  style=mso-number-format:"@" nowrap="nowrap" >'.$Nfactura.'</td>';
$documento.='<td  style=mso-number-format:"@"  nowrap="nowrap" >'.$rs_data->fields["crpadet_fechaemision"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["tipdocdet_id"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["crpadet_valor"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["crpadet_valorapagar"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["crpadet_saldo"].'</td>';
