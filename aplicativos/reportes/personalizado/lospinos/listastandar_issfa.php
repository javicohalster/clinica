...
<br>';
  ...
  <tr>
        ...
        <td bgcolor="#F0F0F0"><strong>FECHA REGISTRO</strong></td>
				<td bgcolor="#F0F0F0"><strong>FECHA PRIMERA ATENCION</strong></td>
				
				<td bgcolor="#F0F0F0"><strong>DIAS</strong></td>
				<td bgcolor="#F0F0F0"><strong>ESPECIALIDAD</strong></td>
        <td bgcolor="#F0F0F0"><strong>PRECUENTA</strong></td>
  </tr>';

  if ($concatena_sql) {
    $lista_doc = "select * from app_cliente where " . $concatena_sql;
  }
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
      ...
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["clie_nombre"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["clie_apellido"] . '</td>';

      $fecha_i = array();
      $fecha_i = explode(" ", $rs_data->fields["clie_registro"]);

      $documento .= '<td style=mso-number-format:"@" >' . $fecha_i[0] . '</td>';


      $busca_datavalor = "select * from lista_formulario where clie_id='" . $rs_data->fields["clie_id"] . "' order by anam_fecharegistro asc limit 1";
      $rs_datavalor = $DB_gogess->executec($busca_datavalor, array());

      $busca_datavalor2 = "select * from lista_formulario  as f
      inner join  dns_precuenta  as p on f.clie_id=p.clie_id
      inner join  pichinchahumana_extension.dns_convenios as c on  p.convepr_id=c.conve_id
      where f.clie_id='" . $rs_data->fields["clie_id"] . "' order by f.anam_fecharegistro desc limit 1";
      $rs_datavalor2 = $DB_gogess->executec($busca_datavalor2, array());

      $anam_fecharegistro = array();
      $anam_fecharegistro = explode(" ", $rs_datavalor->fields["anam_fecharegistro"]);



      $documento .= '<td style=mso-number-format:"@" >' . $anam_fecharegistro[0] . '</td>';

      $dias = '';
      if ($anam_fecharegistro[0]) {
        $fecha1 = new DateTime($fecha_i[0]);
        $fecha2 = new DateTime($anam_fecharegistro[0]);
        ...
        $anam_fecharegistro[0] = '';
        $dias = '';
      }


      $documento .= '<td style=mso-number-format:"@" >' . $dias . '</td>';
	   $documento .= '<td style=mso-number-format:"@" >' . $rs_datavalor2->fields["especialidad"] . '</td>';
     $documento .= '<td style=mso-number-format:"@" >' . $rs_datavalor2->fields["conve_nombre"] . '</td>';
