...
if ($_SESSION['datadarwin2679_sessid_inicio']) {
  ...
  if ($fecha_i != '' and $fecha_f != '') {
    $sql3 = " 	anam_fecharegistro>='" . $fecha_i . " 00:00:00' and anam_fecharegistro<='" . $fecha_f . " 23:59:59' and ";
  } else {

    if ($fecha_i != '' and $fecha_f == '') {
      $sql3 = " anam_fecharegistro>='" . $fecha_i . "  00:00:00' and ";
    } else {
      if ($fecha_i == '' and $fecha_f != '') {
        $sql3 = " anam_fecharegistro<='" . $fecha_f . " 23:59:59' and ";
      }
    }
  }
  ...
  if ($clie_institucionval) {
    $sql5 = "  conve_id = '" . trim($clie_institucionval) . "' and ";
  }
...
<br>';
  ...
  <tr>
        ...
        <td bgcolor="#F0F0F0"><strong>CEDULA</strong></td>
				<td bgcolor="#F0F0F0"><strong>NOMBRE</strong></td>
				<td bgcolor="#F0F0F0"><strong>APELLIDO</strong></td>
        <td bgcolor="#F0F0F0"><strong>ESTADO CIVIL</strong></td>
        <td bgcolor="#F0F0F0"><strong>GENERO</strong></td>
				<td bgcolor="#F0F0F0"><strong>CELULAR</strong></td>
        <td bgcolor="#F0F0F0"><strong>EMAIL</strong></td>
        <td bgcolor="#F0F0F0"><strong>DIRECCION</strong></td>
        <td bgcolor="#F0F0F0"><strong>USUARIO</strong></td>
        <td bgcolor="#F0F0F0"><strong>FECHA NACIEMINTO</strong></td>
        <td bgcolor="#F0F0F0"><strong>ESPECIALIDAD</strong></td>
  </tr>';

  if ($concatena_sql) {
    $lista_doc = "SELECT app_usuario.usua_id,usua_ciruc,usua_codigoiniciales,usua_nombre,usua_apellido,usua_genero,usua_celular,usua_direcciondom,usua_telefonodom,usua_usuario,usua_email,usua_fechanacimiento,prof_nombre,civil_id FROM app_usuario left join dns_gridfuncionprofesional on app_usuario.usua_enlace=dns_gridfuncionprofesional.usua_enlace left join pichinchahumana_extension.dns_profesion on dns_gridfuncionprofesional.prof_id=pichinchahumana_extension.dns_profesion.prof_id where " . $concatena_sql;
  } else {
    $lista_doc = "SELECT app_usuario.usua_id,usua_ciruc,usua_codigoiniciales,usua_nombre,usua_apellido,usua_genero,usua_celular,usua_direcciondom,usua_telefonodom,usua_usuario,usua_email,usua_fechanacimiento,prof_nombre,civil_id FROM app_usuario left join dns_gridfuncionprofesional on app_usuario.usua_enlace=dns_gridfuncionprofesional.usua_enlace left join pichinchahumana_extension.dns_profesion on dns_gridfuncionprofesional.prof_id=pichinchahumana_extension.dns_profesion.prof_id order by usua_nombre asc";
  }
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
      ...
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["usua_id"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["usua_ciruc"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["usua_nombre"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["usua_apellido"] . '</td>';
      $nestado = $objformulario->replace_cmb("pichinchahumana_combos.app_estadocivil", "civil_id,civil_nombre", " where civil_id =", $rs_data->fields["civil_id"], $DB_gogess);

      $documento .= '<td style=mso-number-format:"@" >' . $nestado . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["usua_genero"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["usua_celular"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["usua_email"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["usua_direcciondom"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["usua_usuario"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["usua_fechanacimiento"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["prof_nombre"] . '</td>';
