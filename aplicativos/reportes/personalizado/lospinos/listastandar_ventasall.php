...
$crb_ncomprobante='';
$busca_comp="select * from lpin_cobropago where doccab_id='".$rs_data->fields["doccab_id"]."'";
$rs_listcomp = $DB_gogess->executec($busca_comp,array());
	  if($rs_listcomp)
      {
			  while (!$rs_listcomp->EOF) {
				
				$crb_ncomprobante.=$rs_listcomp->fields["crb_ncomprobante"]." ";

				$rs_listcomp->MoveNext();					
			  }
	  }	  


$documento.='<td nowrap="nowrap"  style=mso-number-format:"@" >'.$crb_ncomprobante.'</td>';
...
$documento.='<td align="right" >'.$tdost.'</td>';
	...
	$total_ventaasiento=0;
	$total_ventareal=0;
	$total_ventaasiento=round($rs_okas->fields["tasi"],2);
	$total_ventareal=round($subtotal,2);
	
	$fondo_alerta='';
	if($total_ventaasiento!=$total_ventareal)
	{
	  $fondo_alerta=' bgcolor="#FBDDE6" ';
	}
	
	$alertav='';
	if(round($suma_vaoras,2)!=round($rs_okas->fields["tasi"]*$signo,2))
	{
	   $alertav='ALERTA';
	}
	
$documento.='<td '.$fondo_alerta.' align="right"  >'.round($rs_okas->fields["tasi"]*$signo,2).'</td>';	

$documento.='<td '.$fondo_alerta.' align="right"  >'.$suma_cuentas.'<BR>'.$alertav.'</td>';

$gtotlatotalasiento=$gtotlatotalasiento+round($rs_okas->fields["tasi"]*$signo,2);
...
$documento.='<tr>
	...
	<td align="right" >'.@$gtotlatotalasiento.'</td>
	<td align="right" ></td>
  </tr>
