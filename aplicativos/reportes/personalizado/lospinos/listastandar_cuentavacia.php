...
<br>';
  ...
  <tr>
        <td bgcolor="#F0F0F0"><strong>No</strong></td>
				<td bgcolor="#F0F0F0"><strong>COMPROBANTE ID</strong></td>
        <td bgcolor="#F0F0F0"><strong>TABLA</strong></td>
				<td bgcolor="#F0F0F0"><strong>ID TABLA</strong></td>
        <td bgcolor="#F0F0F0"><strong>TIPO</strong></td>
        <td bgcolor="#F0F0F0"><strong>No COMPROBANTE</strong></td>
  </tr>';


  $lista_doc = "select distinct comcont_id,comcont_tabla,comcont_idtabla from lpin_comprobantecontable inner join lpin_detallecomprobantecontable on lpin_comprobantecontable.comcont_enlace=lpin_detallecomprobantecontable.comcont_enlace  where comcont_anulado=0 and detcc_cuentacontable=''";
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
      ...
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["comcont_tabla"] . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["comcont_idtabla"] . '</td>';


      $tipodoc = '';
      $ndocumento = '';
      if ($rs_data->fields["comcont_tabla"] == 'beko_documentocabecera') {
        $busca_datavalor = "select * from beko_documentocabecera where doccab_id='" . $rs_data->fields["comcont_idtabla"] . "'";
        $rs_datavalor = $DB_gogess->executec($busca_datavalor, array());

        $tipodoc = 'FACTURA VENTA';
        $ndocumento = $rs_datavalor->fields["doccab_ndocumento"];
      }

      if ($rs_data->fields["comcont_tabla"] == 'dns_compras') {
        $busca_datavalor = "select * from dns_compras where compra_id='" . $rs_data->fields["comcont_idtabla"] . "'";
        $rs_datavalor = $DB_gogess->executec($busca_datavalor, array());

        $tipodoc = 'FACTURA COMPRA';
        $ndocumento = $rs_datavalor->fields["compra_nfactura"];
      }

      if ($rs_data->fields["comcont_tabla"] == 'app_movimientobancos') {
        $tipodoc = 'BANCOS';
        $ndocumento = $rs_data->fields["comcont_idtabla"];
      }


      if ($rs_data->fields["comcont_tabla"] == '') {
        $tipodoc = 'ASIENTO MANUAL';
        $ndocumento = $rs_data->fields["comcont_id"];
      }


      $documento .= '<td style=mso-number-format:"@" >' . $tipodoc . '</td>';
      $documento .= '<td style=mso-number-format:"@" >' . $ndocumento . '</td>';
      $documento .= '</tr>';
