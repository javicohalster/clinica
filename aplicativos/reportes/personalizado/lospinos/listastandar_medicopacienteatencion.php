...
if ($_SESSION['datadarwin2679_sessid_inicio']) {
  ...
  if ($fecha_i != '' and $fecha_f != '') {
    $sql3 = " DATE_FORMAT(anam_fecharegistro,'%Y-%m-%d')>='" . $fecha_i . "' and DATE_FORMAT(anam_fecharegistro,'%Y-%m-%d')<='" . $fecha_f . "' and ";
  } else {

    if ($fecha_i != '' and $fecha_f == '') {
      $sql3 = " DATE_FORMAT(anam_fecharegistro,'%Y-%m-%d')>='" . $fecha_i . "' and ";
    } else {
      if ($fecha_i == '' and $fecha_f != '') {
        $sql3 = " DATE_FORMAT(anam_fecharegistro,'%Y-%m-%d')<='" . $fecha_f . "' and ";
      }
    }
  }
...
$lista_pacientesx="";  
  ...
  $lista_pacientesx="select distinct lista_pacientes.clie_id from lista_pacientes where usua_id='".$usua_id."'";  
  $sql11=" app_cliente.clie_id in (".$lista_pacientesx.") and ";
...
<br>';
  ...
  <td></td>	
	...
	<td>ESPECIALIDAD MEDICO</td>
	<td>ESPECIALIDAD ATENCION</td>	
	<td>FECHA ATENCION</td>		
  </tr>';

  if ($concatena_sql) {

    $lista_doc = "select * from app_cliente left join  pichinchahumana_extension.dns_tipoproceso on app_cliente.tippo_id=pichinchahumana_extension.dns_tipoproceso.tippo_id  left join pichinchahumana_extension.dns_convenios on app_cliente.conve_id=pichinchahumana_extension.dns_convenios.conve_id left join  lista_formulario on app_cliente.clie_id=lista_formulario.clie_id
	where  " . $concatena_sql . " order by clie_apellido asc";
  } else {

    $lista_doc = "select * from app_cliente left join  pichinchahumana_extension.dns_tipoproceso on app_cliente.tippo_id=pichinchahumana_extension.dns_tipoproceso.tippo_id  left join pichinchahumana_extension.dns_convenios on app_cliente.conve_id=pichinchahumana_extension.dns_convenios.conve_id left join lista_formulario on app_cliente.clie_id=lista_formulario.clie_id order by clie_apellido asc";
  }
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
	 ...
	 // echo $prof_id;
	  ...
	  $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["especialidad"] . '</td>';	
	  $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["anam_fecharegistro"] . '</td>';	 
