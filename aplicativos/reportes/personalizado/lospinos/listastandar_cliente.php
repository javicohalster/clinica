...
if ($_SESSION['datadarwin2679_sessid_inicio']) {
  ...
  if ($clie_id) {
    $sql5 = " app_proveedor.provee_id = '" . trim($clie_id) . "' and ";
  }
...
<br>';
  ...
  if ($concatena_sql) {
    ...
    $lista_doc = "select app_proveedor.provee_id,app_proveedor.tipoident_codigocl,app_proveedor.tipop_id,app_proveedor.provee_cedula,app_proveedor.provee_ruc,app_proveedor.provee_nombre,app_proveedor.provee_nombrecomercial,app_proveedor.provee_direccion,app_proveedor.provee_telefono,app_proveedor.provee_email,app_proveedor.emp_id,app_proveedor.provee_cespecial,app_proveedor.provee_fecharegistro,(select count(1) as facturas from beko_documentocabecera where proveeve_id=app_proveedor.provee_id) as facturas,usuac_id from app_proveedor where " . $concatena_sql . " and provee_borradologico=0 order by provee_fecharegistro asc";
  } else {


    $lista_doc = "select app_proveedor.provee_id,app_proveedor.tipoident_codigocl,app_proveedor.tipop_id,app_proveedor.provee_cedula,app_proveedor.provee_ruc,app_proveedor.provee_nombre,app_proveedor.provee_nombrecomercial,app_proveedor.provee_direccion,app_proveedor.provee_telefono,app_proveedor.provee_email,app_proveedor.emp_id,app_proveedor.provee_cespecial,app_proveedor.provee_fecharegistro,(select count(1) as facturas from beko_documentocabecera where proveeve_id=app_proveedor.provee_id) as facturas,usuac_id from app_proveedor where provee_borradologico=0 order by provee_fecharegistro asc";
  }
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
      ...
      $n_facturas = 0;
      $n_facturas = $rs_data->fields["facturas"];
      ...
      if ($n_facturas > 0) {
        ...
        $nusuario = '';
        $nusuario = $objformulario->replace_cmb("app_usuario", "usua_id,usua_nombre,usua_apellido", " where usua_id =", $rs_data->fields["usuac_id"], $DB_gogess);


        $documento .= '<tr>';
        $documento .= '<td  nowrap="nowrap">' . $contador_t . '</td>';
        $documento .= '<td >' . $rs_data->fields["provee_id"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $tipoc . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $tipop_id . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["provee_cedula"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["provee_ruc"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["provee_nombre"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["provee_nombrecomercial"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["provee_direccion"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["provee_telefono"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["provee_email"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["provee_cespecial"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["provee_fecharegistro"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $nusuario . '</td>';
        $documento .= '</tr>';
      }
