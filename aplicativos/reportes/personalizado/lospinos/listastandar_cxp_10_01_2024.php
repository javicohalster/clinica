...
if($concatena_sql)
{  
$lista_doc="SELECT compra_id,compra_fecha,provee_nombre,compra_nfactura,compra_descripcion,neto,compra_iva,compra_total,compra_anulado,totalret,saldo,sriretencion,tipdoc_nombre,proveevar_id,compra_autorizacion FROM dns_compras_vista  where ".$concatena_sql."  and compra_anulado=0 and tipdoc_id!=9";
}
else
{
$lista_doc="SELECT compra_id,compra_fecha,provee_nombre,compra_nfactura,compra_descripcion,neto,compra_iva,compra_total,compra_anulado,totalret,saldo,sriretencion,tipdoc_nombre,proveevar_id,compra_autorizacion FROM dns_compras_vista  where compra_anulado=0 and tipdoc_id!=9";
...
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["neto"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["compra_iva"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["compra_total"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["compra_anulado"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["totalret"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$suman_nc.'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$suman_pagos.'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["saldo"].'</td>';
...
$documento.='<tr>
    ...
    <td></td>
    <td nowrap="nowrap"></td>
    <td nowrap="nowrap">'.$suma_saldo.'</td>
  </tr>
</table>';
