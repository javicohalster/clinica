...
if ($_SESSION['datadarwin2679_sessid_inicio']) {
  ...
  if ($previsual == 2) {
    ...
    $fechahoy = date("Y-m-d");
    header("Content-Disposition: attachment; filename=" . "asientos_" . $fechahoy . ".csv");
  }
  ...
  if ($previsual == '1') {
    $limitv = ' limit 300 ';
    $mensaje_data = '<center><b>Por la cantidad de registros para previsualizar solo se motraran 300<br></b></center>';
  }
...
<br>';
  ...
  $documento .= $mensaje_data . '<table border="1" cellpadding="0" cellspacing="0">
  <tr>
        ...
        <td bgcolor="#F0F0F0"><strong>CUENTA</strong></td>
				<td bgcolor="#F0F0F0"><strong>DEBE</strong></td>
				<td bgcolor="#F0F0F0"><strong>HABER</strong></td>
  </tr>';

  $documentocsv .= "No; ID; TIPO; FECHA; CONCEPTO; NUMERO DE COMPROBANTE; ANULADO; CODIGO; CUENTA; DEBE; HABER\n";

  if ($concatena_sql) {

    $lista_doc = "select * from lpin_comprobantecontable where comcont_anulado=0 and  " . $concatena_sql . " order by comcont_fecha asc " . $limitv;
  } else {

    $lista_doc = "select * from lpin_comprobantecontable where comcont_anulado=0 order by comcont_fecha asc " . $limitv;
  }
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
      ...
      ///saca el debe
      //$lista_debe = "select sum(detcc_debe) as detcc_debe from lpin_detallecomprobantecontable where comcont_enlace='" . $rs_data->fields["comcont_enlace"] . "'";
      //$rs_debe = $DB_gogess->executec($lista_debe);
      //saca el debe

      //saca el haber
      //$lista_haber = "select sum(detcc_haber) as detcc_haber from lpin_detallecomprobantecontable where comcont_enlace='" . $rs_data->fields["comcont_enlace"] . "'";
      ...
      if ($banderad == 1) {
        $contador_t++;
        $busca_tipo = "select * from lpin_tipoasiento where tipoa_id='" . $rs_data->fields["tipoa_id"] . "'";
        $rs_tipo = $DB_gogess->executec($busca_tipo);

        $documento .= '<tr>';
        $documento .= '<td  nowrap="nowrap">' . $contador . '</td>';
        $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["comcont_id"] . '</td>';
        $documento .= '<td style=mso-number-format:"@" >' . $rs_tipo->fields["tipoa_nombre"] . '</td>';
        $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["comcont_fecha"] . '</td>';
        $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["comcont_concepto"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["comcont_numeroc"] . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["comcont_anulado"] . '</td>';
        ...
        $documentocsv .= $contador . ';' . $rs_data->fields["comcont_id"] . ';"' . $rs_tipo->fields["tipoa_nombre"] . '";' . $rs_data->fields["comcont_fecha"] . ';"' . $rs_data->fields["comcont_concepto"] . '";' . $rs_data->fields["comcont_numeroc"] . ';' . $rs_data->fields["comcont_anulado"] . ";;;;\n";

        $suma_debexc = 0;
        $suma_haberxc = 0;
        //listsa cuentas//
        $lista_debehaber = "select * from lpin_detallecomprobantecontable left join lpin_plancuentas on 	planc_codigoc=detcc_cuentacontable where comcont_enlace='" . $rs_data->fields["comcont_enlace"] . "'";
        $rs_debehaber = $DB_gogess->executec($lista_debehaber);
        if ($rs_debehaber) {
          while (!$rs_debehaber->EOF) {

            $documento .= '<tr>';
            $documento .= '<td  nowrap="nowrap"></td>';
            $documento .= '<td style=mso-number-format:"@" ></td>';
            $documento .= '<td style=mso-number-format:"@" ></td>';
            $documento .= '<td style=mso-number-format:"@" ></td>';
            $documento .= '<td style=mso-number-format:"@" ></td>';
            ...
            $documento .= '<td  style=mso-number-format:"@" >' . $rs_debehaber->fields["planc_nombre"] . '</td>';
            $documento .= '<td  style=mso-number-format:"@" >' . $rs_debehaber->fields["detcc_cuentacontable"] . '</td>';

            $documento .= '<td >' . str_replace(".", ",", round($rs_debehaber->fields["detcc_debe"], 2)) . '</td>';
            $documento .= '<td >' . str_replace(".", ",", round($rs_debehaber->fields["detcc_haber"], 2)) . '</td>';
            $documento .= '</tr>';

            $documentocsv .= ";;;;;;;" . $rs_debehaber->fields["planc_nombre"] . ";" . $rs_debehaber->fields["detcc_cuentacontable"] . ";" . str_replace(".", ",", round($rs_debehaber->fields["detcc_debe"], 2)) . ";" . str_replace(".", ",", round($rs_debehaber->fields["detcc_haber"], 2)) . "\n";

            $suma_debexc = $suma_debexc + round($rs_debehaber->fields["detcc_debe"], 2);
            $suma_haberxc = $suma_haberxc + round($rs_debehaber->fields["detcc_haber"], 2);
        ...
        if ($usua_id == 1) {
          $documento .= '<tr>';
          $documento .= '<td  nowrap="nowrap">TOTAL</td>';
          $documento .= '<td style=mso-number-format:"@" ></td>';
          $documento .= '<td style=mso-number-format:"@" ></td>';
          $documento .= '<td style=mso-number-format:"@" ></td>';
          $documento .= '<td style=mso-number-format:"@" ></td>';
          ...
          $documento .= '<td>' . str_replace(".", ",", round($suma_debexc, 2)) . '</td>';
          $documento .= '<td>' . str_replace(".", ",", round($suma_haberxc, 2)) . '</td>';
          $documento .= '</tr>';

          $documentocsv .= "TOTAL;;;;;;;;;" . str_replace(".", ",", round($suma_debexc, 2)) . ";" . str_replace(".", ",", round($suma_haberxc, 2)) . "\n";
        }

        $suma_debe = $suma_debe + round($suma_debexc, 2);
        $suma_haber = $suma_haber + round($suma_haberxc, 2);
      }
  ...
  $documentocsv .= ";;;;;;;;;" . $suma_debe . ";" . $suma_haber . "\n";
