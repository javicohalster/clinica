...
<br>';
  ...
  if ($concatena_sql) {
    ...
    $lista_doc = "select * from lpin_comprobantecontable where comcont_anulado=0 and  " . $concatena_sql . " order by comcont_fecha asc";
  } else {

    $lista_doc = "select * from lpin_comprobantecontable where comcont_anulado=0 order by comcont_fecha asc";
  }
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
      ...
      if ($banderad == 1) {
        ...
        $documento .= '<td>' . round($rs_debe->fields["detcc_debe"], 2) . '</td>';
        $documento .= '<td>' . round($rs_haber->fields["detcc_haber"], 2) . '</td>';
        $documento .= '<td  style=mso-number-format:"@" >' . $diferente . '</td>';
        $documento .= '</tr>';
