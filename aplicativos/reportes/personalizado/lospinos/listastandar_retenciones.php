...
{
   $sql3=" 	compretcab_fechaemision_cliente>='".$fecha_i."' and compretcab_fechaemision_cliente<='".$fecha_f."' and ";
}  
else
{
  ...
  if($fecha_i!='' and $fecha_f=='')
  {  
    $sql3=" compretcab_fechaemision_cliente>='".$fecha_i."' and ";
  }
  else
  {
    if($fecha_i=='' and $fecha_f!='')
	{
	   $sql3=" compretcab_fechaemision_cliente<='".$fecha_f."' and ";  
    }
  }
...
if($compretcab_rucci_cliente)
  {
   $sql4=" compretcab_rucci_cliente like '%".trim($compretcab_rucci_cliente)."%' and ";
  }  
...
$documento.='<table border="1" cellpadding="0" cellspacing="0">
  <tr>
	  ...
	  <td bgcolor="#CCCCCC" class="css_titulo"><div align="center">ANULADO</div></td>
	  <td bgcolor="#CCCCCC" class="css_titulo"><div align="center">SRI</div></td>
	  <td bgcolor="#CCCCCC" class="css_titulo"><div align="center">MOTIVO SRI</div></td>
	  
	  <td bgcolor="#CCCCCC" class="css_titulo"><div align="center">FACTURA DE COMPRA</div></td>
	  <td bgcolor="#CCCCCC" class="css_titulo"><div align="center">FECHA FACTURA DE COMPRA</div></td>
...
$rs_data = $DB_gogess->executec($lista_doc,array());
 if($rs_data)
 {
	 ...
	 $anulado_data='';
	 if($rs_data->fields["compretcab_anulado"]==1)
	 {
	   $anulado_data='ANULADO';
	 }
	 
	 $texto_wrap='';
	 $texto_wrap=wordwrap($rs_data->fields["compretcab_motivodev"], 40, "<br />\n",true);	
	 
	 $documento.='<tr>';	 
	 $documento.='<td  nowrap="nowrap">'.$contador.'</td>';
	 $documento.='<td  nowrap="nowrap">'.$rs_data->fields["compretcab_nretencion"].'</td>';
	 $documento.='<td  nowrap="nowrap">'.$rs_data->fields["compretcab_fechaemision_cliente"].'</td>';
	 $documento.='<td  nowrap="nowrap">'.$rs_data->fields["compretcab_nombrerazon_cliente"].'</td>';
	 $documento.='<td  nowrap="nowrap">'.$rs_data->fields["compretcab_rucci_cliente"].'</td>';	 
	 $documento.='<td  nowrap="nowrap">'.$anulado_data.'</td>';	 
	 $documento.='<td  nowrap="nowrap">'.$rs_data->fields["compretcab_estadosri"].'</td>';	
	 $documento.='<td  nowrap="nowrap">'.$texto_wrap.'</td>'; 
	 $documento.='<td  nowrap="nowrap">'.$rs_data->fields["compretcab_nfactura"].'</td>';
	 $documento.='<td  nowrap="nowrap">'.$rs_data->fields["compretcab_nfacturafecha"].'</td>';
