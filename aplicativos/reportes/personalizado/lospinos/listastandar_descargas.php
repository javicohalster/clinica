...
$documento.='<table border="1" cellpadding="0" cellspacing="0">
  <tr>
	  ...
	  <td bgcolor="#CCCCCC" class="css_titulo"><div align="center">Cedula</div></td>
	  <td bgcolor="#CCCCCC" class="css_titulo"><div align="center">Paciente</div></td>
	  <td bgcolor="#CCCCCC" class="css_titulo"><div align="center">Tipo Cobro</div></td>
	  <td bgcolor="#CCCCCC" class="css_titulo"><div align="center">Convenio Seguro</div></td>
...
$rs_data = $DB_gogess->executec($lista_doc,array());
 if($rs_data)
 {
	 ...
	 $busca_paciente="select * from app_cliente left join pichinchahumana_extension.dns_tipoproceso on app_cliente.tippo_id=pichinchahumana_extension.dns_tipoproceso.tippo_id left join pichinchahumana_extension.dns_convenios on app_cliente.conve_id=pichinchahumana_extension.dns_convenios.conve_id where clie_id='".$rs_data->fields["clie_id"]."'";
	 ...
	 $documento.='<td  nowrap="nowrap">'.$rs_data->fields["detapre_codigop"].'</td>';
	 ...
	 $documento.='<td  nowrap="nowrap">'.$rs_paciente->fields["clie_rucci"].'</td>';
	 $documento.='<td  nowrap="nowrap">'.$rs_paciente->fields["clie_nombre"].' '.$rs_paciente->fields["clie_apellido"].'</td>';
	 $documento .= '<td  style=mso-number-format:"@" >' . $rs_paciente->fields["tippo_nombre"] . '</td>';	  
	 $documento .= '<td  style=mso-number-format:"@" >' . $rs_paciente->fields["conve_nombre"] . '</td>';	 
	  
	 $documento.='<td  nowrap="nowrap">'.$rs_data->fields["detapre_fecharegistro"].'</td>';
	 
	 $usua_nombre = '';
     $usua_nombre = $objformulario->replace_cmb("app_usuario", "usua_id,usua_nombre,usua_apellido", " where usua_id like ", $rs_data->fields["usuar_id"], $DB_gogess);
	 
	 $documento.='<td  nowrap="nowrap">'.$usua_nombre.'</td>';
