...
$ib=0;
 if($rs_listadiario)
 {
	 ...
	 // and tipoa_id=6
	  ...
	  $sumatotales="select round(sum(detcc_debe - detcc_haber),2) as totales from lpin_detallecomprobantecontable_vista where ".$concatena_sql." and detcc_cuentacontablep like '".$rs_listadiario->fields["planc_codigoc"].".%' and comcont_anulado=0 and tipoa_id=6 and comcont_id='5173'";
	 ...
	 if($rs_listadiario->fields["planc_signo"]=='-1')
	 {				
	 $documento.='<td '.$stilo_data.' >'.$negritai.'$'.number_format($total_data, 2, '.', ',').$negritai.'</td>';
	 ...
	 {
	  $documento.='<td '.$stilo_data.' >'.$negritai.'$'.number_format(abs($total_data), 2, '.', ',').$negritaf.'</td>';
	   $suma_cuentas[trim($rs_listadiario->fields["planc_codigoc"])]=number_format(abs($total_data), 2, '.', '');
