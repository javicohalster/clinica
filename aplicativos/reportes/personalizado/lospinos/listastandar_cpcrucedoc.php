...
<br>';
  ...
  <td>Tipo de Transacci&oacute;n</td>
	<td>Fecha de Emisi&oacute;n cruce</td>
  <td>Fecha registro de  cruce</td>
	<td>Persona</td>
  ...
  <td>Fecha Emisi&oacute;n documento</td>
  <td>Valor cruce</td>
      ...
      if ($rs_crudeta) {
        while (!$rs_crudeta->EOF) {
          ...
          if ($crudet_documento > 0) {
            $saca_saldox = "select saldo,compra_nfactura,compra_total from dns_compras_vista where compra_id='" . $crudet_documento . "'";
            ...
            $total_recivecruze = $rs_sacsaldox->fields["compra_total"];
          }

          $ndocumento_ventaval = '';
          if ($crudet_documentoventa  != '') {
            $saca_saldo = "select saldo,doccab_ndocumento,doccab_total from beko_documentocabecera_vista where doccab_id='" . $crudet_documentoventa  . "'";
            $rs_sacsaldo = $DB_gogess->executec($saca_saldo, array());
            //$valor_saldo = $rs_sacsaldo->fields["saldo"];
            $ndocumento_ventaval =  $rs_sacsaldo->fields["doccab_ndocumento"];
            $saldo_final = $rs_sacsaldo->fields["saldo"];

            $total_recivecruze = $rs_sacsaldo->fields["doccab_total"];
          }
          ...
          $documento .= '<td>' . str_replace(".", ",", $total_recivecruze)  . '</td>';


          $documento .= '<td  style=mso-number-format:"@" >' . $crudet_fechaemision . '</td>';
          $documento .= '<td>' . str_replace(".", ",", $crudet_valor) . '</td>';
          $documento .= '<td>' . str_replace(".", ",", $saldo_final) . '</td>';
