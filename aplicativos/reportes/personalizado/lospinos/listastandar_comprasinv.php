...
if ($_SESSION['datadarwin2679_sessid_inicio']) {
  ...
  if ($previsual == 2) {
    ...
    $fechahoy = date("Y-m-d");
    header("Content-Disposition: attachment; filename=" . "comprasxls_" . $fechahoy . ".csv");
  }
...
<br>';
  ...
  <tr>
        ...
        <td bgcolor="#F0F0F0"><strong>TIPO DETALLE</strong></td>
        <td bgcolor="#F0F0F0"><strong>CODIGO PRODUCTO</strong></td>
        <td bgcolor="#F0F0F0"><strong>DESCRIPCION</strong></td>
        <td bgcolor="#F0F0F0"><strong>CANTIDAD</strong></td>
        <td bgcolor="#F0F0F0"><strong>VALOR UNIDAD</strong></td>
        <td bgcolor="#F0F0F0"><strong>SUB TOTAL</strong></td>
        <td bgcolor="#F0F0F0"><strong>DESCUENTO</strong></td>
        <td bgcolor="#F0F0F0"><strong>TOTAL</strong></td>
  </tr>';

  $documentocsv .= "No;ID;TIPO;FECHA EMISION;CONCEPTO;NUMERO DE COMPROBANTE;ANULADO;TIPO DETALLE;CODIGO PRODUCTO;DESCRIPCION;CANTIDAD;VALOR UNIDAD;SUB TOTAL;DESCUENTO;TOTAL\n";

  if ($concatena_sql) {

    $lista_doc = "select * from dns_compras where  tipdoc_id!=9 and " . $concatena_sql . " order by compra_fecha asc";
  } else {

    $lista_doc = "select * from dns_compras where  tipdoc_id!=9 order by compra_fecha asc";
  }
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
      ...
      if ($banderad == 1) {
        $contador_t++;
        $busca_tipo = "select * from dns_tipodocumentogeneral where tipdoc_id='" . $rs_data->fields["tipdoc_id"] . "'";
        $rs_tipo = $DB_gogess->executec($busca_tipo);

        /*$documento .= '<tr>';
        $documento .= '<td  nowrap="nowrap">' . $contador . '</td>';
        $documento .= '<td style=mso-number-format:"@" >' . $rs_data->fields["compra_id"] . '</td>';
        ...
        $lista_debehaber = "select * from beko_detalles_compra where compra_enlace='" . $rs_data->fields["compra_enlace"] . "'";
        ...
        if ($rs_debehaber) {
          while (!$rs_debehaber->EOF) {
            ...
            $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["compra_nfactura"] . '</td>';
            $estadoanulado = '';
            if ($rs_data->fields["compra_anulado"] == 1) {
              $estadoanulado = 'ANULADO';
            }
            $documento .= '<td  style=mso-number-format:"@" >' . $estadoanulado . '</td>';
            $documento .= '<td  style=mso-number-format:"@" >' . $rs_debehaber->fields["tipo"] . '</td>';
            $documento .= '<td  style=mso-number-format:"@" >' . $rs_debehaber->fields["docdet_codprincipal"] . '</td>';
            $documento .= '<td  style=mso-number-format:"@" >' . $rs_debehaber->fields["docdet_descripcion"] . '</td>';
            $documento .= '<td  style=mso-number-format:"@" >' . str_replace(".", ",", $rs_debehaber->fields["docdet_cantidad"]) . '</td>';
            $documento .= '<td  style=mso-number-format:"@" >' . str_replace(".", ",", $rs_debehaber->fields["docdet_preciou"]) . '</td>';
            $subt_tota = 0;
            $subt_tota = $rs_debehaber->fields["docdet_cantidad"] * $rs_debehaber->fields["docdet_preciou"];
            $documento .= '<td  style=mso-number-format:"@" >' . $subt_tota . '</td>';
            $documento .= '<td  style=mso-number-format:"@" >' . str_replace(".", ",", $rs_debehaber->fields["docdet_descuento"]) . '</td>';
            $documento .= '<td  style=mso-number-format:"@" >' . str_replace(".", ",", $rs_debehaber->fields["docdet_total"]) . '</td>';
            $documento .= '</tr>';

            $documentocsv .= $contador . ';' . $rs_data->fields["compra_id"] . ';"' . $rs_tipo->fields["tipdoc_nombre"] . '";' . $rs_data->fields["compra_fecha"] . ';"' . $rs_data->fields["compra_descripcion"] . '";' . $rs_data->fields["compra_nfactura"] . ';' . $estadoanulado . ";" . $rs_debehaber->fields["tipo"] . ";" . $rs_debehaber->fields["docdet_codprincipal"] . ';"' . $rs_debehaber->fields["docdet_descripcion"] . '";' . $rs_debehaber->fields["docdet_cantidad"] . ";" . $rs_debehaber->fields["docdet_preciou"] . ";" . $subt_tota . ";" . $rs_debehaber->fields["docdet_descuento"] . ";" . $rs_debehaber->fields["docdet_total"] . "\n";

            if ($rs_data->fields["compra_anulado"] == 0) {
              $suma_totalv = $suma_totalv + round($rs_debehaber->fields["docdet_total"], 2);
            }
  ...
  $documento .= '<br><br><center>NOTAS DE CREDITO</center> <table border="1" cellpadding="0" cellspacing="0">
  <tr>
        <td bgcolor="#F0F0F0"><strong>No</strong></td>
				<td bgcolor="#F0F0F0"><strong>ID</strong></td>
        <td bgcolor="#F0F0F0"><strong>TIPO</strong></td>
      ...
      if ($banderad == 1) {
        ...
        if ($rs_debehaber) {
          while (!$rs_debehaber->EOF) {
            ...
            $subt_tota = 0;
            $subt_tota = $rs_debehaber->fields["docdet_cantidad"] * $rs_debehaber->fields["docdet_preciou"];
            $documento .= '<td  style=mso-number-format:"@" >' . str_replace(".", ",", $subt_tota) . '</td>';
            $documento .= '<td  style=mso-number-format:"@" >' . str_replace(".", ",", $rs_debehaber->fields["docdet_descuento"]) . '</td>';
            $documento .= '<td  style=mso-number-format:"@" >' . str_replace(".", ",", $rs_debehaber->fields["docdet_total"]) . '</td>';
            $documento .= '</tr>';
