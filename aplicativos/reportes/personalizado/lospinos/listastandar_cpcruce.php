...
<br>';
  ...
  <td>Descripci&oacute;n</td>
	...
	<td>Documento cruce documento compra</td>
  <td>Documento cruce documento venta</td>
  <td>Documento cruce anticipo</td>
  <td>Documento cruce cuenta</td>
  <td>Fecha Emisi&oacute;n</td>
  <td>Valor cruce</td>

  <td>Saldo actual doc: Compra/ Venta</td>
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
      ...
      //DOCUMENTO
      $docu_cru = "select * from pichinchahumana_extension.lpin_crucedetalle where crudoc_enlace='" . $rs_data->fields["crudoc_enlace"] . "'";
      $rs_crudeta = $DB_gogess->executec($docu_cru, array());
      if ($rs_crudeta) {
        while (!$rs_crudeta->EOF) {

          $tipo_cruceval = 'DOCUMENTO';
          $crudet_documento = $rs_crudeta->fields["crudet_documento"];
          $crudet_documentoventa = $rs_crudeta->fields["crudet_documentoventa"];
          $crudet_fechaemision = $rs_crudeta->fields["crudet_fechaemision"];
          $crudet_tipodocumento = $rs_crudeta->fields["crudet_tipodocumento"];
          $crudet_valor = $rs_crudeta->fields["crudet_valor"];

          $saldo_final = 0;

          $ndocumento_compraval = '';
          if ($crudet_documento > 0) {
            $saca_saldox = "select saldo,compra_nfactura from dns_compras_vista where compra_id='" . $crudet_documento . "'";
            $rs_sacsaldox = $DB_gogess->executec($saca_saldox, array());
            //$valor_saldox = $rs_sacsaldox->fields["saldo"];
            $ndocumento_compraval = $rs_sacsaldox->fields["compra_nfactura"];
            $saldo_final = $rs_sacsaldox->fields["saldo"];
          }

          $ndocumento_ventaval = '';
          if ($crudet_documentoventa  != '') {
            $saca_saldo = "select saldo,doccab_ndocumento from beko_documentocabecera_vista where doccab_id='" . $crudet_documentoventa  . "'";
            $rs_sacsaldo = $DB_gogess->executec($saca_saldo, array());
            //$valor_saldo = $rs_sacsaldo->fields["saldo"];
            $ndocumento_ventaval =  $rs_sacsaldo->fields["doccab_ndocumento"];
            $saldo_final = $rs_sacsaldo->fields["saldo"];
          }
          ...
          $documento .= '<td>' . $total_valor . '</td>';
          $documento .= '<td>' . $valor_saldo . '</td>';
          ...
          //cruce documento
          $documento .= '<td  style=mso-number-format:"@" >' . $ndocumento_compraval . '</td>';
          $documento .= '<td  style=mso-number-format:"@" >' . $ndocumento_ventaval  . '</td>';
          $documento .= '<td  style=mso-number-format:"@" >' . $persona_val . '</td>';
          $documento .= '<td  style=mso-number-format:"@" >' . $cuenta_val . '</td>';
          $documento .= '<td  style=mso-number-format:"@" >' . $crudet_fechaemision . '</td>';
          $documento .= '<td  style=mso-number-format:"@" >' . $crudet_valor . '</td>';
          $documento .= '<td>' . $saldo_final . '</td>';
...
inner join lpin_movanticipos on app_anticipos.anti_enlace=lpin_movanticipos.anti_enlace
          ...
          if ($rs_anticipo) {
            while (!$rs_anticipo->EOF) {
              ...
              $documento .= '<td  style=mso-number-format:"@" ></td>';
              $documento .= '<td  style=mso-number-format:"@" >' . $rs_anticipo->fields["provee_nombre"]  . '</td>';
              $documento .= '<td  style=mso-number-format:"@" >' . $rs_anticipo->fields["plancant_id"] . '</td>';
              $documento .= '<td  style=mso-number-format:"@" >' . $rs_anticipo->fields["anti_fechaemision"] . '</td>';

              $documento .= '<td  style=mso-number-format:"@" >' . str_replace(".", ",", $rs_crudetaanticipo->fields["cruant_valorpago"]) . '</td>';
              //cruce anticipo
              $documento .= '<td></td>';
      ...
      //CUENTA CONTABLE
      $docu_cru = "select * from pichinchahumana_extension.lpin_crucecuentas left join pichinchahumana_original.app_proveedor on pichinchahumana_extension.lpin_crucecuentas.crucue_persona=pichinchahumana_original.app_proveedor.provee_id where crudoc_enlace='" . $rs_data->fields["crudoc_enlace"] . "'";
      $rs_cuentacontablecrudeta = $DB_gogess->executec($docu_cru, array());
      if ($rs_cuentacontablecrudeta) {
        while (!$rs_cuentacontablecrudeta->EOF) {

          $tipo_cruceval = 'CUENTA CONTABLE';
          $crudet_documento = $rs_cuentacontablecrudeta->fields["crudet_documento"];
          $crudet_documentoventa = $rs_cuentacontablecrudeta->fields["crudet_documentoventa"];
          $crudet_fechaemision = $rs_cuentacontablecrudeta->fields["crudet_fechaemision"];
          $crudet_tipodocumento = $rs_cuentacontablecrudeta->fields["crudet_tipodocumento"];
          $crucue_valorpago = $rs_cuentacontablecrudeta->fields["crucue_valorpago"];
          ...
          $documento .= '<td  style=mso-number-format:"@" ></td>';
          $documento .= '<td  style=mso-number-format:"@" ></td>';
          $documento .= '<td  style=mso-number-format:"@" >' . $rs_cuentacontablecrudeta->fields["crucue_cuenta"] . '</td>';
          $documento .= '<td  style=mso-number-format:"@" >' . $rs_data->fields["crudoc_fechaemision"] . '</td>';
          $documento .= '<td  style=mso-number-format:"@" >' . str_replace(".", ",", $crucue_valorpago) . '</td>';
          //cruce cc
          $documento .= '<td></td>';
          //lista data
          $documento .= '</tr>';
          ...
          $rs_cuentacontablecrudeta->MoveNext();
        }
      }
