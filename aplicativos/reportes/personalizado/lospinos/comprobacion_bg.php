...
//busca inicial
$fecha_anterio=date("Y-m-d",strtotime($fecha_i."- 1 days")); 


$separar_fechainicial=array();
$separa_anterior=array();

$separar_fechainicial=explode("-",$fecha_i);
$separa_anterior=explode("-",$fecha_anterio);

$concatena_sqlinicial='';
//echo "Anterios:".$separa_anterior[0]."<br>";
//echo "Busca:".$separar_fechainicial[0]."<br>";
if($separa_anterior[0]<$separar_fechainicial[0])
{
//rango busca inicial
$fecha_rangoi=$separa_anterior[0]."-01-01";
$fecha_rangof=$fecha_anterio;
//echo $fecha_rangoi." -> ".$fecha_rangof;
$concatena_incial="";
$concatena_incial=" comcont_fecha>='".$fecha_rangoi."' and comcont_fecha<='".$fecha_rangof."' ";
$suma_existe="select count(*) as cantidad from lpin_detallecomprobantecontable_vista where ".$concatena_incial." and comcont_anulado=0";
$rs_existe = $DB_gogess->executec($suma_existe,array());  
if($rs_existe->fields[cantidad]>0)
{
 //++++++++++++++++
  $concatena_sqlinicial=$concatena_incial;
 //++++++++++++++++ 
}
else
{
//================
$suma_existeinicalasiento="select * from lpin_comprobantecontable where ".$concatena_sql." and comcont_anulado=0 and comcont_inicial=1";
$rs_existeiniasiento = $DB_gogess->executec($suma_existeinicalasiento,array());  

if($rs_existeiniasiento->fields["comcont_id"]>0)
{
  $concatena_sqlinicial=" comcont_id='".$rs_existeiniasiento->fields["comcont_id"]."' ";
}
//==================
}
//rango busca inicial
...
$documento.='<table width="700" border="1" cellpadding="2" cellspacing="0">
    <tr>
    <td width="153" ><div align="center"><strong>CODIGO</strong></div></td>
	<td width="356" ><div align="center"><strong>CUENTA</strong></div></td>
    <td width="179" ><div align="center"><strong>SALDO INICIAL</strong></div></td>
	<td width="179" ><div align="center"><strong>DEBE</strong></div></td>
...
$ib=0;
 if($rs_listadiario)
 {
	 ...
	 // and tipoa_id=6
	  ...
	  $sumatotales="select round(sum(detcc_debe - detcc_haber),2) as totales from lpin_detallecomprobantecontable_vista where ".$concatena_sqlinicial." and detcc_cuentacontablep like '".$rs_listadiario->fields["planc_codigoc"].".%' and comcont_anulado=0";
	 ...
	 {		 
	  ...
	  $suma_cuentas[trim($rs_listadiario->fields["planc_codigoc"])]=number_format($valor_data, 2, '.', '');

$suma_debe_valor=0;	  
$suma_haber_valor=0;

$busca_debe="select round(sum(detcc_debe),2) as totalesd from lpin_detallecomprobantecontable_vista where ".$concatena_sql." and detcc_cuentacontablep like '".$rs_listadiario->fields["planc_codigoc"].".%' and comcont_anulado=0 and  comcont_inicial=0";
$rs_sdebe = $DB_gogess->executec($busca_debe,array());
$suma_debe_valor=$rs_sdebe->fields["totalesd"];

$busca_haber="select round(sum(detcc_haber),2) as totalesh from lpin_detallecomprobantecontable_vista where ".$concatena_sql." and detcc_cuentacontablep like '".$rs_listadiario->fields["planc_codigoc"].".%' and comcont_anulado=0 and  comcont_inicial=0";
$rs_shaber = $DB_gogess->executec($busca_haber,array());
$suma_haber_valor=$rs_shaber->fields["totalesh"];

	 
	  $documento.='<td>'.number_format($suma_debe_valor, 2, '.', ',').'</td>';
	  $documento.='<td>'.number_format($suma_haber_valor, 2, '.', ',').'</td>';  
	  
// a partir de la cuenta 2,3 la formula es 
//$saldo_valorcomp=$valor_data-$suma_debe_valor+$suma_haber_valor;	
	  
$saldo_valorcomp=$valor_data+$suma_debe_valor-$suma_haber_valor;	  
	
	  
	  $documento.='<td>'.number_format($saldo_valorcomp, 2, '.', ',').'</td>';  
