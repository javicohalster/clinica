...
$comprobante_codex=@$_POST["comprobante_codex"];
}
else
{
...
$comprobante_codex=@$_GET["comprobante_codex"];
}
...
$suma_haber=0;
$lista_cuentas=array();
$lista_ncuentas=array();

$lista_cuentasfinales="select * from lpin_plancuentas where planc_codigoc like '".$planc_codigoc.".%' order by planc_orden asc";
$rs_cuentapl = $DB_gogess->executec($lista_cuentasfinales,array());
$ib=0;
 if($rs_cuentapl)
 {
     while (!$rs_cuentapl->EOF) {	
	 
	     $cantidadd_valor=0;
	     $cantidadd_valor=busca_conarbol($rs_cuentapl->fields["planc_codigoc"],$DB_gogess);
	     
		 if($cantidadd_valor==0)
	     {
	       $lista_cuentas[$ib]=$rs_cuentapl->fields["planc_codigoc"];
		   $lista_cuentasn[$ib]=$rs_cuentapl->fields["planc_nombre"];
		   $ib++;
	     }
		 
	  
	    $rs_cuentapl->MoveNext();
	 }
  }	 


$documentocab='<table width="700" border="1" align="center" cellpadding="0" cellspacing="0">';

$acumula_data='';

for($o=0;$o<count($lista_cuentas);$o++)
{

if($lista_cuentas[$o])
{
$sql6=" detcc_cuentacontablep = '".trim($lista_cuentas[$o]).".' and ";
}  

if($comprobante_codex)
{
$sql7=" comcont_id = '".trim($comprobante_codex)."' and ";
}

$suma_debe=0;
$suma_haber=0;

$concatena_sql=$sql1.$sql2.$sql3.$sql4.$sql5.$sql6.$sql7;
...
$listadiariohaycantidad="select count(*) as total from lpin_detallecomprobantecontable_vista where ".$concatena_sql." and comcont_anulado=0 order by comcont_fecha,detcc_id asc";
$rs_haycantidad = $DB_gogess->executec($listadiariohaycantidad,array());

if($rs_haycantidad->fields["total"]>0)
{
//=======================================================================

$acumula_data.='<tr>
    <td><b>'.$lista_cuentas[$o].'</b></td>
    <td><b>'.$lista_cuentasn[$o].'</b></td>
  </tr>';
...
$acumula_data.='<table width="700" border="1"  cellpadding="2" cellspacing="0">
    <tr bgcolor="#E9E9E9" >
    <td><div align="center"><strong>ID COMPROBANTE</strong></div></td>
	<td><div align="center"><strong></strong></div></td>
    <td><div align="center"><strong>Cuenta</strong></div></td>
	<td><div align="center"><strong>Nombre cuenta</strong></div></td>
	...
	<td><div align="center"><strong>Ndoc</strong></div></td>
...
$lista_cuentasx="select * from lpin_plancuentas_vista where planc_codigocp='".$rs_listadiario->fields["detcc_cuentacontablep"]."'";
$rs_cuentasx = $DB_gogess->executec($lista_cuentasx,array());
	 ...
	 $acumula_data.='<tr>
			...
			<td nowrap="nowrap" >'.$rs_cuentasx->fields["planc_nombre"].'</td>
			<td nowrap="nowrap" >'.$rs_listadiario->fields["comcont_fecha"].'</td>
			<td>'.$rs_listadiario->fields["comcont_concepto"].'</td>				
			<td>'.$rs_listadiario->fields["detcc_descricpion"].'</td>
			<td style=mso-number-format:"@"  >'.$rs_listadiario->fields["comcont_numeroc"].'</td>			
...
$valor_final=round($suma_debe, 2)-round($suma_haber, 2);
 ...
 $acumula_data.='<tr bgcolor="#E9E9E9" >
			<td></td>	
			<td nowrap="nowrap" ></td>
			<td nowrap="nowrap" ></td>
			<td nowrap="nowrap" ></td>
...
$documento.=$documentocab.$acumula_data.$documentopie;
