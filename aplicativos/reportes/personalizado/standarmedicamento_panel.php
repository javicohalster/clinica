...
<SCRIPT LANGUAGE=javascript>
	...
	function ejecuta_reporte() {
		...
		$("#ver_reporte").load("lospinos/listastandar_medicamento.php", {
...
<table width="500" border="1" align="center" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td bgcolor="#E6F1F2" class="style1">CODIGO</td>
		<td bgcolor="#E6F1F2" class="style1">RED PUBLICA</td>
		<td bgcolor="#E6F1F2" class="style1">PRIVADA</td>
		<td bgcolor="#E6F1F2" class="style1"></td>
		<td bgcolor="#E6F1F2" class="style1"></td>
		<td bgcolor="#E6F1F2" class="style1">&nbsp;</td>
		<td bgcolor="#E6F1F2" class="style1">&nbsp;</td>
	...
	<tr>
		...
		<td></td>
		<td>
			<input name="clie_institucionval" type="text" id="clie_institucionval" value="" />
		</td>
		<td>
		...
		<td>
			<select name="precu_activo" id="precu_activo">
				...
				<?php
				$objformulario->fill_cmb("gogess_sino", "value,etiqueta", @$precu_activo, @$orden, $DB_gogess);
				?>
			</select>
		...
		<td>
			...
			<select name="precu_facturar" id="precu_facturar">
				<option value="">--seleccionar--</option>
				<?php
				$objformulario->fill_cmb("gogess_sino", "value,etiqueta", @$precu_activo, @$orden, $DB_gogess);
				?>
...
<script type="text/javascript">
	...
	function ver_pdf() {
		...
		window.open("lospinos/listastandar_medicamento.php?precu_facturar=" + $('#precu_facturar').val() + "&precu_activo=" + $('#precu_activo').val() + "&convepr_id=" + $('#convepr_id').val() + "&categesp_id=" + $('#categesp_id').val() + "&especipr_id=" + $('#especipr_id').val() + "&clie_id=" + $('#clie_id').val() + "&centro_id=" + $('#centro_id').val() + "&fecha_inicio=" + $('#fecha_inicio').val() + "&fecha_fin=" + $('#fecha_fin').val() + "&previsual=3&clie_institucionval=" + $('#clie_institucionval').val() + "&ireport=<?php echo $ireport; ?>", '_blank');
