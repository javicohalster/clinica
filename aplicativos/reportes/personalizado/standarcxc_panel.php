...
function ejecuta_reporte()
{
 ...
 $("#ver_reporte").load("lospinos/listastandar_cxc.php",{
...
<table width="500" border="1" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<td>
	<select name="doccab_rucci_cliente" id="doccab_rucci_cliente" style="width:200px">
      <option value="">--seleccionar--</option>
	  <?php
	
$busca_profesional="select distinct doccab_rucci_cliente,doccab_nombrerazon_cliente from beko_documentocabecera_vista order by doccab_nombrerazon_cliente asc";
$rs_espe = $DB_gogess->Execute($busca_profesional);
if($rs_espe)
{
   while (!$rs_espe->EOF) {
   
   echo '<option value="'.$rs_espe->fields["doccab_rucci_cliente"].'">'.$rs_espe->fields["doccab_nombrerazon_cliente"].'</option>';
...
function ver_excel()
{
 if($('#fecha_inicio').val()=='' && $('#fecha_fin').val()=='')
 {
  ...
  location.href = "lospinos/listastandar_cxc.php?doccab_rucci_cliente="+$('#doccab_rucci_cliente').val()+"&centro_id="+$('#centro_id').val()+"&fecha_inicio="+$('#fecha_inicio').val()+"&fecha_fin="+$('#fecha_fin').val()+"&previsual=2&clie_institucionval="+$('#clie_institucionval').val()+"&ireport=<?php echo $ireport; ?>";
...
function ver_pdf()
{
 ...
 if($('#fecha_inicio').val()=='' && $('#fecha_fin').val()=='')
 {
  ...
  return false;
   ...
   window.open("lospinos/listastandar_cxc.php?doccab_rucci_cliente="+$('#doccab_rucci_cliente').val()+"&centro_id="+$('#centro_id').val()+"&fecha_inicio="+$('#fecha_inicio').val()+"&fecha_fin="+$('#fecha_fin').val()+"&previsual=3&clie_institucionval="+$('#clie_institucionval').val()+"&ireport=<?php echo $ireport; ?>", '_blank');  
