...
<SCRIPT LANGUAGE=javascript>
	...
	function ejecuta_reporte() {
		...
		$("#ver_reporte").load("lospinos/listastandar_anticipo.php", {
...
<table width="500" border="1" align="center" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td bgcolor="#E6F1F2" class="style1">&nbsp;</td>
		<td bgcolor="#E6F1F2" class="style1">TIPO ANTICIPO</td>
		<td bgcolor="#E6F1F2" class="style1">ID</td>
	...
	<tr>
		...
		<td>
			...
			<select name="clie_institucionval" id="clie_institucionval" style="width:200px"  >
			  ...
			  <?php
				$objformulario->fill_cmb("pichinchahumana_combos.cmb_tipomovantipo","movantic_id, movantic_nombre",@$clie_id,"  order by movantic_nombre asc",$DB_gogess);
			  ?> 
			</select>
		</td>
		<td>
			<input name="clie_id" type="text" id="clie_id" value="" />
		</td>
		<td>
		   
		   <select name="precu_activo" id="precu_activo" style="width:200px" class="js-basic-single1 form-control" >
			  <option value="">--seleccionar--</option>
			  <?php
				$objformulario->fill_cmb("app_proveedor","provee_id,provee_nombre",@$clie_id," where provee_borradologico=0 order by provee_nombre asc",$DB_gogess);
			  ?> 
			</select>
		</td>
...
<script type="text/javascript">
	...
	function ver_pdf() {
		...
		window.open("lospinos/listastandar_anticipo.php?precu_facturar=" + $('#precu_facturar').val() + "&precu_activo=" + $('#precu_activo').val() + "&convepr_id=" + $('#convepr_id').val() + "&categesp_id=" + $('#categesp_id').val() + "&especipr_id=" + $('#especipr_id').val() + "&clie_id=" + $('#clie_id').val() + "&centro_id=" + $('#centro_id').val() + "&fecha_inicio=" + $('#fecha_inicio').val() + "&fecha_fin=" + $('#fecha_fin').val() + "&previsual=3&clie_institucionval=" + $('#clie_institucionval').val() + "&ireport=<?php echo $ireport; ?>", '_blank');
