...
$datos_usuariovalor="select * from app_usuario where usua_id='".$_SESSION['datadarwin2679_sessid_inicio']."'";
$rs_usuariovalor = $DB_gogess->executec($datos_usuariovalor,array());

$usua_permisoepi= $rs_usuariovalor->fields["usua_permisoepi"];
$usua_tiempoepi= $rs_usuariovalor->fields["usua_tiempoepi"];
$intervalo_v=20;

if($usua_permisoepi==1)
{
   $intervalo_v=$usua_tiempoepi;
}
//busca datos del paciente
...
$busca_sihaydata="select ".$campo_primariodata.",anam_fecharegistro,DATE_ADD(anam_fecharegistro,INTERVAL ".$intervalo_v." DAY) as fechacierre from ".$table." where anam_id=?";
...
<?php
//$linkapaciente="onClick=ver_formularioenpantallaextendida('aplicativos/documental/opciones/panel/panel_stnnewepicrisisanamnesis.php','Editar','divBody_ext','','".@$clie_id."','125','".$atenc_id."',0,'".$tipofor_id."','".$centro_id."',0,0,0,'".$protoop_idenlace."','".$protoop_tblprincipal."');";
$linkapaciente="onClick=ver_formularioenpantallaextendida('aplicativos/documental/opciones/panel/panel_stnnewepicrisisanamnesis.php','Editar','divBody_ext','','".@$clie_id."','214','".$atenc_id."',0,'".$tipofor_id."','".$centro_id."',0,0,0,'".$protoop_idenlace."','".$protoop_tblprincipal."');";
...
$linksubsecuente="onClick=ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_stnepicrisissubsecuente.php','Perfil','divBody_ext','',".$clie_id.",'126',".$atenc_id.",0,'".$tipofor_id."',".$centro_id.");";
...
$linkimprimir='onClick=imprimir_datos();';
$linkimprimir='onClick=genera_pdf();';
$linkimprimiremg='onClick=genera_pdfemg();';
...
echo '&nbsp;&nbsp;&nbsp;<button type="button" class="mb-sm btn btn-info" '.$linkimprimiremg.'  style="cursor:pointer"><span class="glyphicon glyphicon-print"></span> IMPRIMIR EMG</button>';
...
function  abre_formulariosecundario()
{
  if($('#<?php echo $campo_primario; ?>').val()>0)
   {
     ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_stnepicrisissubsecuente.php','SECUNDARIO','divBody_ext',0,'<?php echo @$clie_id; ?>','126','<?php echo $atenc_id; ?>','<?php echo $tipofor_id; ?>',$('#<?php echo $campo_primario; ?>').val(),'<?php echo $centro_id; ?>');
...
function genera_pdf()
{
  if($('#<?php echo $campo_primariodata; ?>').val()>0)
	 {	 
	    location.href = "pdfformularios/pdformularionewepicrisis.php?ssr=<?php echo $campos_data64."|" ?>"+$('#<?php echo $campo_primariodata; ?>').val();
	 }
	 else
	 {
       alert("Por favor guarde el resgistro para imprimir");	 
...
function genera_pdfemg()
{
  if($('#<?php echo $campo_primariodata; ?>').val()>0)
	 {		    
		 myWindow3=window.open('pdfformularios/pdformularionewepicrisisemg.php?ssr=<?php echo $campos_data64."|" ?>'+$('#<?php echo $campo_primariodata; ?>').val(),'ventana_reporteudata','width=850,height=700,scrollbars=YES');
		 myWindow3.focus();
