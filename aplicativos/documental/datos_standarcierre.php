...
$busca_control="select cierr_fecha from ".$table." where cierr_id=?";
$rs_control = $DB_gogess->executec($busca_control,array($_POST["pVar1"]));

$bloque_registro=0;
if(@$rs_control->fields["cierr_fecha"])
{
  //echo $rs_sihaydata->fields["fechacierre"];
   if(date("Y-m-d")>$rs_control->fields["cierr_fecha"])
   {
                ...
                <div class="card my-4">
                    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                        <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 panel-heading">
                            <h3 class="text-white text-capitalize ps-3 panel-title" style="color:#000033">
                                <?php echo $rs_tabla->fields["tab_title"]; ?>
                            </h3>
                        </div>
                    </div>
					...
					<div class="ms-3 pe-md-3 align-items-center">
                        <button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary"
                            onclick="ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_standarcierre.php','Perfil','divBody_ext',0,'<?php echo $_POST["pVar2"]; ?>',0,0,0,0,0)"
                            style="cursor:pointer"><span class="glyphicon glyphicon-arrow-left"></span> Regresar
                        </button>

                        <?php
$linkimprimir='onClick=imprimir_datos();';
echo '<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" '.$linkimprimir.'  style="cursor:pointer"><span class="glyphicon glyphicon-print"></span> IMPRIMIR </button></div>';
...
function imprimir_datos() {
    ...
    if ($('#<?php echo $campo_primariodata; ?>').val() > 0) {
        myWindow3 = window.open(
            'aplicativos/documental/datos_substandarprintcierre_print.php?iddata=<?php echo $tab_id ?>&pVar2=&pVar4=&pVar5=' +
