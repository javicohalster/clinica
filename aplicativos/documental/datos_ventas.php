...
$lista_usuariox="select * from app_usuario where usua_id='".$_SESSION['datadarwin2679_sessid_inicio']."'";
$rs_usuariox = $DB_gogess->executec($lista_usuariox,array());

$usua_puedehacernc='';
$pemisionnc_id='';
$usua_puedehacernc=$rs_usuariox->fields["usua_puedehacernc"];
$pemisionnc_id=$rs_usuariox->fields["pemisionnc_id"];
//busca usuario
...
if($_POST["pVar1"]==0)
{
$campos_tipo['proveeve_id'] = 'selectbuscadorfiltrado';
}
...
<div align="center">
		 <div class="form-group">
         <div class="col-xs-12" id="btn_grabardata">
...
<div align="center">
		 <div class="form-group">
         <div class="col-xs-12"  id="btn_grabardata2">
			<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" onclick="guarda_data()"  > GRABAR</button>
		</div>
		</div>
                ...
                <div class="card my-4">
                    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                        <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 panel-heading">
                            <h3 class="text-white text-capitalize ps-3 panel-title" style="color:#000033">
                    ...
                    <div class="card-body px-0 pb-2">
                        <div class="ms-3 pe-md-3 align-items-center">
                            <button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary"
                                onclick="ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_ventas.php','Perfil','divBody_ext',0,'<?php echo $_POST["pVar2"]; ?>',0,0,0,0,0)"
                                style="cursor:pointer"><span class="glyphicon glyphicon-arrow-left"></span>&nbsp;&nbsp;Regresar
                            </button>

                            <?php
$linkimprimir='onClick=imprimir_datos();';
echo '<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" '.$linkimprimir.'  style="cursor:pointer"><span class="glyphicon glyphicon-print"></span>&nbsp;&nbsp;IMPRIMIR </button>';

?>

                            <button type="button"
                                class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary"
                                onclick="ver_formularioenpantalla('aplicativos/documental/datos_ventas.php','TARIFARIO','divBody_ext',0,'<?php echo $mnupan_id; ?>',0,0,0,0,0)"
                                style="cursor:pointer"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;NUEVO
                                REGISTRO</button>
                        </div>
...
/// guarda verific totales antes
function guarda_data() {

    let text;
    if ($('#tipocmp_codigo').val() == '04') {
        //if (confirm("ESTA SEGURO QUE QUIERE GUARDAR ESTA NOTA DE CREDITO") == true) {	 
        //}
        //else
        //{
        //  return false;
    ...
    $("#g_totales").load("templateformsweb/maestro_standar_ventas/totales_sriprevio.php", {
        ...
        if ($('#doccab_total').val() == '0') {

            alert("Documento no puede tener como total un valor de cero...");
            return false;
...
function genera_asiento() {
    ...
    if ($('#tipocmp_codigo').val() == '01') {
        urlasiento = 'aplicativos/documental/asientos/ventas/ejecuta_ventas.php';
    }

    if ($('#tipocmp_codigo').val() == '04') {
        urlasiento = 'aplicativos/documental/asientos/ventas/ejecuta_nc.php';
    }

    $("#asiento_c").load(urlasiento, {
        valor_id: $('#<?php echo $campo_primariodata; ?>').val(),
        nombre_campoid: '<?php echo $campo_primariodata; ?>',
        tabla: '<?php echo $table; ?>',
        mnupan_id: '<?php echo $mnupan_id; ?>'

    }, function(result) {

        genera_asientoretencion();

    });
    $("#asiento_c").html("Espere un momento");
}
<?php
}

if($lista_fac=='001-003-')
{
?>
...
function genera_asiento() {
    ...
    urlasiento = '';
    if ($('#tipocmp_codigo').val() == '01') {
        urlasiento = 'aplicativos/documental/asientos/ventas/ejecuta_ventas_cinv.php';
    }

    if ($('#tipocmp_codigo').val() == '04') {
        urlasiento = 'aplicativos/documental/asientos/ventas/ejecuta_nc_cinv.php';
    }
...
function genera_asientoretencion() {


    $("#asiento_c").load("aplicativos/documental/asientos/ventas/ejecuta_ventasretencion.php", {
        valor_id: $('#<?php echo $campo_primariodata; ?>').val(),
        nombre_campoid: '<?php echo $campo_primariodata; ?>',
        tabla: '<?php echo $table; ?>',
        mnupan_id: '<?php echo $mnupan_id; ?>'
