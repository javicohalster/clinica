...
if($_POST["pVar1"]>0)
{
$busca_ccerradaplq="select * from dns_compras where compra_id='".$_POST["pVar1"]."'";
$rs_ccerradolq = $DB_gogess->executec($busca_ccerradaplq,array());

//echo $rs_ccerradolq->fields["tipdoc_id"];
}

if($lq_data==1 or @$rs_ccerradolq->fields["tipdoc_id"]=='19')
{
		 
   $campos_tipo['tipdoc_id']='hidden2';
   $campos_tipo['compra_nfactura']='hidden2';
   $campos_tipo['compra_autorizacion']='hidden2';
   $campos_tipo['compra_claveacceso']='hidden2';
...
$linkretencion='onClick=verdetallesret_xml();';


$btn_retencion='';
$btn_retencion='<button type="button" class="mb-sm btn btn-info" '.$linkretencion.'  style="cursor:pointer"><span class="glyphicon glyphicon-print"></span> GENERAR RETENCION </button>';
...
<div class="panel panel-default">
 <div class="panel-heading">
    ...
    <h3 class="panel-title" style="color:#000033;font-size:11px" ><?php echo $titulo_formualario."&nbsp;&nbsp;&nbsp;".$btn_imprimir; ?>&nbsp;&nbsp;&nbsp;<?php echo $btn_retencion; ?></h3>
...
function genera_asientocompra()
{
	...
	if($('#tipdoc_id').val()=='9')
	{
	   urlasiento='aplicativos/documental/asientos/compras/ejecuta_nc.php';	
	}
	else
	{
	  urlasiento='aplicativos/documental/asientos/compras/ejecuta_compras.php';	
	}
   

   $("#asiento_cc").load(urlasiento,{
  ...
  $("#asiento_cc").html("Espere un momento");  
...
function genera_asientoretencion()
{
   ...
   $("#asiento_cc").load("aplicativos/documental/asientos/compras/ejecuta_comprasretencion.php",{
      valor_id:$('#<?php echo $campo_primariodata; ?>').val(),
	  nombre_campoid:'<?php echo $campo_primariodata; ?>',
	  tabla:'<?php echo $table; ?>',
	  mnupan_id:'<?php echo $mnupan_id; ?>'
...
function genera_asientoretencion2()
{


   $("#asiento_cc").load("aplicativos/documental/asientos/compras/ejecuta_comprasretenciongasto.php",{
      valor_id:$('#<?php echo $campo_primariodata; ?>').val(),
	  nombre_campoid:'<?php echo $campo_primariodata; ?>',
	  tabla:'<?php echo $table; ?>',
	  mnupan_id:'<?php echo $mnupan_id; ?>'
