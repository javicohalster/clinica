...
<b>Imprimir solo lo registrado por el usuario actual:</b><input name="check_imp" type="checkbox" id="check_imp" value="1" /><br />
<?php
$linkapaciente="onClick=ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_substandarformunicoanamnesis.php','Editar','divBody_ext','','".@$clie_id."','44','".$atenc_id."',0,'".$tipofor_id."','".$centro_id."');";
...
$linksubsecuente="onClick=ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_substandaranamnesissubsecuente.php','Perfil','divBody_ext','',".$clie_id.",'59',".$atenc_id.",0,'".$tipofor_id."',".$centro_id.");";
...
<?php
$comill_s="'";
$link_laboratorio= 'abrir_standar('.$comill_s.'aplicativos/documental/opciones/panel/panel_substandarformlab.php'.$comill_s.','.$comill_s.'LABORATORIO'.$comill_s.','.$comill_s.'divBodypop_ext'.$comill_s.','.$comill_s.'divDialogpop_ext'.$comill_s.',900,700,'.$comill_s.$comill_s.','.$comill_s.@$clie_id.$comill_s.','.$comill_s.'60'.$comill_s.','.$comill_s.$atenc_id.$comill_s.',$('.$comill_s.'#'.$campo_primario.$comill_s.').val(),'.$comill_s.'dns_anamesisexamenfisico'.$comill_s.','.$comill_s.$centro_id.$comill_s.');';   
...
function  abre_formulariosecundario()
{
  if($('#<?php echo $campo_primario; ?>').val()>0)
   {
     ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_substandaranamnesissubsecuente.php','SECUNDARIO','divBody_ext',0,'<?php echo @$clie_id; ?>','59','<?php echo $atenc_id; ?>','<?php echo $tipofor_id; ?>',$('#<?php echo $campo_primario; ?>').val(),'<?php echo $centro_id; ?>');
  ...
  if($('#<?php echo $campo_primariodata; ?>').val()>0)
	 {
	    ...
	    location.href = "pdfformularios/pdformulario.php?ssr=<?php echo $campos_data64."|" ?>"+$('#<?php echo $campo_primariodata; ?>').val()+"&filtro="+$('input:checkbox[name=check_imp]:checked').val();
	 }
	 else
	 {
       alert("Por favor guarde el resgistro para imprimir"); 	 
...
function genera_cobro()
{
  $("#genera_pro").load("aplicativos/documental/g_cuentas.php",{
