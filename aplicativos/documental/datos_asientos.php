...
<div align="center">
         ...
         <div class="col-xs-12"><div id="boton_guardarformdata">
			<button type="button" class="mb-sm btn btn-primary" onclick="verifica_cuadre()"  > GRABAR  </button>
		</div></div>
		</div>
	...
	<div class="container" style="padding-top: 1em; padding-right:1em; padding-left:1em; max-width:100%;">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				$linkimprimirpdf = 'onClick=imprimir_datospdf();';

				echo '&nbsp;&nbsp;&nbsp;<button type="button" class="mb-sm btn btn-info" ' . $linkimprimirpdf . '  style="cursor:pointer"><span class="glyphicon glyphicon-print"></span> IMPRIMIR PDF </button>';
				...
				<?php
				if ($csearch > 0) {
					$busca_bloqueo = "select * from lpin_comprobantecontable where comcont_id='" . $csearch . "'";
					$rs_bloqueodata = $DB_gogess->executec($busca_bloqueo, array());

					if ($rs_bloqueodata->fields["tipoa_id"] != 6) {
...
';
	...
	<script type="text/javascript">
		...
		function verifica_cuadre() {
			if ($('#cuadra_valor').val() == '1') {
				enviar_formulariodata('form_<?php echo $table; ?>');
			} else {
		...
		function imprimir_datospdf() {
			if ($('#<?php echo $campo_primariodata; ?>').val() > 0) {
				myWindow3 = window.open('pdfasientos/pdfasientopdf.php?xml=' + $('#<?php echo $campo_primariodata; ?>').val(), 'ventana_reporteunico', 'width=850,height=700,scrollbars=YES');
