...
$lista_servicios="SELECT usuaat_id,prof_id,sum(docdet_cantidad) as valt FROM beko_recibocabecera inner join beko_recibodetalle on beko_recibocabecera.doccab_id=beko_recibodetalle.doccab_id  where (DATE_FORMAT(doccab_fechaemision_cliente,'%Y-%m-%d')>='".$desde_val."' and DATE_FORMAT(doccab_fechaemision_cliente,'%Y-%m-%d')<='".$hasta_val."') and beko_recibocabecera.centro_id = '".$_GET['centro_id']."' and doccab_anulado=0 group by prof_id,usuaat_id";
...
';
 $cuentasn=0;
$suma_valortotales=0;
$documento.='<center><b>RECIBOS</b><br>
