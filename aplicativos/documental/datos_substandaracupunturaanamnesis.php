...
$busca_alerg="select anam_txtalergias from dns_traumatologiaanamesis where clie_id='".$clie_id."' and tipoaler_id=1";
$rs_balerg = $DB_gogess->executec($busca_alerg,array());
 if($rs_balerg)
 {
	  while (!$rs_balerg->EOF) {
...
$busca_alerg="select anam_txtalergias from dns_cardiologiaanamesis where clie_id='".$clie_id."' and tipoaler_id=1";
$rs_balerg = $DB_gogess->executec($busca_alerg,array());
 if($rs_balerg)
 {
	  while (!$rs_balerg->EOF) {
...
$busca_alerg="select anam_txtalergias from dns_rehabilitacionanamesis where clie_id='".$clie_id."' and tipoaler_id=1";
$rs_balerg = $DB_gogess->executec($busca_alerg,array());
 if($rs_balerg)
 {
	  while (!$rs_balerg->EOF) {
...
$busca_alerg="select anam_txtalergias from dns_pediatriaanamesis where clie_id='".$clie_id."' and tipoaler_id=1";
$rs_balerg = $DB_gogess->executec($busca_alerg,array());
 if($rs_balerg)
 {
	  while (!$rs_balerg->EOF) {
...
$busca_alerg="select anam_txtalergias from dns_acupunturaanamesis where clie_id='".$clie_id."' and tipoaler_id=1";
$rs_balerg = $DB_gogess->executec($busca_alerg,array());
 if($rs_balerg)
 {
	  while (!$rs_balerg->EOF) {
...
<?php
$linkapaciente="onClick=ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_substandaracupunturaanamnesis.php','Editar','divBody_ext','','".@$clie_id."','117','".$atenc_id."',0,'".$tipofor_id."','".$centro_id."');";
...
$linksubsecuente="onClick=ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_substandaracupunturasubsecuente.php','Perfil','divBody_ext','',".$clie_id.",'118',".$atenc_id.",0,'".$tipofor_id."',".$centro_id.");";

$linksubsecuente="onClick=abre_formulariosecundario()";

echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button style="width:380px" type="button" class="mb-sm btn btn-primary" '.$linksubsecuente.'  style="cursor:pointer">  <span class="glyphicon glyphicon-list"></span> REGISTRO DE  - EVOLUCI&Oacute;N Y PRESCRIPCIONES </button>';



//cambio de fecha registro
$linkcambior="onclick=abrir_standar('aplicativos/documental/cambio_fecha.php','CAMBIO','divBody_foto','divDialog_foto',500,300,'acupuntura',$('#anam_id').val(),0,0,0,0,0) style='cursor:pointer'";
//echo '&nbsp;&nbsp;&nbsp;<button type="button" class="mb-sm btn btn-warning" '.$linkcambior.'  style="cursor:pointer"> CAMBIAR FECHA REGISTRO </button>';

$linkhistorialr="onclick=abrir_standar('aplicativos/documental/historial_fecha.php','CAMBIO','divBody_foto','divDialog_foto',790,300,'acupuntura',$('#anam_id').val(),0,0,0,0,0) style='cursor:pointer'";
...
function  abre_formulariosecundario()
{
  if($('#<?php echo $campo_primario; ?>').val()>0)
   {
     ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_substandaracupunturasubsecuente.php','SECUNDARIO','divBody_ext',0,'<?php echo @$clie_id; ?>','118','<?php echo $atenc_id; ?>','<?php echo $tipofor_id; ?>',$('#<?php echo $campo_primario; ?>').val(),'<?php echo $centro_id; ?>');
  ...
  if($('#<?php echo $campo_primariodata; ?>').val()>0)
	 {
	    ...
	    location.href = "pdfformularios/pdformularioacupuntura.php?ssr=<?php echo $campos_data64."|" ?>"+$('#<?php echo $campo_primariodata; ?>').val();
	 }
	 else
	 {
       alert("Por favor guarde el resgistro para imprimir"); 	 
