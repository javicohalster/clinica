...
$mensaje_alergia='';
$alergia_medica='';
$busca_alerg="select anam_txtalergias from dns_ginecologiaanamesis where clie_id='".$clie_id."' and tipoaler_id=1";
...
$busca_alerg="select anam_txtalergias from dns_anamesisexamenfisico where clie_id='".$clie_id."' and tipoaler_id=1";
$rs_balerg = $DB_gogess->executec($busca_alerg,array());
 if($rs_balerg)
 {
	  while (!$rs_balerg->EOF) {
...
<?php
$linkapaciente="onClick=ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_substandarginecolgiaanamnesis.php','Editar','divBody_ext','','".@$clie_id."','105','".$atenc_id."',0,'".$tipofor_id."','".$centro_id."');";
...
$linksubsecuente="onClick=ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_substandarginecologiasubsecuente.php','Perfil','divBody_ext','',".$clie_id.",'59',".$atenc_id.",0,'".$tipofor_id."',".$centro_id.");";
...
$linkcambior="onclick=abrir_standar('aplicativos/documental/cambio_fecha.php','CAMBIO','divBody_foto','divDialog_foto',500,300,'ginecologia',$('#anam_id').val(),0,0,0,0,0) style='cursor:pointer'";
echo '&nbsp;&nbsp;&nbsp;<button type="button" class="mb-sm btn btn-warning" '.$linkcambior.'  style="cursor:pointer"> CAMBIAR FECHA REGISTRO </button>';

$linkhistorialr="onclick=abrir_standar('aplicativos/documental/historial_fecha.php','CAMBIO','divBody_foto','divDialog_foto',790,300,'ginecologia',$('#anam_id').val(),0,0,0,0,0) style='cursor:pointer'";
...
<?php
$comill_s="'";
$link_laboratorio= 'abrir_standar('.$comill_s.'aplicativos/documental/opciones/panel/panel_substandarformlab.php'.$comill_s.','.$comill_s.'LABORATORIO'.$comill_s.','.$comill_s.'divBodypop_ext'.$comill_s.','.$comill_s.'divDialogpop_ext'.$comill_s.',900,700,'.$comill_s.$comill_s.','.$comill_s.@$clie_id.$comill_s.','.$comill_s.'60'.$comill_s.','.$comill_s.$atenc_id.$comill_s.',$('.$comill_s.'#'.$campo_primario.$comill_s.').val(),'.$comill_s.'dns_ginecologiaanamesis'.$comill_s.','.$comill_s.$centro_id.$comill_s.');';   
...
function  abre_formulariosecundario()
{
  if($('#<?php echo $campo_primario; ?>').val()>0)
   {
     ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_substandarginecologiasubsecuente.php','SECUNDARIO','divBody_ext',0,'<?php echo @$clie_id; ?>','106','<?php echo $atenc_id; ?>','<?php echo $tipofor_id; ?>',$('#<?php echo $campo_primario; ?>').val(),'<?php echo $centro_id; ?>');
  ...
  if($('#<?php echo $campo_primariodata; ?>').val()>0)
	 {
	    ...
	    location.href = "pdfformularios/pdformularioginecologia.php?ssr=<?php echo $campos_data64."|" ?>"+$('#<?php echo $campo_primariodata; ?>').val();
	 }
	 else
	 {
       alert("Por favor guarde el resgistro para imprimir"); 	 
