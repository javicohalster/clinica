...
<?php
$lista_partida="select distinct cuadrobm_partidapresupuest from dns_compras co left join dns_principalmovimientoinventario det on co.compra_id=det.compra_id inner join cmb_dns_cuadrobasicomedicamentos cm on cm.cuadrobm_id=det.cuadrobm_id where compra_fechaaprobacion<='".$fechai."' and year(compra_fechaaprobacion)>='".$per_activo."' and cm.categ_id='".$categ_id."' and det.tipomov_id in (17,8,9,10,16) and cuadrobm_partidapresupuest!='' order by nombre_med asc";
...
$rs_partida = $DB_gogess->executec($lista_partida);
                   if($rs_partida)
				   {
						while (!$rs_partida->EOF) {
							...
							if($rs_partida->fields["cuadrobm_partidapresupuest"])
							{
?>
