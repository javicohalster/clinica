...
if($categ_id==2)
{
$bu_fcorte="select distinct det.cuadrobm_id,moviin_nlote,moviin_preciocompra,moviin_fechadecaducidad from dns_movimientoinventario det inner join cmb_dns_cuadrobasicomedicamentos cm on cm.cuadrobm_id=det.cuadrobm_id where date_format(moviin_fecharegistro, '%Y-%m-%d')<='".$fechai."' and year(moviin_fecharegistro) >=".$per_activo." and cm.categ_id='".$categ_id."' and  cm.subcateg_id='".$subcateg_id."' and tipom_id=1 and centro_id='".$centro_id."' ".$subo_script." order by moviin_fechadecaducidad asc";
}
else
{
  $bu_fcorte="select distinct det.cuadrobm_id,moviin_nlote,moviin_preciocompra,moviin_fechadecaducidad from dns_movimientoinventario det inner join cmb_dns_cuadrobasicomedicamentos cm on cm.cuadrobm_id=det.cuadrobm_id where date_format(moviin_fecharegistro, '%Y-%m-%d')<='".$fechai."' and year(moviin_fecharegistro) >=".$per_activo." and cm.categ_id='".$categ_id."' and tipom_id=1 and centro_id='".$centro_id."' ".$subo_script." order by moviin_fechadecaducidad asc";
