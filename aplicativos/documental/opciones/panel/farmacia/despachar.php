...
$select_bu="select cuadrobm_id from dns_cuadrobasicomedicamentos where cuadrobm_codigoatc ='".$rs_bupedido->fields["plantra_codigo"]."'";
...
$rs_bultimomovi = $DB_gogess->executec($busca_movi,array());
//busca el ultimo movimiento inventario
...
{
$despachar_data="update ".$_POST["tabla"]." set usuad_id='".$_POST["usua_id"]."',centrod_id='".$_POST["centro_id"]."',plantra_despachado='SI',plantra_fechadespacho=plantra_fecharegistro where plantra_id='".$_POST["plantra_id"]."';";
}
else
{
...
if($rs_ddata)
{
	...
	//busca
		...
		{
	//busca
	$inserta_movi="insert into dns_stockactual (centro_id,cuadrobm_id,unid_id,moviin_cantidadunidadconsumo,uniddesg_id,moviin_id,stock_cantidad,stock_fechaureg,stock_signo,stock_tabla,stock_idtbla, 	stock_tipo) VALUES ('".$_POST["centro_id"]."','".$rs_bu->fields["cuadrobm_id"]."','".$rs_bultimomovi->fields["unid_id"]."','".$rs_bultimomovi->fields["moviin_cantidadunidadconsumo"]."','".$rs_bultimomovi->fields["uniddesg_id"]."',0,'".$rs_bupedido->fields["plantra_cantidad"]."','".date("Y-m-d H:i:s")."',-1,'".$_POST["tabla"]."','".$_POST["plantra_id"]."','CONSUMO')";
	$rs_insertmovi= $DB_gogess->executec($inserta_movi,array());
	//busca	
