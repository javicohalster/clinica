...
$busca_pedido="select * from ".$_POST["tabla"]." where plantrai_id='".$_POST["plantra_id"]."'";
$rs_bupedido= $DB_gogess->executec($busca_pedido,array());


$select_bu="select * from dns_cuadrobasicomedicamentos where cuadrobm_codigoatc in (select plantrai_codigo from ".$_POST["tabla"]." where plantrai_id='".$_POST["plantra_id"]."')";
...
if(trim($rs_stactua->fields["stactual"]*1)>=trim($rs_bupedido->fields["plantrai_cantidad"]*1))
{
//-----------------------------------
...
//BUSCA PERMISO ESPECIAL
$busca_espcial="select permid_id from  pichinchahumana_extension.dns_permisosdespacho where 	permid_tabla='".$_POST["tabla"]."' and 	permid_idtabla='".$_POST["plantra_id"]."' and permid_motivo='INGRESO_ANTERIOR_SOPORTE_SISTEMA'";
$rs_busmotivo = $DB_gogess->executec($busca_espcial,array());
//BUSCA PERMISO ESPECIAL

if($rs_busmotivo->fields["permid_id"]>0)
{
$despachar_data="update ".$_POST["tabla"]." set usuad_id='".$_POST["usua_id"]."',centrod_id='".$_POST["centro_id"]."',plantrai_despachado='SI',plantrai_fechadespacho=plantrai_fecharegistro where plantrai_id='".$_POST["plantra_id"]."';";
}
else
{
$despachar_data="update ".$_POST["tabla"]." set usuad_id='".$_POST["usua_id"]."',centrod_id='".$_POST["centro_id"]."',plantrai_despachado='SI',plantrai_fechadespacho='".date("Y-m-d H:i:s")."' where plantrai_id='".$_POST["plantra_id"]."';";
...
if($rs_ddata)
{
	...
	//busca  
			...
			{
	  //busca
	$inserta_movi="insert into dns_stockactual (centro_id,cuadrobm_id,unid_id,moviin_cantidadunidadconsumo,uniddesg_id,moviin_id,stock_cantidad,stock_fechaureg,stock_signo,stock_tabla,stock_idtbla,stock_tipo) VALUES ('".$_POST["centro_id"]."','".$rs_bu->fields["cuadrobm_id"]."','".$rs_bultimomovi->fields["unid_id"]."','".$rs_bultimomovi->fields["moviin_cantidadunidadconsumo"]."','".$rs_bultimomovi->fields["uniddesg_id"]."',0,'".$rs_bupedido->fields["plantrai_cantidad"]."','".date("Y-m-d H:i:s")."',-1,'".$_POST["tabla"]."','".$_POST["plantra_id"]."','CONSUMO')";
	$rs_insertmovi= $DB_gogess->executec($inserta_movi,array());
	//busca		
