		...
		for($i=1;$i<=42;$i++)
		{
			...
			if($i<$diaSemana || $i>=$last_cell)
			{
				...
				if($fecha_actualdt<$fecha_bu)
				{
				   if($bandera_activa==1)
				   {
				//echo '<tr><td colspan="2" style="border:thin; color:#FFFFFF; cursor:pointer" onclick="'.$link_agregar.'" ><img src="images/agregar.png" /></td></tr>';
					 }
				}
				else
				{
				...
				$busca_citas="select tu.usua_id,integr_hora,cli.clie_id,clie_nombre,clie_apellido,atenc_id from faesa_asigahorario inner join faesa_integragrupo tu on faesa_asigahorario.grup_id=tu.grup_id inner join app_cliente cli  on faesa_asigahorario.clie_id=app_cliente.clie_id where ".$concatena_data." asighor_fecha='".$fecha_bu."'";
				...
				$busca_citas="select tu.usua_id,integr_hora,cli.clie_id,clie_nombre,clie_apellido,atenc_id from faesa_asigahorario inner join faesa_integragrupo tu on faesa_asigahorario.grup_id=tu.grup_id inner join app_cliente cli on faesa_asigahorario.clie_id=app_cliente.clie_id where asighor_fecha='".$fecha_bu."'";
				...
				if($rs_citas)
                   {
	                  while (!$rs_citas->EOF) {	
					   ...
					   echo '<tr>
						     <td>&nbsp;</td>
						     <td>'.$rs_nmedico->fields["usua_codigoiniciales"].' - '.$rs_citas->fields["integr_hora"].'</td>
							 <td>'.utf8_encode($rs_citas->fields["clie_nombre"]).'</td>
					        </tr>';
                ...
                if($concatena_data)
					  {
				$busca_terapias="select terap_id,tu.especi_id,tu.usua_id,terap_hora,cli.clie_id,clie_nombre,clie_apellido from faesa_terapiasregistro  tu inner join app_cliente cli on  tu.clie_id=cli.clie_id  where ".$concatena_data." terap_fecha='".$fecha_bu."' order by terap_hora asc";
				  }
				  else
				  {
				  $busca_terapias="select terap_id,tu.especi_id,tu.usua_id,terap_hora,cli.clie_id,clie_nombre,clie_apellido from faesa_terapiasregistro  tu inner join app_cliente cli on  tu.clie_id=cli.clie_id  where terap_fecha='".$fecha_bu."' order by terap_hora asc";
				...
				if($rs_terapia)
                   {
	                  ...
	                  while (!$rs_terapia->EOF) {	
						     ...
						     <td style="color:#3169D2" ><b> T-'.$rs_area->fields["especi_iniciales"].' '.$rs_terapia->fields["terap_hora"].'<br>'.$rs_nmedico->fields["usua_codigoiniciales"].'</b></td>
							 <td>'.ucwords(strtolower(utf8_encode($rs_terapia->fields["clie_nombre"]))).'</td>
					        </tr>';
...
<br />
<div align="center">
<table id="calendar" width="800px" border="0" >
<tr>
<td width="321">
