	...
	<tr bgcolor="silver">
		...
		for($i=1;$i<=42;$i++)
		{
			...
			if($i<$diaSemana || $i>=$last_cell)
			{
				...
				$link_agregar="abrir_standar('aplicativos/documental/opciones/panel/agendar/seleccion.php','PANTALLA','divBody_pantallag','divDialog_pantallag',600,590,'".$fecha_bu."','".$_POST["atenc_hc"]."','".$_POST["clie_id"]."','".@$_POST["centro_id"]."',0,0,0); ";
				...
				{
				echo '<tr>
					<td colspan="2" style="border:thin; color:#FFFFFF; cursor:pointer" onclick="'.$link_agregar.'" ><img src="images/agregar.png" /></td>
					 </tr>';
				}
				else
				{
				...
				$busca_citas="select tu.usua_id,integr_hora,faesa_asigahorario.clie_id,clie_nombre,clie_apellido,atenc_id from faesa_asigahorario inner join faesa_integragrupo tu on faesa_asigahorario.grup_id=tu.grup_id inner join app_cliente on faesa_asigahorario.clie_id=app_cliente.clie_id where ".$concatena_data." asighor_fecha='".$fecha_bu."'";
				...
				$busca_citas="select tu.usua_id,integr_hora,faesa_asigahorario.clie_id,clie_nombre,clie_apellido,atenc_id from faesa_asigahorario inner join faesa_integragrupo tu on faesa_asigahorario.grup_id=tu.grup_id inner join app_cliente on faesa_asigahorario.clie_id=app_cliente.clie_id where asighor_fecha='".$fecha_bu."'";
                ...
                if($concatena_data)
					  {
				$busca_terapias="select * from faesa_terapiasregistro tu inner join app_cliente on  tu.clie_id=app_cliente.clie_id  where ".$concatena_data." terap_fecha='".$fecha_bu."' order by terap_hora asc";
				  }
				  else
				  {
				  $busca_terapias="select * from faesa_terapiasregistro tu inner join app_cliente on  tu.clie_id=app_cliente.clie_id  where terap_fecha='".$fecha_bu."' order by terap_hora asc";
				...
				if($rs_terapia)
                   {
	                  ...
	                  while (!$rs_terapia->EOF) {	
							 ...
							 echo '<tr>
						     <td onclick="'.$link_b.'" style="cursor:pointer" ><img src="images/borrar_t.png" /></td>
							 ...
							 <td>'.utf8_encode($rs_terapia->fields["clie_nombre"]).'</td>
					        </tr>';
...
<b>T: Terapia</b><br />
<?php
$lista_t="select * from dns_especialidad ";
