...
$campors_buscador="fechai,fechaf,doccab_rucci_cliente,doccab_nombrerazon_cliente,doccab_ndocumento,estado_pp,estado_sri,doccab_anulado";
$lista_campob='';
if($campors_buscador)
{
 $listado_buscador=array();
 $listado_buscador=explode(",",$campors_buscador);
...
function desplegar_grid() {
    $("#grid<?php echo $subindice; ?>").load(
        "aplicativos/documental/opciones/grid/<?php echo $carpeta; ?>/grid<?php echo $subindice; ?>.php", {
            <?php echo $lista_campob; ?>
            pVar2: <?php echo $_POST["pVar2"] ?>,
            campors_buscador: '<?php echo $campors_buscador; ?>'
...
$linkeditar= 'onClick="ver_formularioenpantalla('.$comill_s.'aplicativos/documental/datos_ventas.php'.$comill_s.','.$comill_s.'TARIFARIO'.$comill_s.','.$comill_s.'divBody_ext'.$comill_s.',0,'.$comill_s.$_POST["pVar2"].$comill_s.',0,0,0,0,0)" style=cursor:pointer';	
...
$contenido_btn.='<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" '.$linkeditarbtn.' " ><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;'.$rs_gogessform->fields["vardev_nombre"].'</button>';
...
$contenido_btn.='&nbsp;&nbsp;<a href="aplicativos/reportes/personalizado/panel.php?ireport=53" target="_blank"  class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;TODOS LOS DOCUMENTOS</a>&nbsp;&nbsp;';

$contenido_btn.='&nbsp;&nbsp;<a href="aplicativos/reportes/personalizado/panel.php?ireport=49" target="_blank"  class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;REPORTE SOLO FACTURAS</a>&nbsp;&nbsp;';

$contenido_btn.='&nbsp;&nbsp;<a href="aplicativos/reportes/personalizado/panel.php?ireport=50" target="_blank"  class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;REPORTE SOLO NOTAS DE CREDITO</a>&nbsp;&nbsp;';

$contenido_btn.='&nbsp;&nbsp;<a href="aplicativos/reportes/personalizado/panel.php?ireport=51" target="_blank"  class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;REPORTE SOLO ANULADAS</a>&nbsp;&nbsp;';

//========================================

$busc_usv="select * from app_usuario where usua_id='".$_SESSION['datadarwin2679_sessid_inicio']."'";
$rs_usv = $DB_gogess->executec($busc_usv,array());

if($rs_usv->fields["pemision_id"]>0)
{
echo $contenido_btn.'&nbsp;&nbsp;<br><br><button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary"  '.$linkeditar.'><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;NUEVO REGISTRO</button>';
}
else
{
 echo "<b>Usted no tiene asignado punto de venta...</b>";
}

if($tab_name=='efacsistema_producto')
{
$link_b=' onClick="ver_reportelista()" ';
echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary"  '.$link_b.'>LISTADO</button>';
}
echo '</div>';
echo '</div>';
?>
                                        <?php
include("buscador_ventas.php");
?>
