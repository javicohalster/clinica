...
?>
  <table width="100%" border="1" cellpadding="0" cellspacing="0">
	...
	<?php
	 $valor_total=0;
$lista_precuentas="select * from dns_detalleprecuentaprincipal left join api_bodega on dns_detalleprecuentaprincipal.bodega_id=api_bodega.bode_id  where precu_id='".$precu_id."' and detapre_tipo=3";
...
echo '<input class="form-control" name="cmb_'.$ncampo_val.$rs_lprecuentas->fields[$ide_producto].'" type="text" id="cmb_'.$ncampo_val.$rs_lprecuentas->fields[$ide_producto].'" value="'.round($rs_lprecuentas->fields[$ncampo_val],2).'" size="20" onblur="guardar_camposprcamb('.$tabla_valordata.','.$comulla_simple.$ncampo_val.$comulla_simple.','.$rs_lprecuentas->fields[$ide_producto].',$('.$comulla_simple.'#cmb_'.$ncampo_val.$rs_lprecuentas->fields[$ide_producto].$comulla_simple.').val(),'.$comulla_simple.$ide_producto.$comulla_simple.','.$comulla_simple.$rs_lprecuentas->fields["detapre_cantidad"].$comulla_simple.','.$comulla_simple.$clie_id.$comulla_simple.','.$comulla_simple.$precu_id.$comulla_simple.','.$rs_lprecuentas->fields["detapre_tipo"].')" />';	
echo '</td>';
...
$("#campo_valort").load("aplicativos/documental/opciones/panel/precuentaprincipal/guarda_campostar.php",{
