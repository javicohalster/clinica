...
$stockactual="select sum(stock_cantidad * stock_signo) as stactual from dns_principalstockactual where centro_id=".$centro_id." and cuadrobm_id=".$rs_bu->fields["cuadrobm_id"]." and stock_periodo='".$periodo_actual."'";
...
<table width="200" border="1" align="center" cellpadding="4" cellspacing="0">
  ...
  <?php
  $busca_paraentrega="select * from dns_principalmovimientoinventario where tipom_id=1 and moviin_fecharegistro>='2021-09-01' and cuadrobm_id='".$rs_bu->fields["cuadrobm_id"]."' and year(moviin_fecharegistro)>='".$periodo_actual."' and centro_id='".$centro_id."' order by 	moviin_fechadecaducidad asc";
  ...
  if($rs_paraentrega)
   {
	  ...
	  //envios pacientes
	 //$busca_entregados="select sum(moviin_totalenunidadconsumo) as totalv from dns_principalmovimientoinventario where centro_id='".$centro_id."' and (moviintranscent_id='".$rs_paraentrega->fields["moviin_id"]."' or  moviintranscent_id='0') and tipom_id=2 and moviin_nlote ='".$rs_paraentrega->fields["moviin_nlote"]."' and cuadrobm_id='".$rs_bu->fields["cuadrobm_id"]."' and centro_id='".$centro_id."' and perioac_id='".$periodo_actual."' and moviincent_id=0";
	 
	 $busca_entregados="select sum(moviin_totalenunidadconsumo) as totalv from  dns_principalmovimientoinventario where (moviintranscent_id='".$rs_paraentrega->fields["moviin_id"]."') and centro_id='".$centro_id."' and tipom_id=2 and tomfis_id>=0 and cuadrobm_id='".$rs_bu->fields["cuadrobm_id"]."'";
	  ...
	  $actual_porlote=0;
	  $actual_porlote=$rs_paraentrega->fields["moviin_totalenunidadconsumo"]-$rs_entregados->fields["totalv"]-$cantidad_asig;
	 // $actual_porlote=0;
...
<?php
$compra_nsecx='';
$busca_npd="select distinct dns_egresocentros.egrec_id from dns_temporaldespacho inner join dns_egresocentros on dns_temporaldespacho.egrec_id=dns_egresocentros.egrec_id where dns_temporaldespacho.cuadrobm_id='".$cuadrobm_id."' and egrec_anulado=0 and egrec_procesado=0";
...
{
  echo "ALERTA!!! No se puede entregar ya que aun hay egresos sin procesar con este producto:".$compra_nsecx;
}
?>
...
function entregar_datos(datos_data)
{
  ...
  $("#div_procesaentrega").load("aplicativos/documental/opciones/panel/precuentaprincipal/entregando_medicamentos.php",{
