...
<table id="datatable1" class="display" style="width:100%" border="1" >
  <thead>
  <tr>
    <th>Horario</th> 
	<?php
	...
	if($rs_personal)
    {
	  ...
	  // {
	 echo '<th>'.$rs_personal->fields["usua_nombre"].'<br>'.$nombre_dos[0].'<div style="color:#0099CC; font-weight:bold">'.$n_especialidad.'</div></th>';
	  // }
...
<tbody>
  ...
  for($hi=0;$hi<count($arreglo_horas);$hi++)
	...
	if($rs_personal)
    {
	   ...
	   if ($rs_lbuscat && $rs_lbuscat->fields && isset($rs_lbuscat->fields["terap_id"]) && $rs_lbuscat->fields["terap_id"] > 0)
		{
    		$bandera = 1;
		}
  ...
  ?>		   
	...
	if($rs_personal)
    {
	 ...
	 $lista_buscat="select terap_id,atenc_hc,especi_id,faesa_terapiasregistro.usua_id,faesa_terapiasregistro.clie_id,terap_fecha,terap_hora,terap_autorizacion,terap_estado,terap_fechapago,terap_nfactura,faesa_terapiasregistro.centro_id,faesa_terapiasregistro.usuar_id,terap_fecharegistro,terap_recuperacion,terap_observacion,terap_tipoevatera,tipopac_id,clie_nombre,clie_apellido,terap_motivo,terap_asiste,quiro_id,terap_medicompanies,terap_copago,seg_id from  faesa_terapiasregistro left join app_cliente on faesa_terapiasregistro.clie_id=app_cliente.clie_id where faesa_terapiasregistro.usua_id=".$rs_personal->fields["usua_id"]." and terap_fecha='".$fecha_valor."' and terap_hora='".$arreglo_horas[$hi]."' and terap_cancelado=0";
	   
	  // $lista_buscat="select terap_id,atenc_hc,especi_id,faesa_terapiasregistro.usua_id,faesa_terapiasregistro.clie_id,terap_fecha,terap_hora,terap_autorizacion,terap_estado,terap_fechapago,terap_nfactura,faesa_terapiasregistro.centro_id,faesa_terapiasregistro.usuar_id,terap_fecharegistro,terap_recuperacion,terap_observacion,terap_tipoevatera,tipopac_id,clie_nombre,clie_apellido,terap_motivo,terap_asiste,quiro_id,terap_medicompanies,terap_copago,conve.conve_nombre from  faesa_terapiasregistro left join app_cliente on faesa_terapiasregistro.clie_id=app_cliente.clie_id left join pichinchahumana_extension.dns_convenios conve on faesa_terapiasregistro.conve_id=conve.conve_id where faesa_terapiasregistro.usua_id=".$rs_personal->fields["usua_id"]." and terap_fecha='".$fecha_valor."' and terap_hora='".$arreglo_horas[$hi]."' and terap_cancelado=0";
					 ...
					 if($rs_lbuscat)
					 {
						  while (!$rs_lbuscat->EOF) {
							...
							// $terap_copago="<br>COPAGO:".$rs_lbuscat->fields["terap_copago"];
							 ...
							 $name_seg='';
							 if($terap_medicompanies)
							 {
							 //  $name_seg='MEDICOMPANIES';
							 }
							...
							$apellido_dato=explode(" ",$rs_lbuscat->fields["clie_apellido"]);
							 ...
							 {
							 $busca_seguro="select * from pichinchahumana_extension.dns_convenios where conve_id='".$rs_lbuscat->fields["seg_id"]."'";
						     $rs_bseguro = $DB_gogess->executec($busca_seguro,array());	
							 $n_seguro="<br>".$rs_bseguro->fields["conve_nombre"];
							 }
							 ////////////////// dr
							  ...
							  echo '<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
									  <tr>
										...
										<td ><center><span style="color:#000000" ><b>'.$n_seguro."<BR>".$paciente_data.$terap_motivo.$n_quirofano.$terap_medicompanies.$terap_copago.'/</b></span></center></td>
										<td ></td>
									  </tr>
								 </table>';
