...
$centro_id=$_SESSION['dataceri2209_centro_id'];
  ...
  if($rs_hm)
   {
	   while (!$rs_hm->EOF) {
		...
		if($_SESSION['datadarwin2679_sessid_inicio']==$usua_id)
			 ...
			 {
				if($_SESSION['dataceri2209_usua_adm']==1)
				{
...
$busca_hmaniana="select * from cereni_pacientemateria inner join cereni_asignamaterias on cereni_pacientemateria.asigm_id=cereni_asignamaterias.asigm_id inner join cereni_horario on cereni_pacientemateria.horac_id=cereni_horario.horac_id where (horac_horai='".$hora."') and dia_id='8' and pama_activo=1 and cereni_pacientemateria.centro_id='".$centro_id."' and cereni_asignamaterias.usua_id='".$usua_id."'";
  $rs_hm= $DB_gogess->executec($busca_hmaniana,array());  
  if($rs_hm)
   {
	   while (!$rs_hm->EOF) {
		...
		$link_alerta='onclick=abrir_standar(\'aplicativos/documental/opciones/panel/calendariodiario/obsgrupo.php\',\'OBSERVACION\',\'divBody_obs\',\'divDialog_obs\',400,250,\''.$rs_hm->fields["asigm_id"].'\',\''.$rs_hm->fields["clie_id"].'\',\''.$rs_hm->fields["centro_id"].'\',\''.$usua_id.'\',\''.$fecha_valor.'\',0,0);';		
...
<table width="100%" border="1" cellpadding="5" cellspacing="0" >
  <tr>
	...
	<?php
	$lista_personal="select * from app_usuario where usua_estado=1 and especi_id not in (5,6,0) and centro_id='".$centro_id."' and usua_id!=74";
  ...
  <?php
  $concaten_h='';
  $slq_horext="select distinct horac_horai from cereni_pacientemateria inner join cereni_asignamaterias on cereni_pacientemateria.asigm_id=cereni_asignamaterias.asigm_id inner join cereni_horario on cereni_pacientemateria.horac_id=cereni_horario.horac_id where  pama_activo=1 and cereni_pacientemateria.centro_id='".$centro_id."'";
...
function activar_asisgrupo(asigm_id,clie_id,centro_id,usua_id,asig_fecha)
{
  ...
  $("#actu_asistencia").load("aplicativos/documental/opciones/panel/calendariodiario/aplicar_asistenciagrupo.php",{   
