...
//$obj_agendar=new cls_agendamiento();
...
//ejecuta agendamiento preconfigurado
$fecha_agendar=$fecha_valor;
...
<table  border="1" cellpadding="5" cellspacing="0" align="center" >
  <tr>
    <td style="padding-top:3px; padding-bottom:3px; padding-left:3px; padding-right:3px;font-size:9px" >Horario</td> 
	<?php
	$lista_personal="select * from app_usuario where usua_estado=1 and usua_agenda=1 and usua_id!=74 and usua_id='".$_SESSION['datadarwin2679_sessid_inicio']."' and centro_id='".$centro_id."'";
	...
	if($rs_personal)
    {
	  ...
	  // {
	 echo '<td style="padding-top:3px; padding-bottom:3px; padding-left:3px; padding-right:3px;font-size:8px" ><b>'.$rs_personal->fields["usua_nombre"].'<br>'.$nombre_dos[0].'</b><div style="color:#0099CC; font-weight:bold">'.$n_especialidad.'</div></td>';
	  // }
  ...
  ?>		   
    <tr>
     <td style='padding-top:2px; padding-bottom:2px; padding-left:2px; padding-right:2px; font-family:Verdana, Arial, Helvetica, sans-serif; font-size:8px; font-weight:bold'   >
	...
	if($rs_personal)
    {
	  ...
	  // {
	   ...
	   echo '<td style="padding-top:6px; padding-bottom:6px; padding-left:6px; padding-right:6px;font-size:8px" width="280px" >';
	   ...
	   $rs_lbuscat = $DB_gogess->executec($lista_buscat,array());
					 if($rs_lbuscat)
					 {
						  while (!$rs_lbuscat->EOF) {
							...
							$link_alerta='onclick=abrir_standar(\'aplicativos/documental/opciones/panel/calendariodiario/obs.php\',\'OBSERVACION\',\'divBody_obs\',\'divDialog_obs\',400,250,\''.$rs_lbuscat->fields["terap_id"].'\',0,0,0,0,0,0);';
							
							$link_datosperson='onclick=ver_formularioenpantalla(\'aplicativos/documental/datos_pacientes.php\',\'Editar\',\'divBody_ext\',\''.$rs_lbuscat->fields["clie_id"].'\',\'25\',0,0,0,0,1)';	 

							  
							echo '<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
							  <tr>
								<td '.$link_alerta.'  style="cursor:pointer" ><img src="images/alertaasistencia.png" width="30" height="30" /></td>
								<td style="background-color:#E1EFF4" ><center><span style="color:#000000" ><b>'.$paciente_data.$terap_motivo.$n_quirofano.'</b></span></center></td>
								<td '.$link_datosperson.'  style="cursor:pointer" ><img src="images/person2.png" width="30" height="30" /></td>
							  </tr>
							  <tr>
							      <td style="background-color:#E1EFF4" colspan="3" ><center>ATENDIDO: '.$check_asistencia.'</center></td>
							  </tr>
						     </table>';		
...
function activar_asis(terap_id)
{
  ...
  $("#actu_asistencia").load("aplicativos/documental/opciones/panel/calendariodiario/aplicar_asistencia.php",{   
