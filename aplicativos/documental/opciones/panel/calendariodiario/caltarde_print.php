...
<center>
    ...
    <table width="100%" border="1" cellpadding="5" cellspacing="0">
        <tr>
            <td style="padding-top:6px; padding-bottom:6px; padding-left:6px; padding-right:6px;font-size:10px">Horario
            </td>
            <?php
	//$lista_personal="select * from app_usuario where usua_estado=1 and especi_id not in (5,6,0) and usua_salirenreporte=1 and usua_id!=74";
	...
	if($rs_personal)
    {
	  ...
	  // if($rs_buscat->fields["totalr"]>0)
	   ...
	   //echo '<td style="padding-top:6px; padding-bottom:6px; padding-left:6px; padding-right:6px;font-size:9px" >'.utf8_encode($nombre_uno[0]).'<br>'.utf8_encode($nombre_dos[0]).'</td>';
		echo '<td style="padding-top:6px; padding-bottom:6px; padding-left:6px; padding-right:6px;font-size:9px" >'
    .mb_convert_encoding($nombre_uno[0], 'UTF-8', 'ISO-8859-1')
    .'<br>'
    .mb_convert_encoding($nombre_dos[0], 'UTF-8', 'ISO-8859-1')
    .'</td>';
  ...
  for($hi=0;$hi<count($arreglo_horas);$hi++)
	...
	if($rs_personal)
    {
	   ...
	   if ($rs_lbuscat && isset($rs_lbuscat->fields["terap_id"]) && $rs_lbuscat->fields["terap_id"] > 0) {
    	$bandera = 1;
		}
  ...
  ?>
        <tr>
            <td
                style='padding-top:5px; padding-bottom:5px; padding-left:5px; padding-right:5px; font-family:Verdana, Arial, Helvetica, sans-serif; font-size:9px; font-weight:bold'>
	...
	if($rs_personal)
    {
	 ...
	 //  {
	   ...
	   $lista_buscat="select terap_id,atenc_hc,especi_id,faesa_terapiasregistro.usua_id,faesa_terapiasregistro.clie_id,terap_fecha,terap_hora,terap_autorizacion,terap_estado,terap_fechapago,terap_nfactura,faesa_terapiasregistro.centro_id,faesa_terapiasregistro.usuar_id,terap_fecharegistro,terap_recuperacion,terap_observacion,terap_tipoevatera,tipopac_id,clie_nombre,clie_apellido,terap_motivo,terap_asiste,quiro_id,seg_id from  faesa_terapiasregistro left join app_cliente on faesa_terapiasregistro.clie_id=app_cliente.clie_id where faesa_terapiasregistro.usua_id=".$rs_personal->fields["usua_id"]." and terap_fecha='".$fecha_valor."' and terap_hora='".$arreglo_horas[$hi]."' and terap_cancelado=0";
						  ...
						  while (!$rs_lbuscat->EOF) {
							...
							$apellido_dato=explode(" ",$rs_lbuscat->fields["clie_apellido"]);
						     ...
						     echo '<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
									  <tr>
										<td ><center><span style="color:#000000" ><b>'.$n_seguro.'<br>'.$paciente_data.$terap_motivo.$n_quirofano.'/</b></span></center></td><td>'.$img_data.'</td>
									  </tr>
								 </table>';
