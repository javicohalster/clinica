...
function borrar_registro(tabla,campo,valor)
{
	 ...
	 $("#grid_borrar").load("aplicativos/documental/opciones/panel/agendart/borrar.php",{
...
function ver_id()
{
alert($('input:radio[name=radio_hc]:checked').val());
...

{
  ...
  $("#genera_registros").load("aplicativos/documental/opciones/panel/agendart/genera_registros.php",{
...
function ver_user()
{
  ...
  $("#lista_user").load("aplicativos/documental/opciones/panel/agendart/lista_user.php",{
      especi_idx:$('#especig_idx').val(),
	  centro_id:'<?php echo $_POST["centro_id"];  ?>'
  },function(result){  
...
function ver_calendario_general()
{
	...
	areag:$('#areag').val(),
	centro_id:'<?php echo $_POST["centro_id"]; ?>',
	areag:'<?php echo $_POST["especi_idx"]; ?>',
    usua_idvaltx:'<?php echo $_POST["usua_idvalx"]; ?>'
  },function(result){  
...
function borrar_terapia(tabla,campo,valor)
{
 ...
 if (confirm("Esta seguro que desea borrar terapia?"))
	 ...
	 $("#grid_borrart").load("aplicativos/documental/opciones/panel/agendart/borrart.php",{
...
function ver_terapistacal()
{
  ...
  $("#lista_terapistacal").load("aplicativos/documental/opciones/panel/agendart/lista_terapista.php",{
...
<table border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td><span class="Estilo3" >
	  <input name="areag" type="hidden" id="areag" value="<?php echo $_POST["especi_idx"]; ?>" />
	  <?php
	  
	     echo $objformulario->replace_cmb('dns_especialidad','especi_id,especi_nombre',' where especi_id=',$_POST["especi_idx"],$DB_gogess)
	  ?>
    </span> </td>
	
    <td>&nbsp;</td>
    <td><div id="lista_terapistacal">
<input name="usua_idvaltx" type="hidden" id="usua_idvaltx" value="<?php echo $_POST["usua_idvalx"]; ?>" />
<?php
	  
	     echo $objformulario->replace_cmb('app_usuario','usua_id,usua_nombre,usua_apellido',' where usua_id=',$_POST["usua_idvalx"],$DB_gogess)
	  ?>
</div></td>
    <td>&nbsp;</td>
    <td>
...
function abrir_orden()
{
   ...
   abrir_standar("aplicativos/documental/opciones/panel/agendart/ordenlista.php","Orden","divBody_ortden_div","divDialog_ortden_div",400,400,arr[1],0,0,0,0,0,0);
