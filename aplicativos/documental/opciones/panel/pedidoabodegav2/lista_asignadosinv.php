...
$partop_plasticos=$rs_bprecuentac->fields["partop_plasticos"];
...
echo "<b>PLASTICOS :</b> ".$plastisosino."<br>";
	  ?>	  
	  </td>
    </tr>
  </table>
...
<br /><br />
<?php
$lista_precuentaslbdega="select distinct centrob_id from dns_preddetalleparteoperatorio left join dns_centrosalud_origen on dns_preddetalleparteoperatorio.centrob_id=dns_centrosalud_origen.centro_id  where partop_id='".$partop_id."' and detapre_tipo in (1,2) ";
...
?>
 ...
 $lista_precuentas="select * from dns_preddetalleparteoperatorio left join dns_centrosalud_origen on dns_preddetalleparteoperatorio.centrob_id=dns_centrosalud_origen.centro_id  where partop_id='".$partop_id."' and detapre_tipo in (1,2) and centrob_id='".$rs_lprecuentasbdega->fields["centrob_id"]."'";
...
$rs_lprecuentas = $DB_gogess->executec($lista_precuentas,array());
 if($rs_lprecuentas)
 {
	  ...
	  //busca si ya entrego
		...
		$busca_entregado="select sum(detapre_cantidad) as cantidad_val from dns_detalleprecuenta where areap_id=0 and detapreoper_id='".$rs_lprecuentas->fields["detapre_id"]."'";
...
function procesa_data_redpu(bodega,partop_id,precu_id,clie_id)
{
$('#ejecuta_datax').html(''); 
...
if (confirm("Informativo: Esta seguro en realizar el proceso, si es plasticos verificar precuenta antes de procesar") == true) {
