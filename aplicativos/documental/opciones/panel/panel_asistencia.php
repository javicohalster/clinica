...
function ver_reporteasistenciaindividual() {

 if($('#usua_id').val()=='')
 {
alert("Por favor seleccione el personal");

  }
  else
  {
   
    myWindow2=window.open('reportes/reporte_asistencia.php?ivalor='+$('#usua_id').val()+'&fechai='+$('#fechai').val()+'&fechaf='+$('#fechaf').val()+'&centro_id='+$('#centro_idv').val(),'asistencia','width=750,height=500,scrollbars=YES');
 myWindow2.focus();
...
alert("Por favor seleccione el personal");
  ...
  else
  {
    ...
    myWindow2=window.open('reportes/reporte_asistenciaresumen.php?ivalor='+$('#usua_id').val()+'&fechai='+$('#fechai').val()+'&fechaf='+$('#fechaf').val()+'&centro_id='+$('#centro_idv').val(),'asistencia_resumen','width=750,height=500,scrollbars=YES');
 myWindow2.focus();
...
<?php
$comill_s="'";
$linkeditar= 'onClick="ver_formularioenpantalla('.$comill_s.'aplicativos/documental/datos_asistencia.php'.$comill_s.','.$comill_s.'TARIFARIO'.$comill_s.','.$comill_s.'divBody_ext'.$comill_s.',0,'.$comill_s.$_POST["pVar2"].$comill_s.',0,0,0,0,0)" style=cursor:pointer';	
...
$link_reporte='onClick="ver_reporteasistenciaindividual()" style=cursor:pointer';

echo '<button type="button" class="mb-sm btn btn-primary"  '.$link_reporte.'  style="background-color:#000066"  ><span class="glyphicon glyphicon-th-list"></span>REPORTE INDIVIDUAL</button>';
?>
</div>

<div class="col-xs-2">
<select class="form-control" name="centro_idv" id="centro_idv"  >
...
<select class="form-control" name="usua_id" id="usua_id"  >
           <option value="" >--Seleccion Personal--</option>
			<?php
			$objformulario->fill_cmb('app_usuario','usua_id,usua_nombre,usua_apellido','',' order by usua_apellido asc',$DB_gogess);
			?>
          </select>
</div>
...
$link_reporte='onClick="ver_reporte_resumen()" style=cursor:pointer';

echo '<button type="button" class="mb-sm btn btn-primary"  '.$link_reporte.'  style="background-color:#000066"  ><span class="glyphicon glyphicon-th-list"></span>REPORTE RESUMEN ANUAL</button>';
?>
</div>
</div>
