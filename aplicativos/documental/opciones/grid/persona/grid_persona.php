...
$rucci_valor=$_POST["rucci_valor"];
$ncomercial=$_POST["ncomercial"];
$nrepresentante=$_POST["nrepresentante"];
$tipoper_id=$_POST["tipoper_id"];

if($rucci_valor)
  {
   $sqlb1=" (provee_ruc like '%".$rucci_valor."%' or provee_cedula like '%".$rucci_valor."%') and ";
  }  
  
if($ncomercial)
  {
   $sqlb2=" (provee_nombre like '%".$ncomercial."%' or provee_nombrecomercial like '%".$ncomercial."%') and ";
  }  

if($nrepresentante)
  {
   $sqlb3=" provee_representante like '%".$nrepresentante."%' and ";
  }    


if($tipoper_id==1)
  {
       $sqlb4=" (select count(1) as facturas from dns_compras where proveevar_id=app_proveedor.provee_id)>0 and ";
  }   
  
if($tipoper_id==2)
  {
      $sqlb5=" (select count(1) as facturas from beko_documentocabecera where proveeve_id=app_proveedor.provee_id)>0 and ";
  }    


$buscador_sql='';
$buscador_sql=$sqlb1.$sqlb2.$sqlb3.$sqlb4.$sqlb5.$sqlb6.$sqlb7.$sqlb8.$sqlb9.$sqlb10;


echo @$sqltotal=$sql1.$sql2.$sql3.$buscador_sql;
...
$('#datatable1').DataTable({
    ...
    "columns": [{
            ...
            "mRender": function(data, type, full) {
                ...
                return '<table border="0" cellspacing="0" cellpadding="0" ><tr><td onclick="ver_formularioenpantalla(\'aplicativos/documental/datos_persona.php\',\'Editar\',\'divBody_ext\',' +
                    full["<?php echo $campo_id ?>"] +
                    ',\'<?php echo $_POST["pVar2"]; ?>\',0,0,0,0,0)" style=cursor:pointer ><span class="ms-2 fs-1 material-symbols-outlined">edit_note</span></td></tr></table>'; //<img src="images/editar.png"  />
