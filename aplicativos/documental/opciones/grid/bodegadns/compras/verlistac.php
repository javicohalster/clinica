...
$compra_enlace=$lista_data->fields["compra_enlace"];
				...
				if($rs_buscag)
                   {
					  ...
					  if ($rs_buscag->fields["fie_name"] == 'compra_fechaaprobacion') {
    						$valor_fecha = $lista_data->fields[$rs_buscag->fields["fie_name"]] ?? '';

    						if (!empty($valor_fecha) && strpos($valor_fecha, '-') !== false) {
        						$separa_fechaig = explode("-", $valor_fecha);

        						if (count($separa_fechaig) >= 3) {
           							$dia  = $separa_fechaig[2];
            						$mes  = $separa_fechaig[1];
            						$anio = $separa_fechaig[0];

            						$mes_nombre = $mes_n[$mes] ?? $mes;

            						$leeplantilla = str_replace("-d-", $dia, $leeplantilla);
            						$leeplantilla = str_replace("-m-", $mes_nombre, $leeplantilla);
...
$lista_miv="select *,concat(cuadrobm_principioactivo,' ',cuadrobm_nombredispositivo,' ',cuadrobm_primerniveldesagregcion,' ',cuadrobm_presentacion,' ',cuadrobm_concentracion) nombre_med from lpin_productocompra  left join dns_cuadrobasicomedicamentos on lpin_productocompra.cuadrobm_id=dns_cuadrobasicomedicamentos.cuadrobm_id where compra_enlace='".$compra_enlace."'";
...
if($rs_miv)
	...
	<td>'.$rs_miv->fields["prcomp_codigoext"].'</td>
	<td>'.utf8_encode($rs_miv->fields["prcomp_descripext"]).'</td>
    <td>'.$rs_miv->fields["prcomp_cantidad"].'</td>
	<td>'.number_format($rs_miv->fields["prcomp_preciounitario"], 4, '.', '').'</td>
	<td>'.number_format($rs_miv->fields["prcomp_subtotal"], 2, '.', '').'</td>	
  </tr>	';
  */
  $lista_pro .= '<tr>
	<td>' . $rs_miv->fields["cuadrobm_codigoatc"] . '</td>
	...
	<td>' . $rs_miv->fields["prcomp_codigoext"] . '</td>
	<td>' . mb_convert_encoding($rs_miv->fields["prcomp_descripext"] ?? '', 'UTF-8', 'ISO-8859-1') . '</td>
    <td>' . $rs_miv->fields["prcomp_cantidad"] . '</td>
	<td>' . number_format($rs_miv->fields["prcomp_preciounitario"], 4, '.', '') . '</td>
	<td>' . number_format($rs_miv->fields["prcomp_subtotal"], 2, '.', '') . '</td>	
  </tr>';
  ...
  $suma_grantotal=$suma_grantotal+$rs_miv->fields["prcomp_subtotal"];
