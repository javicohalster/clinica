...
<div class="container-fluid py-2">
    <div class="col-xl-9 col-sm-1 fs-5">
        <div class="alert alert-info text-white" role="alert">
            <strong>Ultimo Precio Inventario Inicial (Dispositivos): </strong>Es el precio mas comun en todos los
            centros
        </div>
        <div class="alert alert-info text-white" role="alert">
            <strong>Alerta Precio Diferente Inventario Inicial (Medicamentos): </strong>Si alg&uacute;n centro tiene un
            precio diferente ahi lo muestra con otro color (ver en [VER SEGUIMIENTO])
        </div>
        <div class="alert alert-info text-white" role="alert">
            <strong>Ultimo Precio Compra (Dispositivos): </strong>&Uacute;ltimo precio de compra en bodega principal, en
            este periodo
        </div>
        <div class="alert alert-info text-white" role="alert">
            <strong>Mayor Precio Compra(Dispositivos): </strong>Mayor precio de compra en este periodo
        </div>
        <div class="alert alert-info text-white" role="alert">
            <strong>Menor Precio Compra (Dispositivos): </strong>Menor precio de compra en este periodo
        </div>
    </div>
</div>
<div class="container-fluid py-2">
        ...
        <div class="col-12">
            <div class="card my-4">
			  ...
			  ?>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7">
                                        <strong>C&oacute;digo ISSPOL</strong>
                                    </th>
                                    <?php
			  }
			  ?>
                                    ...
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Stock
                                            Minimo Nacional</strong></th>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Stock
                                            Actual Bodega</strong></th>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Stock
                                            Actual Nacional Establecimientos de Salud</strong>
                                    </th>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Stock
                                            Global</strong></th>
			  ...
			  ?>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7">
                                        <strong>Ultimo
                                            Precio Inventario Inicial (Medicamentos)</strong>
                                    </th>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7">
                                        <strong>Alerta
                                            Precio Diferente Inventario Inicial
                                            (Medicamentos)</strong>
                                    </th>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7">
                                        <strong>Ultimo
                                            Precio Compra (Medicamentos)</strong>
                                    </th>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Mayor
                                            Precio Compra(Medicamentos)</strong></th>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Menor
                                            Precio Compra (Medicamentos)</strong></th>
                                    <?php
			  }
			  else
			  ...
			  ?>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7">
                                        <strong>Ultimo
                                            Precio Inventario Inicial (Dispositivos)</strong>
                                    </th>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7">
                                        <strong>Alerta
                                    ...
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7">
                                        <strong>Ultimo
                                            Precio Compra (Dispositivos)</strong>
                                    </th>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Mayor
                                            Precio Compra(Dispositivos)</strong></th>
                                    <th bgcolor="#AEDBE8"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Menor
                                            Precio Compra (Dispositivos)</strong></th>
                                    <?php
			  ...
			  ?>
                                    <!-- <th bgcolor="#AEDBE8" class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Precio Inv. Inicial General</strong></th> -->
                                    <th bgcolor="#A3D6E4"
                                        class="text-uppercase text-secondary font-weight-bolder opacity-7">
                                        <strong></strong>
                                    </th>
		  ...
		  if($subcateg_id)
		  {
		    $sqlnu=" subcateg_id='".$subcateg_id."' and ";
		          ...
		          if($_POST["insu"]>0)
				  {
					  if(@$_POST["txtbusca"]=='')
					  {
					    $lista_campor="select * from dns_cuadrobasicomedicamentos_vista where ".$sqlnu."  categ_id in (1,2,3,4,5,6,7,8) order by  cuadrobm_principioactivo asc";
					  }
					  else
					  {
					   ...
					   if($_POST["txtbusca"]!='')
					   {
					   $lista_campor="select * from dns_cuadrobasicomedicamentos_vista where ".$sqlnu."  categ_id in (1,2,3,4,5,6,7,8) and (cuadrobm_codigoatc like '%".$_POST["txtbusca"]."%' or cuadrobm_principioactivo like '%".$_POST["txtbusca"]."%' or cuadrobm_nombrecomercial like '%".$_POST["txtbusca"]."%')  order by  cuadrobm_principioactivo asc";
					   }
				  ...
				  //echo $lista_campor;
                   ...
                   if($rs_listacmp)
				   {
						...
						echo '<td>'.$rs_buc->fields['subcateg_nombre'].'</td>'; 
						...
						echo '<td width="310px" >'.$rs_listacmp->fields["cuadrobm_descripciontn"].'</td>';	
						...
						echo '<td width="310px" >'.$concatena_nom.'</td>';				
						...
						$enbodega=0;
						$enbodega=$rs_stactua->fields["stactual"];
								
						$stockactual1nacional="select sum(stock_cantidad*stock_signo) as stactual from dns_stockactual where stock_periodo>='".$per_activo."' and cuadrobm_id='".$rs_listacmp->fields["cuadrobm_id"]."'";
						$rs_stactuanacional = $DB_gogess->executec($stockactual1nacional);
						
						
						echo '<td>'.$rs_stactuanacional->fields["stactual"].' </td>';	
						
						$stockglobal=0;
						$stockglobal=($rs_stactuanacional->fields["stactual"]*1) + ($enbodega*1);
						
						$alerta_data='';
						if($stockglobal<=$rs_listacmp->fields["cuadrobm_stockminimo"])
						{
						  $alerta_data='<img src="../../../../../images/'.$grafico_gif.'" width="25" height="24" />';
						}
						
						$stockmaximo=$rs_listacmp->fields["cuadrobm_stockminimo"]*3;
						
						if($stockglobal>=$stockmaximo)
						{
						  $alerta_data='<img src="../../../../../images/verde.png" width="15" height="15" />';
						}
						
						echo '<td>'.$stockglobal.' '.$alerta_data.' </td>';	
