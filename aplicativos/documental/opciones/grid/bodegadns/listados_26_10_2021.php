		    ...
		    $sqlnu=" subcateg_id='".$subcateg_id."' and ";
				  ...
				  //echo $lista_campor;
                   ...
                   if($rs_listacmp)
				   {
						...
						$obtiene_precio="select * from dns_principalmovimientoinventario where  tipomov_id in (1,17) and cuadrobm_id='".$rs_listacmp->fields["cuadrobm_id"]."' order by moviin_id desc limit 1";
						$rs_obtprecio= $DB_gogess->executec($obtiene_precio);						
						$moviin_preciocompra=$rs_obtprecio->fields["moviin_preciocompra"];
						
						if($insu==1)
			            {
						$ncampo_val='cuadrobm_preciomedicamento';
						echo '<td>'.$moviin_preciocompra.'</td>';
						}
						else
						{
