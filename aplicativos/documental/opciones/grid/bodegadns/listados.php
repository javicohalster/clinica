...
<div class="container-fluid py-2">
    <div class="col-xl-9 col-sm-1 fs-5">
        <div class="alert alert-info text-white" role="alert">
            <span class="Estilo1">Precio &uacute;ltimo mayor</span>
        </div>
    </div>
</div>
<div class="row">
    <div class="col-12">
        <div class="card my-4">
            <div class="card-body px-0 pb-2">
			  ...
			  ?>
                                <th bgcolor="#AEDBE8"
                                    class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Precio
                                        C.</strong></th>
                                <th bgcolor="#AEDBE8"
                                    class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Precio C.
                                        Red P&uacute;blica</strong></th>
                                <th bgcolor="#AEDBE8"
                                    class="text-uppercase text-secondary font-weight-bolder opacity-7">
                                    <strong>PVP</strong>
                                </th>
                                <th bgcolor="#AEDBE8"
                                    class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>PVP Red
                                        Publica</strong></th>


                                <th bgcolor="#AEDBE8"
                                    class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>P
                                        Techo</strong></th>
			  ...
			  ?>
                                <th bgcolor="#AEDBE8"
                                    class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Detalle
                                        Movimientos</strong></th>
                                <th bgcolor="#A3D6E4"
		    ...
		    $sqlnu=" subcateg_id='".$subcateg_id."' and ";
		          ...
		          if($_POST["insu"]>0)
				  {
					  if(@$_POST["txtbusca"]=='')
					  {
					   if($sqlnu)
					   {
					   $lista_campor="select * from dns_cuadrobasicomedicamentos_vista where ".$sqlnu."  order by  cuadrobm_principioactivo asc limit 200";
					   }
					   else
					   {
					  ...
					  {					   
					   if($_POST["txtbusca"]!='')
					   {
					   $lista_campor="select * from dns_cuadrobasicomedicamentos_vista where ".$sqlnu."  (cuadrobm_codigoatc like '%".$_POST["txtbusca"]."%' or cuadrobm_principioactivo like '%".$_POST["txtbusca"]."%' or cuadrobm_nombrecomercial like '%".$_POST["txtbusca"]."%')  order by  cuadrobm_principioactivo asc limit 200";
					   }			   
				  ...
				  //echo $lista_campor;
                   ...
                   if($rs_listacmp)
				   {
						...
						echo '<td nowrap="nowrap" >'.utf8_encode($rs_listacmp->fields[$ncampo_val]).'</td>';
						...
						echo '<td>'.$preciosredp_valores["pcosto"].'</td>';
						echo '<td>'.$precios_valores["pvp"].'</td>';
						echo '<td>'.$preciosredp_valores["pvp"].'</td>';
						...
						echo '<td><input class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" type="button" name="Submit" value="Ver Seguimiento" '.$clickseguimiento.' /></td>';	
						
						
						$clickedit='';
						$clickedit="onclick=abrir_standar('producto/grid_producto_ver.php','Producto','divBody_producto','divDialog_producto',995,650,".$rs_listacmp->fields[$ide_producto].",0,0,0,0,0,'".$_POST["insu"]."')";
						 $nombre_boton='';
						 if(@$_SESSION['datadarwin2679_usua_subadm']==1)
						 {
							$nombre_boton='Ver'; 
						...
						echo '<td><input class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" type="button" name="Submit" value="'.$nombre_boton.'" '.$clickedit.' /></td>';	
