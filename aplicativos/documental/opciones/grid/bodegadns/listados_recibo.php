...
<div class="row">
                    ...
                    <table border="1" align="center" cellpadding="0" cellspacing="1" id="tabla"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th bgcolor="#AEDBE8"
                                    class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Valor
                                        Factura $</strong></th>
                                <th bgcolor="#AEDBE8"
                                    class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Alerta
                                        $</strong></th>
                                <th bgcolor="#AEDBE8"
                                ...
                                <th bgcolor="#AEDBE8"
                                    class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Fecha
                                        Ingreso</strong></th>
                                <th bgcolor="#A3D6E4"
                                    class="text-uppercase text-secondary font-weight-bolder opacity-7"><strong>Ver
                                        Listado</strong></th>
                                <th bgcolor="#A3D6E4"
   ...
   if($atc_val)
   {
		 $busca_compras="select dns_varioscompras.compra_id from dns_varioscompras inner join dns_temporalovimientoinventario on dns_varioscompras.compra_id=dns_temporalovimientoinventario.compra_id inner join dns_cuadrobasicomedicamentos on dns_temporalovimientoinventario.cuadrobm_id=dns_cuadrobasicomedicamentos.cuadrobm_id where cuadrobm_codigoatc='".$atc_val."'";
		 
		 $sql_cp=" and dns_varioscompras.compra_id in (".$busca_compras.") ";
	}
		          ...
		          if($sqlconcatenado)
				   { 
				   $lista_campor="select * from dns_varioscompras left join app_proveedor on dns_varioscompras.proveevar_id=app_proveedor.provee_id where ".$sqlconcatenado." and  dns_varioscompras.emp_id='".@$_SESSION['datadarwin2679_sessid_emp_id']."' and tipom_id=1 and compra_tipo=1 ".$sql_cp." order by  compra_id desc";	
				   
				   }
				   else
				   {
				    $lista_campor="select * from dns_varioscompras left join app_proveedor on dns_varioscompras.proveevar_id=app_proveedor.provee_id where dns_varioscompras.emp_id='".@$_SESSION['datadarwin2679_sessid_emp_id']."' and tipom_id=1 and compra_tipo=1 ".$sql_cp." order by  compra_id desc";
				  ...
				  //echo $lista_campor;
                   ...
                   if($rs_listacmp)
				   {
						...
						$busca_dataalerta="select sum((centrorecibe_cantidad*moviin_preciocompra)) as total from dns_varioscompras inner join dns_temporalovimientoinventario on dns_varioscompras.compra_id=dns_temporalovimientoinventario.compra_id where dns_varioscompras.compra_id='".$rs_listacmp->fields["compra_id"]."'";						
						$rs_dataalerta = $DB_gogess->executec($busca_dataalerta);
						
						//=================================================================
						
						$busca_datosexiste="select count(*) as numreg from dns_varioscompras inner join dns_temporalovimientoinventario on dns_varioscompras.compra_id=dns_temporalovimientoinventario.compra_id where dns_varioscompras.compra_id='".$rs_listacmp->fields["compra_id"]."'";						
						...
						$tabla_valordata="";
						$campo_valor="";	
						$tabla_valordata="'dns_varioscompras'";
						...
						$grafico_url='';
						$link_borrar="borrar_registro_bu('dns_varioscompras','".$ide_producto."','".$rs_listacmp->fields[$ide_producto]."')";	
                        $link_borrar="";
					...
					$busca_sihayreg="select count(*) as totalreg from dns_temporalovimientoinventario_vista where compra_id='".$rs_listacmp->fields[$ide_producto]."'";
						...
						if($rs_dataalerta->fields["totalalerta"]>0)
						{
							...
							{
							 echo '<td><span class="material-symbols-outlined">block</span></td>'; //<center><b><img src="sinverificar.png"  /></b></center> 
							}
							else
							{
						     echo '<td><span class="material-symbols-outlined">warning</span></td>'; //<center><b><img src="alerta.png"  /></b></center> 
							} 						
						}
						else
						{
						...
						{
						  echo '<td><span class="material-symbols-outlined">close</span></td>'; //<center><b><img src="sinreg.png"  /></b></center>						
						}				
						...
						if($_POST["insu"]==1)
						{
						$clickedit="onclick=abrir_standar('variosingresos/grid_ingresos_nuevo.php','Ingresos','divBody_producto','divDialog_producto',800,600,".$rs_listacmp->fields[$ide_producto].",0,0,0,0,0,'".$_POST["insu"]."')";
						 }
						 else
						 {
						 $clickedit="onclick=abrir_standar('variosingresos/grid_ingresos_nuevo.php','Ingresos','divBody_producto','divDialog_producto',800,600,".$rs_listacmp->fields[$ide_producto].",0,0,0,0,0,'".$_POST["insu"]."')";
						...
						$clickverl="onclick=tablas_verlista('variosingresos/verlista.php','".base64_encode($rs_listacmp->fields["compra_id"])."')"; 
						...
						echo '<td><input class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" type="button" name="Submit" value="Movimientos" onclick="ingresos_prkprod('.$comulla_simple.$rs_listacmp->fields[$ide_producto].$comulla_simple.')" /></td>';
						...
						if($bandera_pruba==1)
						{
						  ...
						  {
						   echo '<td><input class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" type="button" name="Submit" value="Procesar Aprobar"  '.$lins_procesra.' /></td>';	
						   }
...
function procesar_compra(compra_id) {
    if (confirm("Esta seguro que desea Procesar esta factura ?")) {
        ...
        $("#div_procesar").load("movimiento_variosingresos/procesa_ingreso.php", {
            compra_id: compra_id
        }, function(result) {
            listado_facturas();
        });
...
function reversar_compra(compra_id) {
    ...
    if (confirm("Esta seguro que desea Reversar esta factura ?")) {
        ...
        $("#div_procesar").load("movimiento_variosingresos/reversar_ingreso.php", {
            compra_id: compra_id
        }, function(result) {
            listado_facturas();
        });
