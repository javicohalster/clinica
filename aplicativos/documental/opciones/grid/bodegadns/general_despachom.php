                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="input-group input-group-static my-3">
                                                <select class="form-control" name="centro_idbu" id="centro_idbu"
                                                    style="font-size:11px; width:360px">
                                                    <?php
		  if($buus_lab=='1')
				{
				printf("<option value=''>---Sub-Bodega Destino--</option>");  
			  $objformulario->fill_cmb("dns_centrosalud","centro_id,centro_nombre",""," where centro_id!=55 and centro_id=30 order by centro_nombre asc",$DB_gogess);
				}
				else
				{
				  printf("<option value=''>---Sub-Bodega Destino--</option>");  
			  $objformulario->fill_cmb("dns_centrosalud","centro_id,centro_nombre",""," where centro_id!=55 order by centro_nombre asc",$DB_gogess);
           ...
           ?>
                                        ...
                                        <td><input class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" type="button"
                                                name="Button2" value="BUSCAR/ACTUALIZAR"
                                                onclick="listado_despachos()" /></td>
                                        <td><input class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" type="button"
                                                name="Button" value="NUEVO EGRESO A SUB-BODEGA"
                                                onclick="abrir_standar('egresocentros/grid_egresocentros_nuevo.php','Egreso_a_Centros','divBody_producto','divDialog_producto',800,600,0,0,0,0,0,0,'<?php echo $_GET["insu"]; ?>')" />
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
...
?>
    ...
    function compras_prkprod(egrec_id) {
        //alert(compra_id);
        abrir_standar('egresocentros/seleccionarp.php', 'DESPACHO', 'divBody<?php echo $subindice; ?>',
            'divDialog<?php echo $subindice; ?>', 990, 700, 0, egrec_id, 0, 0, 0,
            '<?php echo $bodega_principal; ?>', '<?php echo $insu_valorx; ?>');
    ...
    function borrar_registro_bunew(tabla, campo, valor) {

        if (confirm("Esta seguro que desea borrar este registro ?")) {

            $("#grid_borrar").load("../../../../../aplicativos/documental/opciones/grid/grid_borraregreso.php", {
                ptabla: tabla,
                pcampo: campo,
                pvalor: valor
            }, function(result) {
    ...
    function borrar_registro_temp(tabla, campo, valor) {
        ...
        if (confirm("Esta seguro que desea borrar este registro ?")) {
            ...
            $("#grid_borraritemvalor").load("../../../../../aplicativos/documental/opciones/grid/grid_borrar.php", {
                ptabla: tabla,
                pcampo: campo,
                pvalor: valor
            }, function(result) {
            ...
            $("#grid_borraritemvalor").html("Espere un momento...");

        }

    }

    function borrar_registro_tempvalor(tabla, campo, valor) {

        if (confirm("Esta seguro que desea borrar este registro ?")) {

            $("#div_borraritemvalor").load(
                "../../../../../aplicativos/documental/opciones/grid/grid_borraritemvalor.php", {
                    ptabla: tabla,
                    pcampo: campo,
                    pvalor: valor
                },
            ...
            $("#div_borraritemvalor").html("Espere un momento...");
    ...
    function listado_despachos() {
        ...
        $("#listado_v").load("listados_desp.php", {

            insu: '<?php echo $_GET["insu"]; ?>',
            txtbusca: $('#txtbusca').val(),
            centro_id: $('#centro_idbu').val(),
            atc_val: $('#atc_val').val()
