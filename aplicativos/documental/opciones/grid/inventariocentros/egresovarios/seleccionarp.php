...
$busca_ecentro="select * from dns_invegresosvarios where egrec_id='".$egrec_id."'";
...
<center><?php echo "<b>No. Egreso:".$compra_nsec." - ".$rs_ncentro->fields["centro_nombre"]."</b>"; ?></center>
...
abrir_standar("egresovarios/buscar_producto_select.php","Buscar","divBody_productox","divDialog_productox",750,450,0,0,0,0,0,0,0);
	 
}

function busca_contexto_select()
  {
	  $("#lista_bucx").load("egresovarios/lista_bu.php",{
...
function busca_producto()
{
   ...
   $("#lista_pd").load("egresovarios/lista_productos.php",{
...
function busca_disponibilidad()
{
  ...
  $('#btn_ejecuu').hide();
  $("#procesos_cmp").load("egresovarios/busca_disponibilidad.php",{
    cuadrobm_id:$('#cuadrobm_id').val(),
...
function enviar_atemporal()
{
   ...
   $("#lista_agregados").load("egresovarios/despacho_temporal.php",{
    cuadrobm_id:$('#cuadrobm_id').val(),
	cantidad_val:$('#cantidad_val').val(),
	egrec_id:'<?php echo $egrec_id; ?>'
  },function(result){
...
function lista_agregadosproducto()
{
   ...
   $("#l_agregados").load("egresovarios/lista_despachot.php",{
    egrec_id:'<?php echo $egrec_id; ?>'
  },function(result){ 
