...
<br />
<?php
 $ncentro= $objformulario->replace_cmb("dns_centrosalud","centro_id,centro_nombre"," where centro_id=",$_SESSION['datafrank1109_centro_id'],$DB_gogess);
 $centro_id=$_SESSION['datafrank1109_centro_id'];
?>
...
<div align="center">
  <table border="0" cellpadding="0" cellspacing="0">
    <tr>
	  ...
	  <td>&nbsp;</td>
	  <td><select name="centro_idb" id="centro_idb"  style="font-size:11px; width:360px" >
          <?php
	          printf("<option value=''>---Centro--</option>");  
      ...
      <td><!-- <input type="button" name="Button" value="Nuevo <?php echo $nombre_lista ?>" onclick="abrir_standar('producto/grid_producto_nuevo.php','Producto','divBody_producto','divDialog_producto',990,600,0,0,0,0,0,0,'<?php echo $insu_valorx; ?>')" /> --></td>
    </tr>
  </table>
  <br />
</div>
...
function compras_prk(produ_id,centro_id)
 {
	 ...
	 abrir_standar('movimiento/movimiento.php','Movimiento','divBody<?php echo $subindice; ?>','divDialog<?php echo $subindice; ?>',1200,700,0,0,0,0,0,centro_id,produ_id);
...
function listado_inventario()
{
  if($('#centro_idb').val()=='')
  {
   ...
   $("#listado_v").load("listadosini.php",{
...
myWindow4=window.open('general_totalini.php?insu='+id,'ventana_celda','width=100%,height=100%,scrollbars=YES');
myWindow4.focus();
