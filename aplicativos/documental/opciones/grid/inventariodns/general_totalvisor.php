...
?>
<center><B>LISTADO GENERAL</B></center><br />
...
<div align="center">
  <table border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td> 
	  <select name="centro_idb" id="centro_idb"   class="form-control" style="font-size:11px; width:360px" >
          <?php
	          printf("<option value=''>---Centro--</option>");  
			  $objformulario->fill_cmb("dns_centrosalud","centro_id,centro_nombre",""," order by centro_nombre asc",$DB_gogess);
           ?>
      </select>
	  </td>
      <td>
      ...
      <td>&nbsp;</td>
	  <td><input type="button" name="Button2" value="Excel" onclick="listado_inventarioexcel(0)" class="form-control" /></td>
    </tr>
  </table>
  <br />
</div>
...
function listado_inventarioexcel(id) {
   myWindow4=window.open('listadosvisorxls2.php?insu='+<?php echo $_GET["insu"]; ?>+'&txtbusca='+$('#txtbusca').val()+'&centro_idb='+$('#centro_idb').val()+'&produ_idpp='+$('#produ_idpp').val(),'ventana_exls','width='+screen.width+',height='+screen.height+',top=0, left=0,scrollbars=YES,fullscreen=yes');
   myWindow4.focus();
...
function listado_inventario()
{
  ...
  if($("#centro_idb").val()>0)
  {
   ...
   $("#listado_v").load("listadosvisor.php",{
  ...
  {
    alert("Seleccione el centro por favor");
...
myWindow4=window.open('general_totalvisor.php?insu='+id,'ventana_visor','width=100%,height=100%,scrollbars=YES');
myWindow4.focus();
