...
<?php
 ...
 ?>
<div align="center" class="style1_titulo"><?php echo $ncentro; ?><br /></div>
		    ...
		    $sqlnu=" subcateg_id='".$subcateg_id."' and ";
		          ...
		          if($_POST["insu"]>0)
				  {
					  if(@$_POST["txtbusca"]=='' and @$_POST["produ_idip"]=='' and @$_POST["produ_idpp"]=='')
					  {
					    if($sqlnu)
						{
						$lista_campor="select * from dns_cuadrobasicomedicamentos_vista where ".$sqlnu." order by  cuadrobm_principioactivo asc";
						}
						else
						{
					  ...
					  else
					  {
					   ...
					   if($sqlnu)
					   {
					   $lista_campor="select * from dns_cuadrobasicomedicamentos_vista where ".$sqlnu." and (cuadrobm_codigoatc like '%".$_POST["txtbusca"]."%' or cuadrobm_principioactivo like '%".$_POST["txtbusca"]."%' or cuadrobm_nombrecomercial like '%".$_POST["txtbusca"]."%')  order by  cuadrobm_principioactivo asc";
					   }
					   else
					   {
				 ...
				 //echo $lista_campor;
                   ...
                   if($rs_listacmp)
				   {
						...
						$stockactual="select sum(stock_cantidad*stock_signo) as stactual from dns_stockactual where stock_periodo='".$per_activo."'  and centro_id=".$_POST["centro_idb"]." and cuadrobm_id=".$rs_listacmp->fields["cuadrobm_id"];
						...
						{
						  if(@$_SESSION['datafrank1109_usua_subadm']==1)
						  {
						...
						$busca_ultimoprecio="select * from dns_movimientoinventario where moviin_fecharegistro>='2021-09-01' and cuadrobm_id='".$rs_listacmp->fields["cuadrobm_id"]."' and tipom_id=1  and centro_id='".$_POST["centro_idb"]."' order by moviin_id desc";
						$rs_bprecio = $DB_gogess->executec($busca_ultimoprecio);
						
						if($_POST["insu"]==1)
						  {				
						$ncampo_val='cuadrobm_preciomedicamento';
						echo '<td>'.$rs_bprecio->fields["moviin_preciocompra"].'</td>';
						}
						...
						$clickedit="onclick=abrir_standar('producto/grid_producto_nuevo.php','Producto','divBody_producto','divDialog_producto',995,650,".$rs_listacmp->fields[$ide_producto].",0,0,0,0,0,'".$_POST["insu"]."')";
						 ...
						 if(@$_SESSION['datafrank1109_usua_subadm']==1)
						 {
							$nombre_boton='Ver Caracteristica'; 
						 }
						 else
						 {
							 $nombre_boton='Ver Caracteristica';
						...
						echo '<td><input type="button" name="Submit" value="Inventario Inicial" onclick="compras_prk('.$comulla_simple.$rs_listacmp->fields[$ide_producto].$comulla_simple.','.$_POST["centro_idb"].')" /></td>';  
						echo '
