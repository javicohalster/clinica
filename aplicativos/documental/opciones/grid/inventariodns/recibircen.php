...
$sqltotal="";
$table='dns_invegresosvarios'; 
$subindice='_invegresosvarios';
$objformulario= new  ValidacionesFormulario();

$leeplantilla=leer_contenido_completo("../../../../../plantillas/invegreso_cab.php");	

$id_mob=$_POST["pVar1"];
$lista_data="select * from  dns_invegresosvarios where egrec_id='".$id_mob."'";
...
$rmpdestino='';				
if($lista_data->fields["tdes_id"]==1)
{

    $rmpdestino=$objformulario->replace_cmb("dns_centrosalud","centro_id,centro_nombre"," where centro_id=",$lista_data->fields["centrod_id"],$DB_gogess); 

}

if($lista_data->fields["tdes_id"]==2)
{
 
     $rmpdestino=$objformulario->replace_cmb("dns_destinosvarios","destv_id,destv_nombre"," where destv_id=",$lista_data->fields["destv_id"],$DB_gogess); 

}
			   
$leeplantilla=str_replace("-destinov-",$rmpdestino,$leeplantilla);				   
...
if($rs_miv)
    ...
    <td>'.utf8_encode($rs_miv->fields["cuadrobm_principioactivo"]).'</td>
    <td>'.$rs_miv->fields["cuadrobm_nombrecomercial"].'</td>
	...
	<td>'.$rs_miv->fields["cantidad_val"].'</td>
	<td>'.$rs_miv->fields["moviin_preciocompra"].'</td>	
	<td>'.$total_cal.'</td>	
  </tr>	';
...
$lista_valor='<table width="100%" border="1" cellpadding="0" cellspacing="0">
  <tr>
	          ...
	          <td ><strong>Fecha Expiraci&oacute;n</strong></td>
			  <td ><strong>Cantidad(Unidades)</strong></td>
			  <td ><strong>Valor Unitario (USD)</strong></td>
			  <td ><strong>Valor Total (USD)</strong></td>
	</tr>
  '.$lista_pro.'
  <tr>
    <td></td>
...
?>
<div id="proceso_ingval">
<?php
if($lista_data->fields["egrec_recibido"]==0)
{
?>
<center>
<button type="button" class="btn btn-success" style="width:150px" onclick="procesar_IngresoPrincipal()">Procesar Ingreso</button>
</center>
<?php
}
?>
...
function procesar_IngresoPrincipal()
{
  $("#proceso_ingval").load("aplicativos/documental/opciones/grid/inventariodns/procesar_recepcioncenprinn.php",{
   egrec_id:'<?php echo $id_mob; ?>'
 
  },function(result){ 
      listar_ingresos();
  });  

  $("#proceso_ingval").html("Espere un momento...");
