...
if($csearch)
{
	$busca_fecha="select *,DATE_ADD(psicolo_fecharegistro,INTERVAL 3 DAY) as fechacierre from ".$table." where ".$campo_primariodata."='".$csearch."'";
...
<button type="button" class="mb-sm btn btn-success" onclick="ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_substandarformpsicologia.php','Perfil','divBody_ext',0,'<?php echo @$clie_id; ?>','<?php echo $mnupan_id; ?>','<?php echo $atenc_id; ?>',0,0,0)" style="cursor:pointer"><span class="glyphicon glyphicon-arrow-left"></span> Regresar </button>
...
$linkgayuda="onclick=abrir_standar('ayuda/ayuda.php','AYUDA','divBody_ext','divDialog_ext',990,600,5,0,0,0,0,0,0) style='cursor:pointer'";
...
<div class="form-group">
	 ...
	 <div class="col-md-6">
	      ...
	      <button type="button" class="mb-sm btn btn-primary" onclick="abre_listasecundario()" style="cursor:pointer"><span class="glyphicon glyphicon-list"></span> <?php echo utf8_encode($rs_tablasec->fields["tab_title"]) ?></button>
...
function  abre_listasecundario()
{
  if($('#<?php echo $campo_primario; ?>').val()>0)
   {
     ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_listasubsecuentepsicologia.php','SECUNDARIO','divBody_ext',0,'<?php echo @$clie_id; ?>','99','<?php echo $atenc_id; ?>',0,0,$('#<?php echo $campo_primario; ?>').val());
