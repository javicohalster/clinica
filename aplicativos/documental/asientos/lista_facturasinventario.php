...
$busca_listatx="select * from beko_documentocabecera where month(doccab_fechaemision_cliente)>1";
...
if($rs_listatx)
	...
	$busca_listadett="select count(*) total from beko_documentodetalle where doccab_id='".$rs_listatx->fields["doccab_id"]."' and  docdet_codprincipal in (SELECT prod_codigo FROM efacsistema_producto WHERE prod_nombre like '%paque%')";
	$rs_listatdett = $DB_gogess->executec($busca_listadett);
		
	if($rs_listatdett->fields["total"]>0)
	{	
			?>
  <tr>
	...
	<input name="valor_id<?php echo $contador; ?>" type="hidden" id="valor_id<?php echo $contador; ?>" value="<?php echo $rs_listatx->fields["doccab_id"]; ?>" />
	<input name="tipocmp_codigo<?php echo $contador; ?>" type="hidden" id="tipocmp_codigo<?php echo $contador; ?>" value="<?php echo $rs_listatx->fields["tipocmp_codigo"]; ?>" />
	<?php
	$busca_listadet="select * from beko_documentodetalle where doccab_id='".$rs_listatx->fields["doccab_id"]."' and  docdet_codprincipal in (SELECT prod_codigo FROM efacsistema_producto WHERE prod_nombre like '%paque%')";
	$rs_listatdet = $DB_gogess->executec($busca_listadet);
	if($rs_listatdet)
	{
	   while (!$rs_listatdet->EOF) 
			{
			  echo $rs_listatdet->fields["docdet_codprincipal"]." -> ".$rs_listatdet->fields["docdet_descripcion"]." -> ".docdet_cantidad." -> ".$rs_listatdet->fields["docdet_total"]."<br>";
			
			 $rs_listatdet->MoveNext();
			}
	}		
	?>
