...
<table border="1" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td bgcolor="#F0F0F0"><strong>VER ASIENTOS</strong></td>
	<td bgcolor="#F0F0F0"><strong>GENERA ASIENTO RETENCION </strong></td>
  </tr>
<?php
//and compra_id=122
$busca_listatx="select * from dns_compras where compra_id!=1 order by compra_fecha asc";
...
if($rs_listatx)
	{
	   while (!$rs_listatx->EOF) 
	        ...
	        $contador++;
			$estado='';
			if($rs_listatx->fields["compra_anulado"]=='1')
			{			
			 $estado='ANULADO';			
			}	
			...
			$tipdoc_nombre='';
			$busca_tipo="select * from dns_tipodocumentogeneral where tipdoc_id='".$rs_listatx->fields["tipdoc_id"]."'";
            $rs_bdtipo = $DB_gogess->executec($busca_tipo);
            
			$tipdoc_nombre=$rs_bdtipo->fields["tipdoc_nombre"];
			
			$compra_total='';
		    $compra_total=$rs_listatx->fields["compra_total"];	

			?>
  <tr>
    <td><?php echo $contador; ?></td>
	<td nowrap="nowrap"><?php echo $rs_listatx->fields["compra_id"]."->".$rs_listatx->fields["compra_nfactura"]; ?></td>
	
	<td><?php echo $tipdoc_nombre; ?></td>
	
    <td><?php echo $rs_listatx->fields["compra_autorizacion"]; ?></td>
    <td nowrap="nowrap"><?php echo $rs_listatx->fields["compra_fecha"]; ?></td>
...
function genera_asientocompraf1(num)
{
   ...
   var urlasiento;
	...
	urlasiento='../../../pdfasientos/pdfasientocompra_ver.php';	
...
function genera_asientocompraf2(num)
{
