...
if ($_SESSION['datadarwin2679_sessid_inicio']) {
	...
	$number_mes = 0;
	$number_mes = cal_days_in_month(CAL_GREGORIAN, $comcont_mes, $comcont_anio); // 31
	//echo "There were {$number} days in August 2003";


	$fecha_claculomes = $comcont_anio . "-" . $comcont_mes . "-" . $number_mes;

	$anio_actual = $comcont_anio;
	...
	if ($num_detasiento == 0) {
		...
		$busca_cabeceraasiento = "delete from lpin_comprobantecontable where  comcont_anio='" . $comcont_anio . "' and  comcont_mes='" . $comcont_mes . "' and comcont_tabla='" . $tabla_asiento . "' and comcont_idtabla='" . $valor_id . "' and tipoa_id='11'";
	...
	if ($num_detasiento > 0) {
		...
		echo $busca_cabeceraasiento = "select * from lpin_comprobantecontable where comcont_anio='" . $comcont_anio . "' and comcont_mes='" . $comcont_mes . "' and comcont_tabla='" . $tabla_asiento . "' and comcont_idtabla='" . $valor_id . "' and tipoa_id='11'";
		$rs_bcabecera = $DB_gogess->executec($busca_cabeceraasiento);
		...
		if ($rs_bcabecera->fields["comcont_id"] > 0) {
			...
			$concepto = '';
			$concepto = 'DEPRECIACION ACTIVO FIJO. MES: ' . $nombre_mes[$comcont_mes] . ' AN:' . $comcont_anio . ' CODIGO:' . $acfi_codigo . ' ' . $acfi_nombre . ' ';
			//$concepto=$concepto;
			//concepto factura
			//++++++++++++++++++++++++++
			...
			for ($i = 0; $i < count($array_haber); $i++) {
				...
				//BUSCA CUENTA
				$detcc_fecharegistro = date("Y-m-d H:i:s");

				$lista_data = "INSERT INTO lpin_detallecomprobantecontable (detcc_id, detcc_cuentacontable, detcc_descricpion, detcc_referencia, detcc_entidad, detcc_debe, detcc_haber, usua_id, detcc_fecharegistro,comcont_enlace,detcc_tipo) VALUES (NULL, '" . $detcc_cuentacontable . "', '" . $detcc_descricpion . "', '" . $detcc_referencia . "', '','" . round($detcc_debe, 2) . "','" . round($detcc_haber, 2) . "','" . $_SESSION['datadarwin2679_sessid_inicio'] . "', '" . $detcc_fecharegistro . "','" . $comcont_enlace . "','" . $detcc_tipo . "') ";
			...
			echo $inserta_cab = "INSERT INTO lpin_comprobantecontable ( tipoa_id, comcont_fecha, comcont_concepto, comcont_numeroc, comcont_estado, comcont_diferencia, comcont_enlace, usua_id, comcont_fecharegistro, centro_id, comcont_tabla, comcont_idtabla,comcont_obs,comcont_anulado,comcont_mes,comcont_anio) VALUES
( '11', '" . $fecha_claculomes . "', '" . $concepto . "', '" . $acfi_codigo . "', 'APROBADO', 0, '" . $comcont_enlace . "', '" . $_SESSION['datadarwin2679_sessid_inicio'] . "', '" . $fecha_hoy . "','" . $_SESSION['datadarwin2679_centro_id'] . "', '" . $tabla_asiento . "', '" . $valor_id . "','AUTOMATICO','" . $doccab_anulado . "','" . $comcont_mes . "','" . $comcont_anio . "');";
