...
<table border="1" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td bgcolor="#F0F0F0"><strong>DOCUMENTO</strong></td>
	<td bgcolor="#F0F0F0"><strong>ID DETALLE</strong></td>
    ...
    <td bgcolor="#F0F0F0"><strong>RETORNA INVENTARIO</strong></td>
  </tr>
<?php

$busca_listatx="select * from beko_mhdetallefactura where moviin_id!=0 order by mhdetfac_fecharegistro desc";
...
if($rs_listatx)
	{
	   while (!$rs_listatx->EOF) 
			...
			$busca_sitienecabecera="select * from beko_documentocabecera where doccab_id='".$doccab_id."'";
			$rs_sicabecera = $DB_gogess->executec($busca_sitienecabecera);
			
			
if($rs_sicabecera->fields["doccab_anulado"]=='1')
{

$moviin_idx=$rs_listatx->fields["moviin_id"];
$busca_datosx="select * from dns_movimientoinventario where moviin_id='".$moviin_idx."'";
$rs_bdatisx = $DB_gogess->executec($busca_datosx);

	if($rs_bdatisx->fields["moviin_id"]>0)
	{
	  ...
	  <tr>
		...
		<td><?php echo $rs_sicabecera->fields["doccab_ndocumento"]; ?></td>
		<td><?php echo $rs_listatx->fields["mhdetfac_id"]; ?></td>
		<td><?php echo $rs_listatx->fields["mhdetfac_codprincipal"]; ?></td>
		<td><?php echo $rs_listatx->fields["mhdetfac_cantidad"]; ?></td>
		<td><?php echo $rs_listatx->fields["mhdetfac_descripcion"]; ?></td>
		<td><?php echo $rs_listatx->fields["mhdetfac_fecharegistro"]; ?></td>	
		<td><input name="valor_id<?php echo $contador; ?>" type="hidden" id="valor_id<?php echo $contador; ?>" value="<?php echo $rs_listatx->fields["mhdetfac_id"]; ?>" />
		<input type="button" name="Submit" value="RETORNAR" onclick="retornar_inventario('<?php echo $contador; ?>')" />
		 </td>
...
function retornar_inventario(num)
{

  $("#asiento_cr"+num).load("farmacia/retorna_invanulado.php",{
