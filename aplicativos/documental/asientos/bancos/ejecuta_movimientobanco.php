...
//principal
$cuenta_lista=0;
$busca_principal="select * from app_movimientobancos where movban_id='".$movban_id."'";
$rs_bprincipal = $DB_gogess->executec($busca_principal);
$tmv_id=$rs_bprincipal->fields["tmv_id"];

$busca_printotal="select sum(movbanc_monto) as total from app_movimientobancos inner join lpin_movbancos on app_movimientobancos.movban_enlace=lpin_movbancos.movban_enlace where movban_id='".$movban_id."'";
$rs_bprintotal = $DB_gogess->executec($busca_printotal);
$total=$rs_bprintotal->fields["total"];


$movban_enlace=$rs_bprincipal->fields["movban_enlace"];
...
$array_haber[$cuenta_lista]["CUENTA"]=$rs_bprincipal->fields["movban_ctabancaria"];
...
$obtiene_enlace="select * from lpin_movbancos where movban_enlace='".$movban_enlace."'";
$rs_enlace = $DB_gogess->executec($obtiene_enlace);
	...
	if($rs_enlace)
	{
	   while (!$rs_enlace->EOF) 
			...
			$array_haber[$cuenta_lista]["CUENTA"]=$rs_enlace->fields["planc_id"];
			$array_haber[$cuenta_lista]["VALOR"]=$rs_enlace->fields["movbanc_monto"];
			$cuenta_lista++;
...
$busca_cabeceraasiento="select * from lpin_comprobantecontable where comcont_tabla='".$tabla_asiento."' and comcont_idtabla='".$valor_id."' and tipoa_id='".$tipo_code."'";
...
$busca_principalx="select * from app_movimientobancos where movban_id='".$movban_id."'";
...
$crb_fecha=$rs_bprincipalx->fields["movban_fechaemision"];
$doccab_anulado=0;
...
$concepto='';
$concepto=$n_tipo.' MOVIMIENTO BANCO. '.$doccab_ndocumento.' '.$doccab_nombrerazon_cliente;
//$concepto=$concepto;
//concepto factura
//++++++++++++++++++++++++++
