...
<table border="1" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td bgcolor="#F0F0F0"><strong>CDE FAC</strong></td>
	<td bgcolor="#F0F0F0"><strong>RETORNA INVENTARIO</strong></td>
  </tr>
<?php

$busca_listatx="select * from dns_movimientoinventario where doccab_id!='' order by moviin_fecharegistro desc";
...
if($rs_listatx)
 ...
 $cuadrobm_id=$rs_listatx->fields["cuadrobm_id"];	
 
 $busca_medy="select * from dns_cuadrobasicomedicamentos where cuadrobm_id='".$cuadrobm_id."'";
 $rs_medy = $DB_gogess->executec($busca_medy);	
 
 $busca_sihayfactura="select * from beko_documentocabecera where doccab_id='".$rs_listatx->fields["doccab_id"]."'";
 $rs_sihay = $DB_gogess->executec($busca_sihayfactura);

 if($rs_sihay->fields["doccab_id"]=='')
 {			
	$contador++;		
		?>
  <tr>
    <td><?php echo $contador; ?></td>
	<td><?php echo $rs_listatx->fields["moviin_id"]; ?></td>
    <td><?php echo $rs_medy->fields["cuadrobm_codigoatc"]; ?></td>
    <td><?php echo $rs_listatx->fields["moviin_totalenunidadconsumo"]; ?></td>
    <td><?php echo $rs_medy->fields["cuadrobm_principioactivo"]; ?></td>
	<td><?php echo $rs_listatx->fields["moviin_fecharegistro"]; ?></td>	
	<td><?php echo $rs_listatx->fields["doccab_id"]; ?></td>		
    <td><input name="valor_id<?php echo $contador; ?>" type="hidden" id="valor_id<?php echo $contador; ?>" value="<?php echo $rs_listatx->fields["moviin_id"]; ?>" />
...
function retornar_inventario(num)
{
  ...
  $("#asiento_cr"+num).load("farmacia/retorna_inv.php",{
