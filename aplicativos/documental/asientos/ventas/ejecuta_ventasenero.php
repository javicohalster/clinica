...
$rs_prd = $DB_gogess->executec($lista_productos);
	...
	if($rs_prd)
	{
	   while (!$rs_prd->EOF) 
			...
			$busca_cuenta="select distinct ltra_cuenta as categ_cuentacontableventas from lpin_tarifarioenero where ltra_codigo='".$rs_prd->fields["mhdetfac_codprincipal"]."'";
...
$rs_prd = $DB_gogess->executec($lista_productos);
	...
	if($rs_prd)
	{
	   while (!$rs_prd->EOF) 
			...
			$busca_cuenta="select distinct ltra_cuenta,ltra_cuentacosto,ltra_cuentainventario from lpin_tarifarioenero where ltra_codigo='".$rs_prd->fields["mhdetfac_codprincipal"]."'";			 
			$rs_cuentabu = $DB_gogess->executec($busca_cuenta);
			
			if($rs_cuentabu->fields["ltra_cuenta"]=='4.1.2.1' or $rs_cuentabu->fields["ltra_cuenta"]=='4.1.2.2')
			{
						
					// saca precio de costo		
					if($precio_costo>0 and $rs_cuentabu->fields["ltra_cuentacosto"]!='')
					{
					$array_haber[$cuenta_lista]["TIPO"]="DEBE";
					$array_haber[$cuenta_lista]["CUENTA"]=$rs_cuentabu->fields["ltra_cuentacosto"];
					$array_haber[$cuenta_lista]["VALOR"]=$precio_costo;
					$cuenta_lista++;
					$array_haber[$cuenta_lista]["TIPO"]="HABER";
					$array_haber[$cuenta_lista]["CUENTA"]=$rs_cuentabu->fields["ltra_cuentainventario"];
					$array_haber[$cuenta_lista]["VALOR"]=$precio_costo;			
					$cuenta_lista++;
