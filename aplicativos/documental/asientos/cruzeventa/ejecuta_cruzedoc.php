...
if ($_SESSION['datadarwin2679_sessid_inicio']) {
	...
	$asiento_elimina = "delete from lpin_comprobantecontable where comcont_tabla='" . $tabla_asiento . "' and comcont_idtabla='" . $valor_id . "' and tipoa_id='7' and crudet_id not in (" . $busca_listabrrado . ")";
	...
	if ($rs_bcobros) {
		while (!$rs_bcobros->EOF) {
			...
			if ($num_detasiento == 0) {
				...
				$busca_cabeceraasiento = "delete from lpin_comprobantecontable where comcont_tabla='" . $tabla_asiento . "' and comcont_idtabla='" . $valor_id . "' and tipoa_id='7' and comcont_tablas='beko_documentocabecera' and comcont_idtablas='" . $rs_bcobros->fields["crudoc_ndocumentoventa"] . "' and crudet_id='" . $rs_bcobros->fields["crudet_id"] . "'";
			...
			if ($num_detasiento > 0) {
				...
				$busca_cabeceraasiento = "select * from lpin_comprobantecontable where comcont_tabla='" . $tabla_asiento . "' and comcont_idtabla='" . $valor_id . "' and tipoa_id='7' and comcont_tablas='beko_documentocabecera' and comcont_idtablas='" . $rs_bcobros->fields["crudoc_ndocumentoventa"] . "' and crudet_id='" . $rs_bcobros->fields["crudet_id"] . "'";
				$rs_bcabecera = $DB_gogess->executec($busca_cabeceraasiento);
				...
				if ($rs_bcabecera->fields["comcont_id"] > 0) {
					...
					//COMPRA
					$busca_compra = "select * from dns_compras where compra_id='" . $rs_bcobros->fields["crudet_documento"] . "'";
					$rs_bucompra = $DB_gogess->executec($busca_compra);
					$compra_nfactura = $rs_bucompra->fields["compra_nfactura"];

					//COMPRA

					$concepto = '';
					$concepto = 'ING. COBRO. CRUCE DOCUMENTO ' . $doccab_ndocumento . ' ' . $doccab_nombrerazon_cliente . ' ' . $doccab_apellidorazon_cliente . ' CRUCE DOC:' . $compra_nfactura . ' ' . $rs_bcobros->fields["crudoc_descripcion"];
					//$concepto=$concepto;
					//concepto factura
					//++++++++++++++++++++++++++
					...
					$crudoc_fechaemision = $rs_bcobros->fields["crudoc_fechaemision"];
					$concepto = '';
					$concepto = 'ING. COBRO. CRUCE DOCUMENTO ' . $doccab_ndocumento . ' ' . $doccab_nombrerazon_cliente . ' ' . $doccab_apellidorazon_cliente . ' ' . $rs_bcobros->fields["crudoc_descripcion"];
...
( '7', '" . $crudoc_fechaemision . "', '" . $concepto . "', '" . $doccab_ndocumento . "', 'APROBADO', 0, '" . $comcont_enlace . "', '" . $_SESSION['datadarwin2679_sessid_inicio'] . "', '" . $fecha_hoy . "','" . $_SESSION['datadarwin2679_centro_id'] . "', '" . $tabla_asiento . "', '" . $valor_id . "','AUTOMATICO','" . $doccab_anulado . "','beko_documentocabecera','" . $rs_bcobros->fields["crudoc_ndocumentoventa"] . "','" . $crudet_id . "');";
