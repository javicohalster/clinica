...
$compra_nummodif=$rs_bdatis->fields["compra_nummodif"];
...
$lista_fijos="select * from dns_activosfijos inner join lpin_tipoafijo on dns_activosfijos.tiacf_id=lpin_tipoafijo.tiacf_id where compra_enlace='".$compra_enlace."'";
$rs_prdfijos = $DB_gogess->executec($lista_fijos);
	...
	if($rs_prdfijos)
	{
	   while (!$rs_prdfijos->EOF) 
			...
			$array_haber[$cuenta_lista]["TIPO"]="HABER";
			$array_haber[$cuenta_lista]["CUENTA"]=$rs_prdfijos->fields["tiacf_cuenta"];
			$array_haber[$cuenta_lista]["VALOR"]=$rs_prdfijos->fields["acfi_subtotal"];
...
$busca_cabeceraasiento="delete from lpin_comprobantecontable where comcont_tabla='".$tabla_asiento."' and comcont_idtabla='".$valor_id."' and tipoa_id='1'";
...
$busca_cabeceraasiento="select * from lpin_comprobantecontable where comcont_tabla='".$tabla_asiento."' and comcont_idtabla='".$valor_id."' and tipoa_id='1'";
$rs_bcabecera = $DB_gogess->executec($busca_cabeceraasiento);
...
//$compra_anulado=$rs_dattos->fields["compra_anulado"];

$tipdoc_nombre=$rs_dattos->fields["tipdoc_nombre"];

$concepto='';
$concepto='COMPRAS '.$tipdoc_nombre.' '.$doccab_ndocumento.' '.$doccab_nombrerazon_cliente.' Documento modif.: '.$compra_nummodif;;

//concepto factura
//++++++++++++++++++++++++++


$actualiza_data="update lpin_comprobantecontable set proveeasiento_id='".$proveevar_id."',comcont_anulado='".$compra_anulado."',comcont_fecha='".$doccab_fechaemision_cliente."',comcont_concepto='".$concepto."',comcont_numeroc='".$doccab_ndocumento."' where comcont_id='".$rs_bcabecera->fields["comcont_id"]."'";
...
$inserta_cab="INSERT INTO lpin_comprobantecontable ( tipoa_id, comcont_fecha, comcont_concepto, comcont_numeroc, comcont_estado, comcont_diferencia, comcont_enlace, usua_id, comcont_fecharegistro, centro_id, comcont_tabla, comcont_idtabla,comcont_obs,comcont_anulado,proveeasiento_id) VALUES
( 1, '".$doccab_fechaemision_cliente."', '".$concepto."', '".$doccab_ndocumento."', 'APROBADO', 0, '".$comcont_enlace."', '".$_SESSION['datadarwin2679_sessid_inicio']."', '".$fecha_hoy."','".$_SESSION['datadarwin2679_centro_id']."', '".$tabla_asiento."', '".$valor_id."','AUTOMATICO','".$compra_anulado."','".$proveevar_id."');";
