...
$busca_cliente="select * from dns_consultaexterna where conext_id='".$id_valor."'";
...
$actualiza_paciente="update dns_consultaexterna set conext_fecharegistro='".$nueva_fecha."' where conext_id='".$id_valor."'";
$rs_acpaciente = $DB_gogess->executec($actualiza_paciente,array());


if(@$rs_acpaciente)
{
   //CONSULTA EXTERNA - EVOLUCION / SOLO LLENAR EN CASO DE PACIENTES QUE RETORNAN CON RESULTADOS DE EXAMENES EL MISMO DIA
    $busca_fechareg=explode(" ",$fecha_anterior);
    $busca_organos="update dns_detalleconsultaexterna set detext_fecharegistro='".$nueva_fecha."' where conext_enlace='".$conext_enlace."' and detext_fecharegistro like '".$busca_fechareg[0]."%'";
    $rs_organos = $DB_gogess->executec($busca_organos,array());
   //CONSULTA EXTERNA - EVOLUCION / SOLO LLENAR EN CASO DE PACIENTES QUE RETORNAN CON RESULTADOS DE EXAMENES EL MISMO DIA
   
   //dns_diagnosticoexterna
   $busca_organos="update dns_diagnosticoexterna set diagn_fecharegistro='".$nueva_fecha."' where conext_enlace='".$conext_enlace."' and diagn_fecharegistro like '".$busca_fechareg[0]."%'";
   $rs_organos = $DB_gogess->executec($busca_organos,array());    
   //dns_diagnosticoexterna
   
   //dns_cuadrobasicocexterno   
   $busca_organos="update dns_cuadrobasicocexterno set cuabas_fecharegistro='".$nueva_fecha."' where conext_enlace='".$conext_enlace."' and cuabas_fecharegistro like '".$busca_fechareg[0]."%'";
   $rs_organos = $DB_gogess->executec($busca_organos,array());   
   //dns_cuadrobasicocexterno
   ...
   $lista_rescetdia="select * from dns_recetaconsultaexterna where plantra_despachado='' and conext_enlace='".$conext_enlace."' and plantra_fecharegistro like '".$busca_fechareg[0]."%'";
   $rs_listarecetas = $DB_gogess->executec($lista_rescetdia,array());
   if($rs_listarecetas)
		{
			while (!$rs_listarecetas->EOF) {
    
   $busca_organos1="update dns_recetaconsultaexterna set plantra_fecharegistro='".$nueva_fecha."' where plantra_id='".$rs_listarecetas->fields["plantra_id"]."'";
   $rs_organos1 = $DB_gogess->executec($busca_organos1,array()); 
   
    $permid_idtabla=$rs_listarecetas->fields["plantra_id"];
	$permid_tabla="dns_recetaconsultaexterna";
	$permid_acepta=1;
  ...
  //borra permisos anteriores
   ...
   //dns_dispositivomedicoexterno
   
   $lista_rescetdia="select * from dns_dispositivomedicoexterno where plantrai_despachado='' and conext_enlace='".$conext_enlace."' and plantrai_fecharegistro like '".$busca_fechareg[0]."%'";
   $rs_listarecetas = $DB_gogess->executec($lista_rescetdia,array());
   if($rs_listarecetas)
		{
			while (!$rs_listarecetas->EOF) {
    
   $busca_organos1="update dns_dispositivomedicoexterno set plantrai_fecharegistro='".$nueva_fecha."' where plantrai_id='".$rs_listarecetas->fields["plantrai_id"]."'";
   $rs_organos1 = $DB_gogess->executec($busca_organos1,array()); 
   
    $permid_idtabla=$rs_listarecetas->fields["plantrai_id"];
	$permid_tabla="dns_dispositivomedicoexterno";
	$permid_acepta=1;
	$permid_motivo="INGRESO_ANTERIOR_SOPORTE_SISTEMA";
	$permid_fecharegistro=date("Y-m-d H:i:s");
	$usua_id=$usua_id_vl;
  ...
  //borra permisos anteriores
   ...
   $inserta_rastro="insert into pichinchahumana_extension.dns_cambiofecharegistro (cbfech_tabla,cbfech_idvalor,cbfech_fecharegistroanterior,cbfech_fecharegistronueva,cbfech_motivo,cbfech_fechacambio,usua_id) values ('dns_consultaexterna','".$id_valor."','".$fecha_anterior."','".$nueva_fecha."','".$ttext_motivo."','".date("Y-m-d H:i:s")."','".$_SESSION['datadarwin2679_sessid_inicio']."')";
