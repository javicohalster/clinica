...
$config_imp="select imp_id,imp_desarrollo from app_impresion where emp_id=? and tipocmp_id=6";
...
<div align="center">
         ...
         <div class="col-xs-12">
          ...
          <div id="guardab_btn" >
			...
			<button type="button" class="mb-sm btn btn-primary" onclick="ejecutar_submit()" >GUARDAR '.strtoupper($rs_tabla->fields["tab_title"]).'</button>
          </div>
...
<button type="button" class="mb-sm btn btn-success" onclick="ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_recibos.php','Perfil','divBody_ext',0,'<?php echo $_POST["pVar2"]; ?>',0,0,0,0,0)" style="cursor:pointer"><span class="glyphicon glyphicon-arrow-left"></span> Regresar </button>
...
echo '&nbsp;&nbsp;&nbsp;<button type="button" class="mb-sm btn btn-primary"  '.$linkeditar.'  style="background-color:#000066"  ><span class="glyphicon glyphicon-plus"></span>NUEVA PROFORMA</button>';
...
else
{
	...
	echo $_POST["pVar2"];
   $varable_enviafunc='';
   $varable_enviafunc=base64_encode("ver_formularioenpantalla('aplicativos/documental/opciones/panel/panel_recibos.php','Perfil','divBody_ext','','".$_POST["pVar2"]."',0,0,0,0,0)");
...
if($('#tippo_id').val()==1)
{
   if($('#doccab_hciess').val()=='')
   {
     alert('PARA CUENTAS POR COBRAR ES OBLIGATORIO INGRESAR Historia Clinica IESS.');
	 return false;
   }
}
