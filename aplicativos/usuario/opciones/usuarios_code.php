...
function guarda_usuario_fin(divresultado,mensaje)
{
...
function alias_data()
{
  $("#div_alias").load("aplications/usuario/opciones/extras/alias.php",{
    title:$('#title').val()
  },function(result){  
   $('#alias').val($('#alias_valor').val()); 
  });  
  $("#div_alias").html("Espere un momento...");  
...
function sacapath()
{
  $("#div_path").load("aplications/usuario/opciones/extras/path.php",{
    parent_id:$('#parent_id').val(),
	alias:$('#alias').val()
  },function(result){  
   $('#path').val($('#path_valor').val()); 
  });  
  $("#div_path").html("Espere un momento..."); 
...
$("#div_link").load("aplications/usuario/opciones/extras/link.php",{
    articulo_val:$('#articulo_val').val()
  },function(result){  
   $('#link').val($('#link_valor').val()); 
  });  
  $("#div_link").html("Espere un momento..."); 
...
{   
   ...
   $("#div_ncontenido").load("aplications/usuario/opciones/extras/assent_usuario.php",{
    usr_cedula:$('#usr_cedula').val()
  },function(result){  
   
  });  
  $("#div_ncontenido").html("Espere un momento...");  
...
<div align="center">
 ...
 <?php					
	...
	$linkeditar= 'onclick=abrir_standar("aplications/usuario/opciones/grid/grid'.$subindice.'_nuevo.php","Nuevo","divBody'.$subindice.'","divDialog'.$subindice.'",900,500,0,0,0,0,0,0,0) style=cursor:pointer';									
	$link_panel_buscar='onclick=abrir_standar_bu("aplications/usuario/opciones/grid/grid'.$subindice.'_buscar.php","Buscar","div_body_buscar","div_dialog_buscar",900,500,0,0,0,0,0,0,0) style=cursor:pointer';
