...
$subindice="_usuarios";
$tabla="spag_postulante";
$objgrid_fk->campos_visualizar="'usua_id','usr_cedula','usr_nombre','usr_apellido','usr_activo'";
$ordenlistado="order by usua_id desc";
$objgrid_fk->orden=$ordenlistado;
$objgrid_fk->leer_data("spag_postulante","","","",90,$sqltotal,$DB_gogess);
...
echo '<table width="650" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	if(count($objgrid_fk->filas)>0)
	   ...
	   echo '<tr bgcolor="#ffffff"  onmouseout=this.style.backgroundColor="#ffffff" onmouseover=this.style.cursor="hand";this.style.backgroundColor="#d4d0c8" >';
		 ...
		 $linkeditar= 'onclick=abrir_standar("aplications/usuario/opciones/grid/grid'.$subindice.'_nuevo.php","Editar","divBody'.$subindice.'","divDialog'.$subindice.'",990,500,'.$datoslista["usua_id"].',0,0,0,0,0,0) style=cursor:pointer';
       ...
       $linkborrar= 'onclick=borrar_registro("'.$tabla.'","usua_id","'.$datoslista["usua_id"].'") style=cursor:pointer';
