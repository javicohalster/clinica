...
$conectandosqlserver=mysql_connect($this->coneccionsql,$this->usuariosql,$this->clavesql);
//$conectandosqlserver=mysql_connect("(local)","sa","sa");
//if (!$conectandosqlserver) die("Error"); else {echo "conectado";};

mysql_select_db($this->basealterna);
...
//echo $sqldatos;
			   ...
			   $resultdatoscompleto = mysql_query($sqldatoscompleto);
			   ...
			   if ($resultdatos)
			   {
				 while($filasdato = mysql_fetch_array($resultdatos) and ($k<$this->desplegar)) 
					{
...
mysql_close($conectandosqlserver);
