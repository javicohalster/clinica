...
$conectandosqlserver=mssql_connect($this->coneccionsql,$this->usuariosql,$this->clavesql);
//$conectandosqlserver=mssql_connect("(local)","sa","sa");
//if (!$conectandosqlserver) die("Error"); else {echo "conectado";};

mssql_select_db($this->basealterna);
...
//echo $sqldatos;
			   ...
			   $resultdatos = mssql_query($sqldatos);
			   
			   $resultdatoscompleto = mssql_query($sqldatoscompleto);
			   ...
			   if ($resultdatos)
			   {
				 while($filasdato = mssql_fetch_array($resultdatos) and ($k<$this->desplegar)) 
					{
...
mssql_close($conectandosqlserver);
