...
return $resultado;
			...
			$objformulario->sendvar["ocupacio_enlacex"]=$aletorioid;
...
$objformulario->generar_formulario_bootstrap(@$submit,$table,181,$DB_gogess); 
?>
         ...
         $(function() {
            ...
            $( "#ocupacio_cie" ).autocomplete({
			   ...
			   select: function( event, ui ) {
				  ...
				  $('#ocupacio_cie_visorcie').val(ui.item.descripcion);
...
function ocupacio_cie_agregar() {

var datos;

datos=$('#ocupacio_impresiondiagnostica').val() + $('#ocupacio_cie_visorcie').val()+ ",";

$('#ocupacio_impresiondiagnostica').val(datos);
...
$('#ocupacio_impresiondiagnostica').ckeditor(config1);
$('#ocupacio_recomterapeutica').ckeditor(config1);	 
$('#ocupacio_recomfamiliares').ckeditor(config1);	 
$('#ocupacio_recomescolares').ckeditor(config1);	
$('#ocupacio_recommultidiciplinarias').ckeditor(config1);
