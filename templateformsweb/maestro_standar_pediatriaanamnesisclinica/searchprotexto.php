...
$lista_clientes="select prod_codigo,prod_nombre,prod_precio from efacsistema_producto where prod_nivel=".$rs_bnivel->fields["permif_id"]." and prod_nombre like '%".$_GET[ "term" ]."%'";
$rs_clientes = $DB_gogess->executec($lista_clientes,array());
 if($rs_clientes)
 {
	  while (!$rs_clientes->EOF) {	
	  
	 $companies[$ic]["label"]=utf8_encode($rs_clientes->fields["prod_nombre"]);
	 $companies[$ic]["value"]=utf8_encode($rs_clientes->fields["prod_nombre"]);
	 $companies[$ic]["codigo"]=utf8_encode($rs_clientes->fields["prod_codigo"]);
	 $companies[$ic]["descripcion"]=utf8_encode($rs_clientes->fields["prod_nombre"]);
	 $companies[$ic]["precio"]=utf8_encode($rs_clientes->fields["prod_precio"]);
