...
//busca enlace del formulario
$enlace_formulariorigin='';
if(@$this->contenid["anam_enlace"])
{
   $enlace_formulariorigin=@$this->contenid["anam_enlace"];
}
else
{
   $enlace_formulariorigin=@$this->sendvar["anam_enlacex"];
}
...
opcion:opcionp,
enlace:enlacep,
enlaceorigen:'<?php echo $enlace_formulariorigin; ?>',
...
<div class="panel-body"><div style="color:#0000FF" id="edad_sexo_<?php echo $this->fie_id;  ?>"></div><b><span style="color:#0000FF">Por favor usar punto para decimales.</span></b>
...
$rs_lcanp = $DB_gogess->executec($lista_campos,array());
 if($rs_lcanp)
 {
	  ...
	  if($rs_lcanp->fields["gridfield_tipo"]=='text')
	  {
		  ...
		  echo '<div class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].'"><label><b>'.$rs_lcanp->fields["gridfield_title"].'</b></label>';
		  echo '<input placeholder="'.$rs_lcanp->fields["gridfield_title"].'" name="'.$rs_lcanp->fields["gridfield_nameid"].'" type="text" id="'.$rs_lcanp->fields["gridfield_nameid"].'" class="form-control" value="" '.$rs_lcanp->fields["gridfield_extra"].' onkeyup="this.value = this.value.replace (/[^_0-9./ ]/,\'\');" />';
		  echo '</div>';
	  }
...
echo $this->sendvar["atenc_enlacex"]; 
...
edad_sexo_<?php echo $this->fie_id;  ?>();
//  End -->
</script>