...
<table id="calendar" width="900px" >
	...
	<tr bgcolor="silver">
...

		{
	   	...
	   	$busca_terapias="select distinct *,faesa_terapiasregistro.usua_id as medic from faesa_terapiasregistro inner join app_cliente on  faesa_terapiasregistro.clie_id=app_cliente.clie_id  where terap_fecha='".$fecha_bu."' and faesa_terapiasregistro.clie_id='".$_POST["clie_id"]."' order by terap_hora asc";
				...
				if($rs_terapia)
                   {
	                  ...
	                  while (!$rs_terapia->EOF) {	
					   ...
					   $busca_espe="select especi_nombre from dns_especialidad where especi_id=".$rs_terapia->fields["especi_id"];
					   $rs_bespe = $DB_gogess->executec($busca_espe,array());
					   
					   //print_r($rs_terapia->fields);
					    $nombre_medicox="select * from app_usuario where usua_id=".$rs_terapia->fields["medic"];
					   $rs_nmedicox = $DB_gogess->executec($nombre_medicox,array());
					   
						
						echo ucwords(strtolower(utf8_encode($nombres_p[0].' '.$apellido_p[0]))).'(T-'.$rs_terapia->fields["terap_hora"].' '.$rs_bespe->fields["especi_nombre"].'-'.$rs_nmedicox->fields["usua_codigoiniciales"].')'.$alerta;
					    echo '<br />';
