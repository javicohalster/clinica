...
//cambio de fecha registro
			...
			$datos_representante="select * from dns_newrepresentante where clie_enlace='".$rs_dcliente->fields["clie_enlace"]."' order by repres_id asc limit 1";
...
{
     $busca_diag="select count(*) as total from dns_diagnosticonewinterconsulta where intercon_enlace='".$objformulario->contenid["intercon_enlace"]."'";
...
{
     $busca_diag="select count(*) as total from dns_diagnosticonewinterconsulta where intercon_enlace='".$objformulario->sendvar["intercon_enlacex"]."'";
     $rs_diag = $DB_gogess->executec($busca_diag,array());
...
$.ajax({
    ...
    complete : function(xhr, status) {
		...
		grid_extras_11581($('#intercon_enlace').val(),0,1);
    }
});
