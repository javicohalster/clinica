...
$lista_fechatabla="select *,DATE_ADD(anam_fecharegistro,INTERVAL 20 DAY) as fechacierre from ".$rs_fields->fields["tab_name"]." where ".$campo_enlace."='".$_POST['enlace']."'";
$rs_dataftabla = $DB_gogess->executec($lista_fechatabla,array());


$lista_servicios="select *,DATE_ADD(".$campo_fecharegistro.",INTERVAL 20 DAY) as fechacierre from ".$sub_tabla." where clie_id='".$_POST['clie_id']."' order by ".$campo_fecharegistro." desc";
...
//echo $lista_servicios;
 ...
 if($rs_data)
 {
	...
	if($bloque_registro==1)
		{
	?>
	<tr><td style="font-size:10px;padding:2px;" >&nbsp;</td>
	<?php	
		}
		else
		{
