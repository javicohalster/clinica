...
if($tabla_combo)
{
$lista_servicios="select *,DATE_ADD(".$campo_fecharegistro.",INTERVAL 20 DAY) as fechacierre from ".$sub_tabla." inner join  ".$tabla_combo." on ".$sub_tabla.".".$idtabla_combo."=".$tabla_combo.".".$idtabla_combo." where ".$campo_enlace."='".$_POST['enlace']."'";
}
else
{
$lista_servicios="select *,DATE_ADD(".$campo_fecharegistro.",INTERVAL 20 DAY) as fechacierre from ".$sub_tabla." where ".$campo_enlace."='".$_POST['enlace']."' order by ".$campo_id." desc";
	 ...
	 {		
		...
		switch ($fie_camposgridselect[$i]) {
			case 'signovita_presionarterial':
                ...
                echo '<td>'.$rs_data->fields[$fie_camposgridselect[$i]].' '.$valor_txt.'</td>';
			...
			case 'signovita_masacorporal':
				{
				  ...
				  echo '<td>'.number_format($rs_data->fields[$fie_camposgridselect[$i]], 2, '.', '' ).' '.$berifica_estado.'</td>';
				}
				break;
