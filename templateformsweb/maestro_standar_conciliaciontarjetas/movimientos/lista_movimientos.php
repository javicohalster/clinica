...
if($conct_id>0)
{
$busca_elanterior="select * from app_tarjetaconciliacion where conct_fechacorte < '".$conct_fechacorte."' and conct_cuenta='".$conct_cuenta."' order by conct_fechacorte desc limit 1";
$rs_ultimafecha = $DB_gogess->executec($busca_elanterior,array());
}
else
{
$busca_elanterior="select * from app_tarjetaconciliacion where conct_cuenta='".$conct_cuenta."' order by conct_id desc limit 1";
$rs_ultimafecha = $DB_gogess->executec($busca_elanterior,array());
}
...
<table class="table table-bordered" style="width:100%">
  <tbody><tr>
   ...
   <tr>
	...
	<td></td>
	<td nowrap="nowrap"></td>
	<td><?php echo $conct_saldocontableant; ?></td>
  </tr>
...
$contador_lista++;	 
	 ...
	 $comcont_idver=$rs_listadata->fields["comcont_id"];
				...
				case 'lpin_cobropago':
					{
					   ...
					   $valor_lote='';
					   $valor_lote=$rs_bd->fields["lote_id"];
					   
					   $busca_lotevalor="select * from  lpin_lote where lottar_id='".$valor_lote."'";
					   $rs_btipoxz= $DB_gogess->executec($busca_lotevalor,array());
					   
					   $valor_lote=$rs_btipoxz->fields["lottar_recap"];
...
//$rs_listadata->fields["comcont_id"]
	...
	<td><?php echo $detalle.' -> '.$comcont_idver; ?></td>
	<td><?php echo $referencia; ?></td>
	<td><?php echo $valor_lote; ?></td>	
...
?>  
	...
	<td></td>
	<td></td>
	<td><div id="stotal"><?php echo $saldo_finalmes; ?></div></td>
  </tr>
...
<?php
echo $conct_saldocontableant."<br>";
...
if($diferencia==0)
{
  ...
  ?>
  $('#estcompt_id').val('2');
  $('#despliegue_estcompt_id').html('CONCLUIDA');
  $("#despliegue_estcompt_id").css('color','#ffffff');
  $("#despliegue_estcompt_id").css('background-color','#009933');
  <?php
}
else
{
   ...
   ?>
    $('#estcompt_id').val('1');
  $('#despliegue_estcompt_id').html('PENDIENTE');
  $("#despliegue_estcompt_id").css('color','#ffffff');
  $("#despliegue_estcompt_id").css('background-color','#990000');
   <?php
...
//enviar_formulariodata('form_app_tarjetaconciliacion');

<?php
if($conct_id>0)
{
$actualiza_data="update app_tarjetaconciliacion set conct_saldobanco='".$conct_saldobanco."',conct_saldocontable='".$saldo_finalmes."',conct_diferencia='".$diferencia."',estcompt_id='".$estcompt_id."' where conct_id='".$conct_id."'";
