...
{
$lista_sql="select * from ".$fie_tabledb."  where ".$sql_filtro." (".$listabusqueda.") and ".$gridfield_filtrobuscador;
}
else
{
$lista_sql="select * from ".$fie_tabledb."  where ".$sql_filtro." (".$listabusqueda.") ";
}
...
?>
  <tr>
    <td style="font-size:10px"><input type="button" name="Submit" value="Sel" onClick="seleccionar_recuenta('<?php echo $rs_lcanp->fields[$lista_campos[0]]; ?>')"></td>
	<?php
	for($i=1;$i<count($lista_campos);$i++)
	{
	  if($lista_campos[$i])
...
<!--
function seleccionar_recuenta(valor)
{
