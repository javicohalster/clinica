   ...
   ?>
    ...
    if ($('#<?php echo @$rs_dtabla->fields["tab_campoprimario"] ?>').val() != '') {
        ...
        myWindow3 = window.open(
            'aplicativos/documental/datos_substandarform_print.php?iddata=<?php echo @$rs_dtabla->fields["tab_id"]; ?>&pVar2=' +
            $('#<?php echo @$rs_dtabla->fields["tab_campoprimario"] ?>').val() +
            '&pVar3=<?php echo $_POST["pVar2"] ?>', 'ventana_imp', 'width=850,height=700,scrollbars=YES');
...
function genera_pdfadmision() {
    if ($('#<?php echo $rs_dtabla->fields["tab_campoprimario"]; ?>').val() > 0) {
        location.href = 'pdfformularios/pdfadmision.php?clie_id=' + $('#clie_id').val() +
            '&iddata=<?php echo @$rs_dtabla->fields["tab_id"]; ?>&pVar2=' + $(
                '#<?php echo @$rs_dtabla->fields["tab_campoprimario"] ?>').val() +
            '&pVar3=<?php echo $_POST["pVar2"] ?>'
    } else {
...
$carpeta_evolpart="informeevolucionparticular";
			...
			$busca_dtabla="select max(clie_hcpinos) as hcmx from app_cliente";
			$rs_dtabla = $DB_gogess->executec($busca_dtabla,array());
			$max_hc = @$rs_dtabla->fields["hcmx"];
			$max_hc = $max_hc+1;
			

			$objformulario->sendvar["clie_hcpinosx"]=$max_hc;
			
			
				


$busca_conv="select * from app_usuario where usua_id='".@$_SESSION['datadarwin2679_sessid_inicio']."'";
$rs_dconv = $DB_gogess->executec($busca_conv,array());
...
<div id="tabs">
    <div class="nav-wrapper position-relative end-0">
        <ul class="nav nav-pills nav-fill p-1 flex-row">

            <li class="nav-item"><a class="nav-link mb-0 px-0 py-1" href="#tabs-1">Validaci&oacute;n e Ingreso de Datos
                    Personales</a></li>


            <li class="nav-item"><a class="nav-link mb-0 px-0 py-1" href="#tabs-2">Preconsulta</a></li>
    ...
    <div id="tabs-1" style="font-size:11px">
        ...
        <p>
            ...
            <?php
$linkimprimir='onClick=genera_pdfadmision();';
echo '<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" '.$linkimprimir.'  style="cursor:pointer"><span class="glyphicon glyphicon-print"></span>&nbsp;&nbsp;IMPRIMIR ADMISIONES</button>&nbsp;&nbsp;&nbsp;';

$linkimprimirresp='onClick=imprimir_datos();';
echo '<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" '.$linkimprimirresp.'  style="cursor:pointer"><span class="glyphicon glyphicon-print"></span>&nbsp;&nbsp;IMPRIMIR INF</button>&nbsp;&nbsp;&nbsp;';
...
echo '<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" '.$linkg.'  style="cursor:pointer"> GIRAR FOTO </button>';
...
echo '&nbsp;&nbsp;&nbsp;<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" '.$linkcambior.'  style="cursor:pointer"> CAMBIAR FECHA REGISTRO </button>';
...
echo '&nbsp;&nbsp;&nbsp;<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" '.$linkhistorialr.'  style="cursor:pointer"> HISTORIAL CAMBIO </button>';
//cambio de fecha registro
...
?>
        ...
        <div class="alert alert-info alert-dismissible text-white alert alert-success" role="alert">
			<b>DATOS DEL PACIENTE / USUARIO <br /><span>!!! AVISO, NO OLVIDE
                    LLENAR LA FECHA DE NACIMIEMTO (IMPORTANTE PARA SIGNOS VITALES)<?php echo $alerta_info; ?></span>
            </b>
		</div>

        <?php
$fecha_hoydia=date("Y-m-d");
$nueva_rango=date("Y-m-d",strtotime($fecha_hoydia."+2 days")); 

//alerta interconsulta
$lista_interx="select * from dns_newinterconsulta where (intercon_fecharegistro>='".$fecha_hoydia." 01:01' and intercon_fecharegistro<='".$nueva_rango." 23:59') and clie_id='".$objformulario->contenid["clie_id"]."'";

$rs_fechainterx = $DB_gogess->executec($lista_interx,array());

if($rs_fechainterx)
   {
			 while (!$rs_fechainterx->EOF) {
			 
			 	
				$busca_ttablat="select * from gogess_sistable where tab_name='".$rs_fechainterx->fields["enlace_tablaexterno"]."'";
				$rs_buntable = $DB_gogess->executec($busca_ttablat,array());
				
				$ntablavalor='';
				$ntablavalor=$rs_buntable->fields["tab_title"];

               echo "<span style='color:#FF0000' ><b>ALERTA: Tiene una Interconsulta ID:".$rs_fechainterx->fields["intercon_id"]." en formulario ".$ntablavalor."</b></span><br>";
                
				 $rs_fechainterx->MoveNext();
      }
  }	  
...
?>
        <br>
        <button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary" onclick="seguro_rp()" style="cursor:pointer"> VERIFICAR RED
            P&Uacute;BLICA </button> &nbsp; &nbsp;<button type="button" class="fs-5 btn bg-gradient-dark mb-0 btn btn-primary"
            onclick="ver_formularioenpantalla('aplicativos/documental/opciones/panel/agendar_terapiaseguros.php','Perfil','divBody_ext','1','130',0,0,0,0,0)"
            style="cursor:pointer"> AGENDAMIENTO SEGUROS </button>
...
function desplegar_grid_informeevolucionparticular() {
    ...
    $("#grid<?php echo $subindice_evolpart; ?>").load(
        "aplicativos/documental/opciones/grid/<?php echo $carpeta_evolpart; ?>/grid<?php echo $subindice_evolpart; ?>.php", {
...
function desplegar_grid_informeevolucion() {
    ...
    $("#grid<?php echo $subindice_evol; ?>").load(
        "aplicativos/documental/opciones/grid/<?php echo $carpeta_evol; ?>/grid<?php echo $subindice_evol; ?>.php", {
...
function desplegar_grid_atencion(filtro) {
    ...
    $("#grid<?php echo $subindice_at; ?>").load(
        "aplicativos/documental/opciones/grid/<?php echo $carpeta_at; ?>/grid<?php echo $subindice_at; ?>.php", {
    ...
    $("#grid<?php echo $subindice_at; ?>").html(
        "Espere un momento <img src='images/progress/progress_1.gif' width='31' height='31' />");
...
function seguro_acces() {
    myWindow2 = window.open(
        'http://www.calidadsalud.gob.ec/acess-app-servicio-ciudadano/public/medicinaPrepagada/consulta.jsf',
        'seguro', 'width=750,height=500,scrollbars=YES');
...
function seguro_rp() {
    myWindow2 = window.open('https://coresalud.msp.gob.ec/coresalud/app.php/publico/rpis/afiliacion/consulta', 'Red',
...
function verificar_seguro(fecha) {
   ...
   $fechaf=date("d-m-Y");
    ?>
        var url = "https://coresalud.msp.gob.ec/coresalud/app.php/publico/rpis/afiliacion/consultafiliacion/" + $(
            '#clie_rucci').val() + "/<?php echo $fechaf; ?>";
        ...
        $("#div_verifica").load("../../../scraping2/lektor.php", {
            ...
            if ($('#dia_clie_fechanacimiento').val() == 'null' || $('#dia_clie_fechanacimiento').val() ==
                '00') {
                $('#dia_clie_fechanacimiento').val($('#dia_clie_fechanacimientov').val());
            }
            if ($('#mes_clie_fechanacimiento').val() == 'null' || $('#mes_clie_fechanacimiento').val() ==
                '00') {
                $('#mes_clie_fechanacimiento').val($('#mes_clie_fechanacimientov').val());
            }
            if ($('#anio_clie_fechanacimiento').val() == 'null' || $('#anio_clie_fechanacimiento').val() ==
                '00000') {
            ...
            fecha_bloqueasignaedad('dia_clie_fechanacimiento', 'mes_clie_fechanacimiento',
                'anio_clie_fechanacimiento', 'clie_fechanacimiento');
...
$(function() {
    $("#clie_rucci").autocomplete({
        ...
        select: function(event, ui) {
            ...
            $('#clie_celular').val(ui.item.celular);
            $('#clie_telefono').val(ui.item.telefono);
            $('#clie_email').val(ui.item.email);
            $('#civil_id').val(ui.item.civil);
            $('#clie_fechanacimiento').val(ui.item.clie_fechanacimiento);

            $('#anio_clie_fechanacimiento').val(ui.item.anio_clie_fechanacimiento);
            $('#mes_clie_fechanacimiento').val(ui.item.mes_clie_fechanacimiento);
            $('#dia_clie_fechanacimiento').val(ui.item.dia_clie_fechanacimiento);
            $('#clie_genero').val(ui.item.clie_genero);
            $('#clie_instruccion').val(ui.item.clie_instruccion);
            $('#grp_id').val('12');

            $('#prob_codigo').val(ui.item.prob_codigo);
            $('#cant_codigo').val(ui.item.cant_codigo);
...
{
?>
showUser_combog('cant_codigo', $('#prob_codigo').val(), 'divcant_codigo', 'prob_codigo', 'app_cliente', '', 0, 0, 0, 0,
    0);
