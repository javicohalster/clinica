  ...
  <tr>
   ...
   <?php
	for($i=0;$i<count($fie_tituloscamposgrid);$i++)
	 {
      echo '<td>'.$fie_tituloscamposgrid[$i].'</td>';
	 }
	?>
  </tr>
<?php
 ...
 if($rs_data)
 {
  ...
  <tr>
	...
	for($i=0;$i<count($fie_camposgridselect);$i++)
	 {
		 $ver_campos="select * from gogess_gridfield where gridfield_nameid='".$fie_camposgridselect[$i]."x' and fie_id=".$_POST["fie_id"]; 
		...
		$busca_valor="select ".$rs_campd->fields["gridfield_camposcmb"]." from ".$rs_campd->fields["gridfield_tablecmb"]." where ".$dataval[0]."='".$rs_data->fields[$fie_camposgridselect[$i]]."';";
		 $rs_bval = $DB_gogess->executec($busca_valor,array());
		 $dato_valor=$rs_bval->fields[$dataval[1]];
		 
		 echo '<td>'.$dato_valor.'</td>';
		 }
		 else
		 {
