...
$bandera_cie=0;
$lista_nex="";
if($objformulario->contenid["conext_enlace"])
{
     $busca_diag="select count(*) as total from pichinchahumana_extension.dns_hospitaldiagnosticoexterna where conext_enlace='".$objformulario->contenid["conext_enlace"]."'";
     $rs_diag = $DB_gogess->executec($busca_diag,array());
	 
	 $lista_nex="select diagn_cie from pichinchahumana_extension.dns_hospitaldiagnosticoexterna where conext_enlace='".$objformulario->contenid["conext_enlace"]."'";
     
	 $bandera_cie=$rs_diag->fields["total"];
}
else
{
     $busca_diag="select count(*) as total from pichinchahumana_extension.dns_hospitaldiagnosticoexterna where conext_enlace='".$objformulario->sendvar["conext_enlacex"]."'";
     $rs_diag = $DB_gogess->executec($busca_diag,array());
	 
	 $lista_nex="select diagn_cie from pichinchahumana_extension.dns_hospitaldiagnosticoexterna where conext_enlace='".$objformulario->sendvar["conext_enlacex"]."'";
...
function genera_cieexternoprex(codigo,diagn_tipox,idext)
{
...
$.ajax({
    ...
    complete : function(xhr, status) {
		...
		grid_extras_8009($('#conext_enlace').val(),0,1);
    }
});
...
//@$objformulario->sendvar["protoop_idenlacex"]=$protoop_idenlace;
	...
	$busca_diag="select * from dns_hospitalanamesis where anam_id='".$anam_id."'";
	$rs_budiag = $DB_gogess->executec($busca_diag,array());	
	
	
	
	 $busca_listadiag="select * from pichinchahumana_extension.dns_hospitaldiagnosticoanamnesis where anam_enlace='".$rs_budiag->fields["anam_enlace"]."' and  diagn_cie not in(".$lista_nex.")  order by 1 asc";
	 $rs_listd = $DB_gogess->executec($busca_listadiag,array());
	 if($rs_listd)
     {
	   while (!$rs_listd->EOF) {	
	   
	    echo " genera_cieexternoprex('".$rs_listd->fields["diagn_cie"]."','".$rs_listd->fields["diagn_tipo"]."','0'); ";
