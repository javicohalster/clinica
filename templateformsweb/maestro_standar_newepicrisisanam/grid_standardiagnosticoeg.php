...
if($tabla_combo)
{
$lista_servicios="select *,DATE_ADD(".$campo_fecharegistro.",INTERVAL ".$intervalo_v." DAY) as fechacierre from ".$sub_tabla." inner join  ".$tabla_combo." on ".$sub_tabla.".".$idtabla_combo."=".$tabla_combo.".".$idtabla_combo." where ".$campo_enlace."='".$_POST['enlace']."'";
}
else
{
$lista_servicios="select *,DATE_ADD(".$campo_fecharegistro.",INTERVAL ".$intervalo_v." DAY) as fechacierre from ".$sub_tabla." where ".$campo_enlace."='".$_POST['enlace']."'";
...
//echo $lista_servicios;
 ...
 if($rs_data)
 {
	  while (!$rs_data->EOF) {	
		...
		$vaalerta=$objformulario->replace_cmb('dns_cie','cie_codigo,cie_reporteobligatorio','where cie_codigo like ',trim($rs_data->fields["diagneg_cie"]),$DB_gogess);
		
		$vaalertacatas=$objformulario->replace_cmb('dns_cie','cie_codigo,cie_catastroficas','where cie_codigo like ',trim($rs_data->fields["diagneg_cie"]),$DB_gogess);
		
		$vaalertahuefana=$objformulario->replace_cmb('dns_cie','cie_codigo,cie_huerfanas','where cie_codigo like ',trim($rs_data->fields["diagneg_cie"]),$DB_gogess);
...
<?php
  ...
  echo '<input name="diagneg_numval" type="hidden" id="diagneg_numval" value="'.$cuenta.'" />';
