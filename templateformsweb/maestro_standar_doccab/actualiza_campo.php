...
$saca_total="select ((docdet_cantidad*docdet_preciou)-docdet_descuento) as total_valor from app_documentodetalle where docdet_id=".$_POST["id_detallep"];
...
echo '<input name="total_linea" type="hidden" id="total_linea" value="'.number_format($rs_totallinea->fields["total_valor"], 2, '.', '').'">';
...
$split_nedita=explode(",",$rs_detalle->fields["subgri_campoedita"]);
 ...
 if($rs_totales)
 {
  ...
  $total_apagar=$suma_general+@$sutotales[2]["conimpuesto"]+@$sutotales[3]["conimpuesto"];
