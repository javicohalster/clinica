...
if($tabla_combo)
{
$lista_servicios="select *,DATE_ADD(".$campo_fecharegistro.",INTERVAL 2000 DAY) as fechacierre from ".$sub_tabla." inner join  ".$tabla_combo." on ".$sub_tabla.".".$idtabla_combo."=".$tabla_combo.".".$idtabla_combo." where ".$campo_enlace."='".$_POST['enlace']."'  order by detcc_haber,detcc_debe desc ";
}
else
{
$lista_servicios="select *,DATE_ADD(".$campo_fecharegistro.",INTERVAL 2000 DAY) as fechacierre from ".$sub_tabla." where ".$campo_enlace."='".$_POST['enlace']."'  order by detcc_haber,detcc_debe desc ";
...
//echo $lista_servicios;
				...
				default:
				   {
					 ...
					 if($fie_camposgridselect[$i]=='detcc_debe' or $fie_camposgridselect[$i]=='detcc_haber')
					 {					 
 ...
 $alerta_valor='SUMA';  
 $alerta_color='#DFE9EE';  
 if(trim($suma_data["detcc_debe"])!=trim($suma_data["detcc_haber"]))
 {
   $alerta_valor='<span class="blink">NO CUADRA</span>';
   $alerta_color='#FF0000'; 
 }
 $oculto_valida='';
 if($suma_data["detcc_debe"]>0 and $suma_data["detcc_haber"]>0 and trim($suma_data["detcc_debe"])==trim($suma_data["detcc_haber"]))
 { 
  $oculto_valida='<input name="cuadra_valor" type="hidden" id="cuadra_valor" value="1" />'; 
 }
 else
 {
   $oculto_valida='<input name="cuadra_valor" type="hidden" id="cuadra_valor" value="0" />';
 }
 
  ?>
  <tr>
    <td bgcolor="<?php echo $alerta_color; ?>" ></td>
	<td bgcolor="<?php echo $alerta_color; ?>" ><b><?php echo $alerta_valor; ?></b> <?php echo $oculto_valida; ?></td>
   <?php
	for($i=0;$i<count($fie_camposgridselect);$i++)
	 {
      echo '<td bgcolor="'.$alerta_color.'" ><b>'.@$suma_data[$fie_camposgridselect[$i]].'</b></td>';
	 }
	?>
  </tr> 
</table>
