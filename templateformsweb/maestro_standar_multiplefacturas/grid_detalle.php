...
$table='beko_multipledocumentodetalle';
...
$busca_exixtenciax="select doccab_estadosri from beko_multipledocumentocabecera where doccab_id='".$_POST["enlace"]."'";
$rs_exixtenx = $DB_gogess->executec($busca_exixtenciax,array());


function tomad_dcampos($gogess_sisfield,$campo)
{
    $datos_campos=array();
	
    for($z=0;$z<count($gogess_sisfield);$z++)
	   {
	       if($gogess_sisfield[$z]["fie_name"]==$campo)
		   {
		     $datos_campos=$gogess_sisfield[$z];
		   }
  ...
  <tr bgcolor="#DBEDEE">
	...
	for($i=0;$i<count($split_ncampo);$i++)
	{
	   ...
	   for($z=0;$z<count($gogess_sisfield);$z++)
	   {
		  ...
		  if($gogess_sisfield[$z]["fie_name"]==$split_ncampo[$i])
		  {
		      echo '<td class="txt_titulo" ><b>'.$gogess_sisfield[$z]["fie_title"].'</b></td>';		  
		  }
...
$rs_preciot= $DB_gogess->executec($busca_preciot,array());
...

	{
	  ...
	  $evento_data='';
	   ...
	   $campo_val=tomad_dcampos($gogess_sisfield,$split_ncampo[$i]);
...

	   {
	      ...
	      $indice_encontro = array_search($campo_val["fie_name"],$split_nedita);
...

		   {
		      ...
		      $evento_data=" onChange=actualiza_campo('beko_multipledocumentodetalle','".$campo_val["fie_name"]."','docdet_id','".$rs_data->fields[$rs_detalle->fields["subgri_campoidts"]]."',$('#val_mod_".$campo_val["fie_name"]."-".$rs_data->fields[$rs_detalle->fields["subgri_campoidts"]]."').val()) ";
...
//actualiza_campo('beko_multipledocumentodetalle','docdet_descuento','docdet_id','1',$('#val_mod_docdet_descuento-1').val())



  $evento_data_dg=" onChange=actualiza_descuentogeneral('beko_multipledocumentodetalle','docdet_descuento_general','doccab_id','".$_POST["enlace"]."',$('#descuento_g').val())  ";
...
/*$busca_existepago="select docfpag_id,docfpag_valor from beko_multipledocumentoformapago where doccab_id='".$_POST["enlace"]."' and docfpag_valor>0";
$rs_existepago = $DB_gogess->executec($busca_existepago,array());
if(@$rs_existepago->fields["docfpag_valor"]!=$total_apagar)
{

     $busca_sidata="select docfpag_id,docfpag_valor from beko_multipledocumentoformapago where frm_id=1 and doccab_id='".$_POST["enlace"]."'";
     $rs_bdata = $DB_gogess->executec($busca_sidata,array());
	 if($rs_bdata->fields["docfpag_id"])
	 {
	   $actualiza="update beko_multipledocumentoformapago set docfpag_valor=".$total_apagar." where frm_id=1 and doccab_id='".$_POST["enlace"]."'";
	   $rs_actualiza = $DB_gogess->executec($actualiza,array());
	 }
	 else
	 {
	   $inserta="insert into beko_multipledocumentoformapago (frm_id,doccab_id,docfpag_valor) values (1,'".$_POST["enlace"]."',".$total_apagar.")";
