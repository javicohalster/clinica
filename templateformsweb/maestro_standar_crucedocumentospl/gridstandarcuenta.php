...
$rs_lcanp = $DB_gogess->executec($lista_campos,array());
 if($rs_lcanp)
 {
	  ...
	  if($rs_lcanp->fields["gridfield_tipo"]=='select')
	  {
		  ...
		  $this->fill_cmb($rs_lcanp->fields["gridfield_tablecmb"],$rs_lcanp->fields["gridfield_camposcmb"],'',$consulta_valor.' '.$rs_lcanp->fields["gridfield_ordercmb"],$DB_gogess);
		  echo '</select>';
...
abrir_standar('templateformsweb/maestro_standar_ventas/buscador/busca_data.php','Buscador','divBody_buscadorgeneral','divDialog_buscadorgeneral',500,400,id,0,0,0,0,0,0);
...
$("#lista_producto").load("<?php echo $this->formulario_path; ?>calculoc.php",{
   ...
   cueven_preciounitariox:$('#cueven_preciounitariox').val(),
   cueven_cantidadx:$('#cueven_cantidadx').val(),
   cueven_descuentodolarx:$('#cueven_descuentodolarx').val(),
   cueven_descuentox:$('#cueven_descuentox').val()
...
if($('#cueven_descuentodolarx').val()=='')
{
  descuento=parseFloat('0');
}
else
{
  descuento=parseFloat($('#cueven_descuentodolarx').val());
}

precio=parseFloat($('#cueven_preciounitariox').val());
cantidad=parseFloat($('#cueven_cantidadx').val());
total=(precio*cantidad)-descuento;
$('#cueven_subtotalx').val(total.toFixed(2));
  genera_totales();
...
$( "#cueven_preciounitariox" ).change(function() {
   calcula_datac();
});

$( "#cueven_cantidadx" ).change(function() {
   calcula_datac();
});

$( "#cueven_descuentodolarx" ).change(function() {
   $('#cueven_descuentox').val('');
   calcula_datac();
});


//porcentaje descuento
$( "#cueven_descuentox" ).change(function() {
   calcula_valordesccuenta();
});

function calcula_valordesccuenta()
{
  ...
  preciounitario=parseFloat($('#cueven_preciounitariox').val());
  cantidad=parseFloat($('#cueven_cantidadx').val());
  porcentaje=parseFloat($('#cueven_descuentox').val());  
  
  if(porcentaje>0)
  {
  valorcalculado=(preciounitario*porcentaje/100)*cantidad;
  
  $('#cueven_descuentodolarx').val(valorcalculado.toFixed(2));
