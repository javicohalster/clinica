...
//echo xml_valor;
		  ...
		  else
		  {
			 ...
			 $buscasies_enveloped2=0;
			 $resultado_emb2 = strpos($xml_valor, 'numeroAutorizacion>');
			 if($resultado_emb2 !== FALSE){
					 $buscasies_enveloped2=1;
			 }
			if($buscasies_enveloped2==1)
			{			 
			...
			{
			  //sin encabezado
			   $arreglo_data=leer_xml_env_sincab($xml_valor);
			   
			  ?>
<script type="text/javascript">
<!--

alert('ALERTA XML SIN CABECERA DE AUTORIZACION...');
...
$compra_procesado=0;
if($arreglo_data["codDoc"]=='04')
{
//busca si es nota de credito y si la factura fue procesada ya no permite
//verifica si ya fue procesada la factura
...
$compra_procesado=$rs_buscfact->fields["compra_procesado"];
//verifica si ya fue procesada la factura
}
$compra_procesado=0;
...
alert('NOTA DE CREDITO NO SE PUEDE SER INGRESADA YA QUE LA FACTURA QUE ESTA AFECTADA YA FUE PROSESADA EN BODEGA...');
...
$cdxml_cantidad=str_replace(",","",$arreglo_data["detalles"][$ip]["cantidad"]);
$cdxml_descripcion=str_replace("'","",$arreglo_data["detalles"][$ip]["descripcion"]);
$cdxml_preciounitario=str_replace(",","",$arreglo_data["detalles"][$ip]["preciounitario"]);
$cdxml_descuento=$arreglo_data["detalles"][$ip]["descuento"];
$cdxml_iva=$arreglo_data["detalles"][$ip]["iva"];
$cdxml_totalsinimpuestos=str_replace(",","",$arreglo_data["detalles"][$ip]["total"]);
