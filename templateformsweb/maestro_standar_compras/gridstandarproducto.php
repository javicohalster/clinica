...
$busca_comprax = "select * from dns_compras where compra_enlace='" . $enlace_data . "'";
...
<div class="panel panel-default">
	<div class="panel-body">
		...
		<button type="button" class="mb-sm btn btn-primary" onClick="genera_copiaproducto()" style="background-color:#000066">COPIAR EL PRIMERO A TODOS</button>
...
<script type="text/javascript">
	...
	function calcula_data() {
		...
		$("#lista_producto").load("<?php echo $this->formulario_path; ?>calculo.php", {

			prcomp_preciounitariox: $('#prcomp_preciounitariox').val(),
			prcomp_cantidadx: $('#prcomp_cantidadx').val(),
			prcomp_descuentox: $('#prcomp_descuentox').val(),
			prcomp_descuentodolarx: $('#prcomp_descuentodolarx').val()
	...
	$("#prcomp_preciounitariox").change(function() {
		calcula_data();
	});

	$("#prcomp_cantidadx").change(function() {
		calcula_data();
	});

	$("#prcomp_descuentox").change(function() {
		calcula_data();
	});

	$("#prcomp_descuentodolarx").change(function() {
		calcula_data();
	});
	...
	function genera_copiaproducto() {


		$("#g_copiaspr").load("<?php echo $this->formulario_path; ?>actualizartodopr.php", {
		...
		$("#g_copiaspr").html("Espere un momento...");
