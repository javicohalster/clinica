...
if ($gridfield_filtrobuscador) {
	$lista_sql = "select * from " . $fie_tabledb . "  where " . $sql_quita . " " . $sql_filtro . " (" . $listabusqueda . ") and " . $gridfield_filtrobuscador;
} else {
	$lista_sql = "select * from " . $fie_tabledb . "  where " . $sql_quita . " " . $sql_filtro . " (" . $listabusqueda . ") ";
}
...
<table border="1" align="center">
	...
	if ($rs_lcanp) {
		while (!$rs_lcanp->EOF) {
			$provee_diasvencimiento = 0;
			$provee_diasvencimiento = $rs_lcanp->fields["provee_diasvencimiento"];
	...
	?>
			<tr>
				<td style="font-size:10px"><input type="button" name="Submit" value="Sel" onClick="seleccionar_regn('<?php echo $rs_lcanp->fields[$lista_campos[0]]; ?>','<?php echo $provee_diasvencimiento; ?>')"></td>
				<?php
				for ($i = 1; $i < count($lista_campos); $i++) {
					if ($lista_campos[$i]) {
...
<script type="text/javascript">
	<!--
	function seleccionar_regn(valor, vencimiento) {

		$('#<?php echo $fie_name ?>').val(valor);
		$('#compra_vencimiento').val(vencimiento);
		//llena_cliente();
