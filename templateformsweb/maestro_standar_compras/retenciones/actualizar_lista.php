...
$lista_detalles="select sum(prcomp_subtotal) as total,porcei_id,prcomp_taricodigo  from lpin_productocompra where compra_enlace='".$compra_enlace."' and porcei_id>0 and prcomp_taricodigo in (2,4) group by prcomp_taricodigo";
$rs_data = $DB_gogess->executec($lista_detalles,array());
 if($rs_data)
 {
	  while (!$rs_data->EOF) {	
		 ...
		 if($rs_data->fields["prcomp_taricodigo"]==2)
		 {
		 $valor_iva=($rs_data->fields["total"]*12)/100;
		 }
		 
		 if($rs_data->fields["prcomp_taricodigo"]==4)
		 {
...
$lista_detalles="select sum(cuecomp_subtotal) as total,porceci_id,taric_id  from lpin_cuentacompra where compra_enlace='".$compra_enlace."' and porceci_id>0 and taric_id in (1,5) group by porceci_id";
$rs_data = $DB_gogess->executec($lista_detalles,array());
 if($rs_data)
 {
	  while (!$rs_data->EOF) {	
		 ...
		 if($rs_data->fields["taric_id"]==1)
		 {
		 $valor_iva=($rs_data->fields["total"]*12)/100;
		 }
		 
		 if($rs_data->fields["taric_id"]==5)
		 {
		 $valor_iva=($rs_data->fields["total"]*15)/100;
		 }
...
$lista_detalles="select sum(acfi_subtotal) as total,porcefi_id,tarif_id  from dns_activosfijos where compra_enlace='".$compra_enlace."' and porcefi_id>0 and tarif_id in (1,5) group by porcefi_id";
$rs_data = $DB_gogess->executec($lista_detalles,array());
 if($rs_data)
 {
	  while (!$rs_data->EOF) {	  
		 ...
		 if($rs_data->fields["tarif_id"]==1)
		 {
		 $valor_iva=($rs_data->fields["total"]*12)/100;
		 }
		 
		 if($rs_data->fields["tarif_id"]==5)
		 {
		 $valor_iva=($rs_data->fields["total"]*15)/100;
		 }
