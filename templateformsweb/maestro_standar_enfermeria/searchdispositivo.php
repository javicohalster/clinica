...
{
$ic=0;
$lista_clientes="select nombre as cuadrobm_nombredispositivo,'' as cuadrobm_presentacion,codigo as cuadrobm_codigoitem,0 as cuadrobm_preciodispositivo from api_productos where estado='A'";
$rs_clientes = $DB_gogess->executec($lista_clientes,array());
 if($rs_clientes)
 {
	  while (!$rs_clientes->EOF) {	
	  
	 $companies[$ic]["label"]=$rs_clientes->fields["cuadrobm_codigoitem"];
	 $companies[$ic]["value"]=$rs_clientes->fields["cuadrobm_codigoitem"];
	 $companies[$ic]["codigo"]=$rs_clientes->fields["cuadrobm_codigoitem"];
