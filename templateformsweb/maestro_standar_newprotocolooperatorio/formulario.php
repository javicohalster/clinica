...
?>
<center><input type="button" name="Submit" value="BORRAR IMAGEN" onclick="borrar_imagen()" /></center><br />
</div>
<div id="signos_id">
<?php
...
{
     $busca_diag="select count(*) as total from pichinchahumana_extension.dns_newprotocolodiagnosticospre where protoop_enlace='".$objformulario->contenid["protoop_enlace"]."'";
...
{
     $busca_diag="select count(*) as total from pichinchahumana_extension.dns_newprotocolodiagnosticospre where protoop_enlace='".$objformulario->sendvar["protoop_enlacex"]."'";
     $rs_diag = $DB_gogess->executec($busca_diag,array());
...
if($objformulario->contenid["protoop_enlace"])
{
     $busca_diagx="select count(*) as total from pichinchahumana_extension.dns_newprotocolodiagnosticospost where protoop_enlace='".$objformulario->contenid["protoop_enlace"]."'";
...
{
     $busca_diagx="select count(*) as total from pichinchahumana_extension.dns_newprotocolodiagnosticospost where protoop_enlace='".$objformulario->sendvar["protoop_enlacex"]."'";
     $rs_diagx = $DB_gogess->executec($busca_diagx,array());
...
{
    let text;
	if (confirm("Esta seguro que desea quitar la imagen") == true) {
	   $('.showImageprotoop_sintesisimagen').html('');
	   $('#protoop_sintesisimagen').val('');
	}
		 ...
		 $(function() {
            $( "#diagnpre_ciex" ).autocomplete({
               source: "templateformsweb/maestro_standar_newprotocolooperatorio/searchcie.php",
               minLength: 2,
		 ...
		 $(function() {
            $( "#diagnpre_descripcionx" ).autocomplete({
               source: "templateformsweb/maestro_standar_newprotocolooperatorio/searchcietexto.php",
               minLength: 3,
			   select: function( event, ui ) {
		 ...
		 $(function() {
            $( "#gridhill_codigox" ).autocomplete({
               source: "templateformsweb/maestro_standar_newprotocolooperatorio/searchhill.php",
               minLength: 2,
			   select: function( event, ui ) {
		 ...
		 $(function() {
            $( "#gridhill_descripcionx" ).autocomplete({
               source: "templateformsweb/maestro_standar_newprotocolooperatorio/searchhilltexto.php",
               minLength: 3,
			   select: function( event, ui ) {
...
$.ajax({
    ...
    url : 'templateformsweb/maestro_standar_newprotocolooperatorio/searchcie.php',
    ...
    complete : function(xhr, status) {
		...
		grid_extras_11457($('#protoop_enlace').val(),0,1);
    }
});
...
$.ajax({
    ...
    complete : function(xhr, status) {
        ///alert('Petición realizada');
		grid_extras_11458($('#protoop_enlace').val(),0,1);
    }
});
