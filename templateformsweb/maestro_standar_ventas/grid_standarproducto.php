...
<?php
$linkicopiar='onClick=proceso_copiarr();';
$linkicopiaiva='onClick=proceso_copiaiva();';

echo '<div align="right"><button type="button" class="mb-sm btn btn-info" '.$linkicopiar.'  style="cursor:pointer"> Copiar Ret. IR </button>  <button type="button" class="mb-sm btn btn-info" '.$linkicopiaiva.'  style="cursor:pointer"> Copiar 	Ret. IVA </button><div id="ac_col1"></div></div>';
...
$validat_sello=0;	
 if($validat_sello>0)
 {
  ...
  ?>
	...
	if($cuenta==1)
			{								  
				$campo_2='cmb_'.$ncampo_val.$rs_data->fields[$ide_producto];								
			}
...
function proceso_copiarr()
{


  path='templateformsweb/maestro_standar_ventas/actualizacolumna1.php';   

 $("#ac_col1").load(path,{ 
   
   campo_1:$('#<?php echo $campo_1; ?>').val(),
  ...
  $("#ac_col1").html("Procesando...");  


}


function proceso_copiaiva()
{


  path='templateformsweb/maestro_standar_ventas/actualizacolumna2.php';   

 $("#ac_col1").load(path,{ 
   
   campo_1:$('#<?php echo $campo_2; ?>').val(),
   enlace:'<?php echo $_POST['enlace']; ?>',
   doccab_id:$('#doccab_id').val()
