...
function leercampos_xml_t($xml)
	{
	  ...
	  $datosrr["fechaemision"] = $struct_detail->infoCompRetencion->fechaEmision->__toString(); 
	  $datosrr["tipoIdentificacionSujetoRetenido"] = $struct_detail->infoCompRetencion->tipoIdentificacionSujetoRetenido->__toString(); 
	  $datosrr["razonSocialSujetoRetenido"] = $struct_detail->infoCompRetencion->razonSocialSujetoRetenido->__toString(); 
	  $datosrr["identificacionSujetoRetenido"] = $struct_detail->infoCompRetencion->identificacionSujetoRetenido->__toString(); 
	  $decimales=2;
	  $contador_det=0;
	  $numdetalle=count($struct_detail->impuestos->impuesto);
	  
	     for($id=0;$id<$numdetalle;$id++)
		  {
		     $descriptxt=stripslashes(htmlentities($struct_detail->impuestos->impuesto[$id]->descripcion));
			 
			 $datosrr["detalles"][$contador_det]["codigo"]=$struct_detail->impuestos->impuesto[$id]->codigo->__toString();
			 $datosrr["detalles"][$contador_det]["codigoRetencion"]=$struct_detail->impuestos->impuesto[$id]->codigoRetencion->__toString();
			 $datosrr["detalles"][$contador_det]["baseImponible"]=$struct_detail->impuestos->impuesto[$id]->baseImponible->__toString();
			 $datosrr["detalles"][$contador_det]["porcentajeRetener"]=$struct_detail->impuestos->impuesto[$id]->porcentajeRetener->__toString();
			 $datosrr["detalles"][$contador_det]["valorRetenido"]=$struct_detail->impuestos->impuesto[$id]->valorRetenido->__toString();
			 $datosrr["detalles"][$contador_det]["codDocSustento"]=$struct_detail->impuestos->impuesto[$id]->codDocSustento->__toString();
			 $datosrr["detalles"][$contador_det]["numDocSustento"]=$struct_detail->impuestos->impuesto[$id]->numDocSustento->__toString();
