...
$busca_pedido="select * from ".$_POST["tabla"]." where doccab_id='".$_POST["precu_id"]."'";
...
$detapre_idgrid=0;
$table='';
$detapre_origen='DESCARGO FARMACIA';
...
//busca serial
$busca_serial="select usua_id,cuadrobm_codigoatc from dns_cuadrobasicomedicamentos where cuadrobm_id='".$cuadrobm_id."'";
$rs_serial = $DB_gogess->executec($busca_serial,array());
//

$busca_agregado="select * from beko_mhdetallefactura where mhdetfac_codprincipal='".$rs_serial->fields["cuadrobm_codigoatc"]."' and doccab_id='".$precu_id."'";
$rs_agregado = $DB_gogess->executec($busca_agregado,array());


$cantidad_nueva=$cantidad_valor;


$busca_dataproducto="select '".$precu_id."' as doccab_id,cuadrobm_codigoatc,cuadrobm_nombrecomercial,'".$cantidad_nueva."' as mhdetfac_cantidad,round((".$valor_precio."),2) as ".$valor_precio.",dns_cuadrobasicomedicamentos.impu_codigo,dns_cuadrobasicomedicamentos.tari_codigo,tari_valor,round((((".$cantidad_nueva."*(".$valor_precio."))*tari_valor)/100),2) as mhdetfac_valorimpuesto,(".$cantidad_nueva."*round((".$valor_precio."),2)) as mhdetfac_total,".$_SESSION['datadarwin2679_sessid_inicio']." as usua_id,'".$moviin_iddes."' as moviin_id from dns_cuadrobasicomedicamentos inner join beko_tarifa on dns_cuadrobasicomedicamentos.tari_codigo=beko_tarifa.tari_codigo where  cuadrobm_id='".$cuadrobm_id."'";
