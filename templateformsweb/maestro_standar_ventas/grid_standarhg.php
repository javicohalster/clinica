...
$busca_precuenta="select * from beko_mhdetallefactura where ".$campo_enlace."='".$_POST["enlace"]."' and ".$campo_id."=".$_POST["idgrid"];
...
<div class="table-responsive" id="formidx">


<?php
$linkicopiarhg='onClick=proceso_copiarrhg();';
$linkicopiaivahg='onClick=proceso_copiaivahg();';

echo '<div align="right"><button type="button" class="mb-sm btn btn-info" '.$linkicopiarhg.'  style="cursor:pointer"> Copiar Ret. IR </button>  <button type="button" class="mb-sm btn btn-info" '.$linkicopiaivahg.'  style="cursor:pointer"> Copiar 	Ret. IVA </button><div id="ac_col1hg"></div></div>';
...
<table class="table table-bordered"  style="width:100%" >
  <tr>
	...
	<?php
	  ...
	  {	
		echo '<input type="button" name="enviar_dx" id="enviar_dx" value="Eliminar" />';
	  }
	?>
	</td>
    <td bgcolor="#DFE9EE" >Eliminar
	<?php
	 if($bloque_registro==0)
	  {	
       echo '	<br><input type="checkbox" name="checkbox" value="0" id="MarcarTodos" />';	
...
$listacuenta=1;
   ...
   $tabla_valordata="";
   $campo_valor="";	
   $tabla_valordata="'beko_mhdetallefactura'";
   $campo_valor="'mhdetfac_id'";
   $ide_producto='mhdetfac_id';
...
$validat_sello=0;	
 if($validat_sello>0)
 {
  ...
  ?>
   <td><?php echo $listacuenta++; ?></td>
  <td><input type="checkbox" name="checkbox_prx[]"  value="<?php echo $rs_data->fields[$campo_id]; ?>" /></td>
	...
	$ncampo_val='porcevhr_id';
	
	if($cuenta==1)
		{								  
			$campo_1hg='cmb_'.$ncampo_val.$rs_data->fields[$ide_producto];								
		}
	...
	$ncampo_val='porcevhi_id';
	
		if($cuenta==1)
			{								  
				$campo_2hg='cmb_'.$ncampo_val.$rs_data->fields[$ide_producto];								
			}
...
$(document).ready(function() {
	$('#enviar_dx').click(function(){
		var selected = '';	
		$('#formidx input[type=checkbox]').each(function(){
			if (this.checked) {
				selected += $(this).val()+', ';
			}
		}); 
...
<script >
   $('#formidx').ready(function () {
   $("#MarcarTodos").change(function () {
      $("#formidx input:checkbox").prop('checked', $(this).prop("checked"));
   });
});
</script>
...
<!--
 ...
 function proceso_copiarrhg()
{


  path='templateformsweb/maestro_standar_ventas/actualizacolumna1hg.php';   

 $("#ac_col1hg").load(path,{ 
   
   campo_1:$('#<?php echo $campo_1hg; ?>').val(),
   enlace:'<?php echo $_POST['enlace']; ?>',
  ...
  $("#ac_col1hg").html("Procesando...");  


}


function proceso_copiaivahg()
{


  path='templateformsweb/maestro_standar_ventas/actualizacolumna2hg.php';   

 $("#ac_col1hg").load(path,{ 
   
   campo_1:$('#<?php echo $campo_2hg; ?>').val(),
   enlace:'<?php echo $_POST['enlace']; ?>',
   doccab_id:$('#doccab_id').val()
