...
@$bloqueo_cierre=$_POST["bloqueo_cierre"];
$bloque_registro=$bloqueo_cierre;
...
if($tabla_combo)
{
$lista_servicios="select *,DATE_ADD(".$campo_fecharegistro.",INTERVAL 0 DAY) as fechacierre from ".$sub_tabla." inner join  ".$tabla_combo." on ".$sub_tabla.".".$idtabla_combo."=".$tabla_combo.".".$idtabla_combo." where ".$campo_enlace."='".$_POST['enlace']."'";
}
else
{
$lista_servicios="select *,DATE_ADD(".$campo_fecharegistro.",INTERVAL 0 DAY) as fechacierre from ".$sub_tabla." where ".$campo_enlace."='".$_POST['enlace']."' order by ".$campo_fecharegistro." desc";
...
//echo $lista_servicios;
 ...
 if($rs_data)
 {
	  ...
	  $total_global=$total_global+$rs_data->fields["ingefec_valor"];
	    $cuenta++;
		//echo $rs_data->fields[$campo_fecharegistro];
...
<b>TOTAL EN DINERO:</b> <?php echo $total_global; ?>
</div>

<script language="javascript">
<!--
  $('#cierr_totaldinero').val('<?php echo $total_global; ?>');
