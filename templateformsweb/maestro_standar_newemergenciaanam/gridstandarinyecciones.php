...
$rs_lcanp = $DB_gogess->executec($lista_campos,array());
 if($rs_lcanp)
 {
	  ...
	  while (!$rs_lcanp->EOF) {
	    ...
	    if($rs_lcanp->fields["gridfield_tipo"]=='label')
	  {
	      echo '<div class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].' form-check"><div '.$rs_lcanp->fields["gridfield_extra"].'>';
		  echo '<label class="form-check-label" >&nbsp;'.$rs_lcanp->fields["gridfield_title"].'</label>'; 
		  echo '</div></div>';
	  }
	  ...
	  if($rs_lcanp->fields["gridfield_tipo"]=='select')
	  {
		  ...
		  $this->fill_cmb($rs_lcanp->fields["gridfield_tablecmb"],$rs_lcanp->fields["gridfield_camposcmb"],'',' where 1 '.$rs_lcanp->fields["gridfield_ordercmb"],$DB_gogess);
		  echo '</select>';
		  echo '</div>';
	  }
