	...
	for($i=0;$i<count($fie_camposgridselect);$i++)
	 {
		...
		$lista_camposz="select gridfield_nameid,gridfield_editarengrid,gridfield_tipo,gridfield_campoafectacheck,gridfield_valordefault,gridfield_orderecibe,gridfield_bloqueado from gogess_gridfield where fie_id=".$_POST["fie_id"]."  and gridfield_nameid='".$fie_camposgridselect[$i]."x"."' and gridfield_editarengrid=1";
		...
		if($rs_lcanpz->fields["gridfield_tipo"]=='textarea')
		 ...
		 if($rs_lcanpz->fields["gridfield_tipo"]=='select')
	     {
			 ...
			 if($rs_data->fields[$rs_lcanpz->fields["gridfield_bloqueado"]]==0) 
			 {
			   ...
			   echo '<td>';		   
			   echo '<select class="form-control" name="'.$fie_camposgridselect[$i].$rs_data->fields[$campo_id].$i.'" id="'.$fie_camposgridselect[$i].$rs_data->fields[$campo_id].$i.'"  '.$rs_lcanpz->fields["gridfield_extra"].' onblur="'.$link_guarda.'" >';
		       echo '<option value="" >--Seleccionar--</option>';		   
		       $this->fill_cmb($rs_lcanpz->fields["gridfield_tablecmb"],$rs_lcanpz->fields["gridfield_camposcmb"],'',' '.$rs_lcanpz->fields["gridfield_ordercmb"],$DB_gogess);
		       echo '</select>';
			   echo '</td>';
