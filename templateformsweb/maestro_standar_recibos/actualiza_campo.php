...
$saca_total="select ((docdet_cantidad*docdet_preciou)-docdet_descuento) as total_valor, (((docdet_cantidad*docdet_preciou)-docdet_descuento) * (docdet_porcentaje/100)) as valorimp from beko_recibodetalle where docdet_id=".$_POST["id_detallep"];
...
$split_nedita=explode(",",$rs_detalle->fields["subgri_campoedita"]);
 ...
 if($rs_totales)
 {
  ...
  $total_apagar=($suma_general-$descuento_general)+@$sutotales[2]["conimpuesto"]+@$sutotales[3]["conimpuesto"]+@$sutotales[4]["conimpuesto"];
...
if(@$sutotales[3]["total"])
 ...
 if(@$sutotales[2]["total"])
  ...
  {
     echo '<input name="subtotaliva1" type="hidden" id="subtotaliva1" value="'.number_format(@$sutotales[4]["total"], $objimpuestos->cgfe_decimales, '.', '').'">';
     $guardar_doccab_subtotaliva=number_format(@$sutotales[4]["total"], $objimpuestos->cgfe_decimales, '.', '');
  }
...
if(@$sutotales[4]["total"])
{
echo '<input name="iva1" type="hidden" id="iva1" value="'.number_format(@$sutotales[4]["conimpuesto"], $objimpuestos->cgfe_decimales, '.', '').'">';
$guardar_doccab_iva=number_format(@$sutotales[4]["conimpuesto"], $objimpuestos->cgfe_decimales, '.', '');
