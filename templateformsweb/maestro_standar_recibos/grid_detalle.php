...
$rs_preciot= $DB_gogess->executec($busca_preciot,array());
 ...
 if($rs_exixtenx->fields["doccab_estadosri"]=='AUTORIZADO')
	  ...
	  {
			 if($campo_val["fie_name"]=='docdet_descripcion')
			 {
			 echo '<td class="txt_titulo" valign="top" ><div id="'.$campo_val["fie_name"].'_'.$rs_data->fields[$rs_detalle->fields["subgri_campoidts"]].'" >';		
			 
			 echo '<textarea class="txt_cmb" name="val_mod_'.$campo_val["fie_name"]."-".$rs_data->fields[$rs_detalle->fields["subgri_campoidts"]].'" cols="40" rows="3" id="val_mod_'.$campo_val["fie_name"]."-".$rs_data->fields[$rs_detalle->fields["subgri_campoidts"]].'" '.$evento_data.'  >'.$rs_data->fields[$campo_val["fie_name"]].'</textarea>';
			 
			 echo $signo_data.'</div></td>';		 
