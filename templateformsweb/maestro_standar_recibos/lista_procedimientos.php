...
if($concatenado)
{
	//$lista_prductos="select * from efacsistema_producto where ".$concatenado." and prod_nivel=1 and prod_precio>0  order by prod_pedido desc";
...
else
{
	...
	//$lista_prductos="select * from efacsistema_producto where and prod_nivel=1 and prod_precio>0 order by prod_pedido desc limit 20";
	$lista_prductos="select * from efacsistema_producto inner join dns_gridaplicaen on efacsistema_producto.prod_enlace=dns_gridaplicaen.prod_enlace where grdifun_activo=1 and prod_nivel=1 and prod_precio>0 and tipp_id=3  order by prod_pedido desc limit 20";
...
?>
  ...
  <tr bgcolor="#EFF3F5">
   ...
   /// if($conve_descuento>0)
	///{
	//    $saca_descuentovalor=($conve_descuento*$rs_data->fields[$fuente_precio])/100;
	  //  $valor_precio=$rs_data->fields[$fuente_precio]-$saca_descuentovalor;
	//}
