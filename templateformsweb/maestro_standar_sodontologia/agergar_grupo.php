...
$rs_clientes = $DB_gogess->executec($lista_clientes,array());
 if($rs_clientes)
 {
	  ...
	  $busca_existe="select plantrai_id from dns_dispositivosodontologia where plantrai_codigo='".$rs_clientes->fields["cuadrobm_codigoatc"]."' and  subsecodont_enlace='".$_POST["subsecodont_enlace"]."'";
	  ...
	  if(!($rs_bexiste->fields["plantrai_id"]))
	  {
		 ...
		 $plantra_valorplanillax=$rs_clientes->fields["cuadrobm_preciodispositivo"]+$calcula_por+$calcula_iva;
		  ...
		  $inserta_valor="insert into dns_dispositivosodontologia (plantrai_codigo,plantrai_nombredispositivo,plantrai_precio,plantrai_cantidad,plantrai_indicaciones,plantrai_preciocompra,plantrai_porcentajeadm,plantrai_porcentajeiva,plantrai_valorplanilla,subsecodont_enlace,plantrai_fecharegistro,usua_id) values ('".$cuadrobm_codigoatc."','".$cuadrobm_nombredispositivo."','".$rs_clientes->fields["cuadrobm_preciodispositivo"]."','1','CONSULTA-".$_POST["grupo"]."','".$cuadrobm_preciodispositivo."','".$rs_dttarifario->fields["emp_valorgastosadm"]."','".$rs_dttarifario->fields["emp_valoriva"]."','".$plantra_valorplanillax."','".$_POST["subsecodont_enlace"]."','".date("Y-m-d H:i:s")."','".$_SESSION['datadarwin2679_sessid_inicio']."')";
