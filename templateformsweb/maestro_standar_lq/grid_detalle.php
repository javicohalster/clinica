...
$busca_exixtenciax="select doccab_estadosri from beko_lqcabecera where doccab_id='".$_POST["enlace"]."'";
...
$rs_preciot= $DB_gogess->executec($busca_preciot,array());
...

	   {
	      ...
	      $indice_encontro = array_search($campo_val["fie_name"],$split_nedita);
...

		   {
		      ...
		      $evento_data=" onChange=actualiza_campo('beko_lqdetalle','".$campo_val["fie_name"]."','docdet_id','".$rs_data->fields[$rs_detalle->fields["subgri_campoidts"]]."',$('#val_mod_".$campo_val["fie_name"]."-".$rs_data->fields[$rs_detalle->fields["subgri_campoidts"]]."').val()) ";
 ...
 $lista_descuentogeneral="select docdet_descuento_general from  beko_lqdetalle where ".$rs_detalle->fields["subgri_campoenlace"]."='".$_POST["enlace"]."' limit 1";
 ...
 $lista_totales="select sum(docdet_total) as total,beko_lqdetalle.tari_codigo,tari_valor from beko_lqdetalle inner join beko_tarifa on beko_lqdetalle.tari_codigo=beko_tarifa.tari_codigo where ".$rs_detalle->fields["subgri_campoenlace"]."='".$_POST["enlace"]."' group by tari_codigo";
...
//actualiza_campo('beko_lqdetalle','docdet_descuento','docdet_id','1',$('#val_mod_docdet_descuento-1').val())



  $evento_data_dg=" onChange=actualiza_descuentogeneral('beko_lqdetalle','docdet_descuento_general','doccab_id','".$_POST["enlace"]."',$('#descuento_g').val())  ";
...
$busca_exixtencia="select * from beko_lqcabecera where doccab_id='".$_POST["enlace"]."'";
...
/*$busca_existepago="select docfpag_id,docfpag_valor from beko_lqformapago where doccab_id='".$_POST["enlace"]."' and docfpag_valor>0";
$rs_existepago = $DB_gogess->executec($busca_existepago,array());
if(@$rs_existepago->fields["docfpag_valor"]!=$total_apagar)
{

     $busca_sidata="select docfpag_id,docfpag_valor from beko_lqformapago where frm_id=1 and doccab_id='".$_POST["enlace"]."'";
     $rs_bdata = $DB_gogess->executec($busca_sidata,array());
	 if($rs_bdata->fields["docfpag_id"])
	 {
	   $actualiza="update beko_lqformapago set docfpag_valor=".$total_apagar." where frm_id=1 and doccab_id='".$_POST["enlace"]."'";
	   $rs_actualiza = $DB_gogess->executec($actualiza,array());
	 }
	 else
	 {
	   $inserta="insert into beko_lqformapago (frm_id,doccab_id,docfpag_valor) values (1,'".$_POST["enlace"]."',".$total_apagar.")";
