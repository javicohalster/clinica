...
//busca serial
$busca_serial="select id as prod_codigo from api_productos where id='".$_POST["prod_id"]."'";
$rs_serial = $DB_gogess->executec($busca_serial,array());
//


$busca_agregado="select * from beko_lqdetalle where docdet_codprincipal='".$rs_serial->fields["prod_codigo"]."' and doccab_id='".$_POST["enlace"]."'";
...
$borra_data="delete from beko_lqdetalle where docdet_codprincipal='".$rs_serial->fields["prod_codigo"]."' and  doccab_id='".$_POST["enlace"]."'";
...
$tabla_r="select '".$_POST["enlace"]."' as doccab_id,id as prod_codigo,nombre as prod_nombre,'".$cantidad_nueva."' as docdet_cantidad,pvp1 as prod_precio,2 as impu_codigo,CASE 
    WHEN porcentaje_iva = '12' THEN '2'
    WHEN porcentaje_iva = '0' THEN '0'
	WHEN porcentaje_iva = '13' THEN '2'
	WHEN porcentaje_iva = '14' THEN '2'
    ELSE '0'
END as  tari_codigo, porcentaje_iva as tari_valor,(((".$cantidad_nueva."*(".$valor_precio."))*porcentaje_iva)/100) as docdet_valorimpuesto,(".$cantidad_nueva."*(".$valor_precio.")) as docdet_total,".$_SESSION['datadarwin2679_sessid_inicio']." as usua_id,'' as prod_enlace from api_productos where id='".$_POST["prod_id"]."'";
...
$busca_dataproducto=$tabla_r;


$rs_dataproducto = $DB_gogess->executec($busca_dataproducto,array());

$valor_precio_isnerta=$rs_dataproducto->fields["prod_precio"];
