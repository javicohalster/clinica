...
$titulos_fieldcampo=array();
$lista_campost="select * from gogess_gridfield where fie_id=".$this->fie_id." order by gridfield_orden asc";
$rs_lcanpt = $DB_gogess->executec($lista_campost,array());
 if($rs_lcanpt)
 {
	  while (!$rs_lcanpt->EOF) {
	  
	    $titulos_fieldcampo[$rs_lcanpt->fields["gridfield_nameid"]]=$rs_lcanpt->fields["gridfield_title"];
	...
	for($i=0;$i<count($campos_validaciongrid);$i++)
	 {
		 echo "		 
		  ...
		  {
		   var titulo_data='".$titulos_fieldcampo[$campos_validaciongrid[$i]."x"]."';
...
$("#lista_detalles_<?php echo $this->fie_id;  ?>").load("<?php echo $this->formulario_path; ?>/grid_standarmoney.php",{

enlace:enlacep,
idgrid:id_grid,
opcion:opcionp,
enlace:enlacep,
bloqueo_cierre:'<?php echo @$this->bloqueo_cierre ?>',
