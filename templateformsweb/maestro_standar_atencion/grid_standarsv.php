...
//echo $lista_servicios;
 ...
 if($rs_data)
 {
  ...
  ?>
  <tr>
    <td  onClick="grid_extras_<?php echo $_POST["fie_id"]; ?>('<?php echo $rs_data->fields[$campo_enlace]; ?>','<?php echo $rs_data->fields[$campo_id]; ?>',2)" style="cursor:pointer;padding:2px;" ><span class="glyphicon glyphicon-ban-circle"></span></td>
	 ...
	 {		
		...
		switch ($fie_camposgridselect[$i]) {
			case 'signovita_presionarterial':
                ...
                echo '<td style="font-size:10px;padding:2px;" >'.$rs_data->fields[$fie_camposgridselect[$i]].' '.$valor_txt.'</td>';
			...
			case 'signovita_masacorporal':
				{
				  ...
				  echo '<td style="font-size:10px;padding:2px;" >'.number_format($rs_data->fields[$fie_camposgridselect[$i]], 2, '.', '' ).' '.$berifica_estado.'</td>';
				}
				break;
