<?php
header('Content-Type: text/html; charset=UTF-8');
ini_set('display_errors',1);
error_reporting(E_ALL);
$tiempossss=140000;
ini_set("session.cookie_lifetime",$tiempossss);
ini_set("session.gc_maxlifetime",$tiempossss);
session_start();


$director='../../';
include("../../cfg/clases.php");
include("../../cfg/declaracion.php");
if($_SESSION['datadarwin2679_sessid_inicio'])
{
$objformulario= new  ValidacionesFormulario();
$comillasimple="'";

$clie_id=$_POST["pVar1"];
$atenc_id=$_POST["pVar2"];

$lista_atencion="select * from dns_atencion where atenc_id=?";
$rs_atencion = $DB_gogess->executec($lista_atencion,array($atenc_id));

//busca datos del paciente
$datos_cliente="select * from app_cliente where clie_id=".$clie_id;
$rs_dcliente = $DB_gogess->executec($datos_cliente,array());
//busca datos del paciente

$datos_usuario="select * from  app_usuario where usua_id=".$_SESSION['datadarwin2679_sessid_inicio'];
$rs_usuario = $DB_gogess->executec($datos_usuario,array());


$nciudad='';
$nciudad=$objformulario->replace_cmb("dns_centrosalud","centro_id,centro_nombre"," where centro_id =",$rs_usuario->fields["centro_id"],$DB_gogess);
?>

<script type="text/javascript">
<!--

function guardar_informe()
{



 if($('#val_situacion').val()=='')
  {

  alert("Debe llenar situacion actual...");
  return false;
  }

  if($('#val_observaciones').val()=='')
  {

  alert("Debe llenar observaciones...");
  return false;

  }
  
   if($('#infodeser_fechadesercion').val()=='')
  {

  alert("Debe llenar la fecha de desercion...");
  return false;

  }
  

    if($('#infodeser_fechaevaluacioninicial').val()=='')
  {

  alert("Debe llenar la fecha de evaluacion inicial...");
  return false;

  }
  
  
    if($('#infodeser_fechainiciopt').val()=='')
  {

  alert("Debe llenar la fecha de inicio...");
  return false;

  }
  


  $("#informe_g").load("templateformsweb/maestro_standar_atencion/guardar_desercion.php",{
  atenc_id:'<?php echo $atenc_id ?>',
  clie_id:'<?php echo $clie_id ?>',
  val_situacion:$('#val_situacion').val(),
  val_observaciones:$('#val_observaciones').val(),
  usua_id:'<?php echo $_SESSION['datadarwin2679_sessid_inicio']; ?>',
  val_diagnostico:$('#val_diagnostico').val(),
  infodeser_fechadesercion:$('#infodeser_fechadesercion').val(),
  infodeser_fechaevaluacioninicial:$('#infodeser_fechaevaluacioninicial').val(),
  infodeser_fechainiciopt:$('#infodeser_fechainiciopt').val()

  },function(result){  
 

  });  
  $("#informe_g").html("Espere un momento"); 

  

}

//  End -->
</script>
<style type="text/css">
<!--
.Estilo1 {font-size: 11px}
.Estilo2 {font-size: 11px; font-family: Verdana, Arial, Helvetica, sans-serif; }
.Estilo3 {font-family: Verdana, Arial, Helvetica, sans-serif}
.Estilo4 {font-size: 11px; font-family: Verdana, Arial, Helvetica, sans-serif; font-weight: bold; }
-->
</style>

<?php
$busca_datos="select * from faesa_informedesercion where atenc_id=".$atenc_id."  and clie_id=".$clie_id." and usua_id=".$_SESSION['datadarwin2679_sessid_inicio'];;
$rs_bdatos = $DB_gogess->executec($busca_datos,array());

?>

 <table width="100%" border="1" cellpadding="1" cellspacing="0">
    <tr>
      <td><div align="center" class="Estilo4">ESTABLECIMIENTO</div></td>
      <td><div align="center" class="Estilo4">NOMBRE</div></td>
      <td><div align="center" class="Estilo4">APELLIDO</div></td>
      <td><div align="center" class="Estilo4">SEXO (M-F) </div></td>
      <td><div align="center" class="Estilo4">No HISTORIA CLINICA </div></td>
    </tr>
    <tr>
      <td><span class="Estilo2">FAESA <?php echo $nciudad ?></span></td>
      <td><span class="texto_caja Estilo1 Estilo3">
        <?php  echo utf8_encode($rs_dcliente->fields["clie_nombre"]); ?>
      </span></td>
      <td><span class="texto_caja Estilo1 Estilo3">
        <?php  echo utf8_encode($rs_dcliente->fields["clie_apellido"]); ?>
      </span></td>
      <td> <span class="Estilo2">
      <?php  echo $rs_dcliente->fields["clie_genero"]; ?>
      </span></td>
      <td><span class="Estilo2">
      <?php  echo $rs_atencion->fields["atenc_hc"]; ?>
      </span></td>
    </tr>
</table>
<br><br>

<div class="form-group">
		<label class="control-label col-xs-2 Estilo4">Diagn&oacute;stico</label>
		<div class="col-xs-10 Estilo2">
		  <textarea name="val_diagnostico" cols="50" rows="6" id="val_diagnostico"><?php
		  echo trim(@$rs_bdatos->fields["infodeser_diagnostico"]);
		  ?></textarea>
        </div>
</div>


<div class="form-group">
		<label class="control-label col-xs-2 Estilo4">Situaci&oacute;n actual</label>
		<div class="col-xs-10 Estilo2">
		  <textarea name="val_situacion" cols="50" rows="6" id="val_situacion"><?php
		  echo trim(@$rs_bdatos->fields["infodeser_situacionactual"]);
		  ?></textarea>
        </div>
</div>

<div class="form-group">
		<label class="control-label col-xs-2 Estilo4">Observaciones</label>
		<div class="col-xs-10 Estilo2">
		  <textarea name="val_observaciones" cols="50" rows="6" id="val_observaciones"><?php
		  echo trim(@$rs_bdatos->fields["infodeser_observaciones"]);
		  ?></textarea>
        </div>
</div>

<div class="form-group">
		<label class="control-label col-xs-2 Estilo4">Fecha de Deserci&oacute;n</label>
		<div class="col-xs-10 Estilo2">
		  <?php
		  echo $objformulario->fecha_bloque("d-m-Y","infodeser_fechadesercion",trim(@$rs_bdatos->fields["infodeser_fechadesercion"]),'');
		  echo '<input name="infodeser_fechadesercion" id="infodeser_fechadesercion" type="hidden" value="'.trim(@$rs_bdatos->fields["infodeser_fechadesercion"]).'" >';
		  ?>
		  
        </div>
</div>
<p>&nbsp;</p>
<div class="form-group">
		<label class="control-label col-xs-2 Estilo4">Fecha de Evaluacion Inicial</label>
		<div class="col-xs-10 Estilo2">
		  <?php
		  echo $objformulario->fecha_bloque("d-m-Y","infodeser_fechaevaluacioninicial",trim(@$rs_bdatos->fields["infodeser_fechaevaluacioninicial"]),'');
		  echo '<input name="infodeser_fechaevaluacioninicial" id="infodeser_fechaevaluacioninicial" type="hidden" value="'.trim(@$rs_bdatos->fields["infodeser_fechaevaluacioninicial"]).'" >';
		  ?>
		  
        </div>
</div>
<p>&nbsp;</p>
<div class="form-group">
		<label class="control-label col-xs-2 Estilo4">Fecha de Inicio</label>
		<div class="col-xs-10 Estilo2">
		  <?php
		  echo $objformulario->fecha_bloque("d-m-Y","infodeser_fechainiciopt",trim(@$rs_bdatos->fields["infodeser_fechainiciopt"]),'');
		  echo '<input name="infodeser_fechainiciopt" id="infodeser_fechainiciopt" type="hidden" value="'.trim(@$rs_bdatos->fields["infodeser_fechainiciopt"]).'" >';
		  ?>
		  
        </div>
</div>



<br>
<div class="form-group">
<div class="col-xs-12">
<?php

if($rs_bdatos->fields["infodeser_id"])
{
  
  echo '<div class="Estilo2" >Registrado por: '.$objformulario->replace_cmb("app_usuario","usua_id,usua_nombre,usua_apellido"," where usua_id =",$rs_bdatos->fields["usua_id"],$DB_gogess)."<br>";
  echo 'Fecha resgistro: '.$rs_bdatos->fields["infodeser_fecharegistro"]."</div>";
}

?>
</div>
</div>
<br>

<div class="form-group">
<div class="col-xs-6">
<center><input type="button" name="Submit" value="Guardar Informe" onClick="guardar_informe()"></center>
</div>
<div class="col-xs-6">
<center><input type="button" name="Submit" value="Imprimir Informe" onClick="ver_informedeser(3,'<?php echo $atenc_id; ?>')"></center>
</div>
</div>


<script>
var config1 ={
  toolbarGroups: [
						{"name":"basicstyles","groups":["basicstyles"]},
						{"name":"paragraph","groups":["list","blocks"]},
						{"name":"styles","groups":["styles"]}
					],
					// Remove the redundant buttons from toolbar groups defined above.
					removeButtons: 'Strike,Subscript,Superscript,Anchor,Styles,Specialchar'
}



$('#val_diagnostico').ckeditor(config1);
$('#val_situacion').ckeditor(config1);	 
$('#val_observaciones').ckeditor(config1);	 

</script>
<p>&nbsp;</p>
<div id="informe_g"></div>

<?php
}			
else
{
echo '<div style="background-color: rgb(255, 238, 221);" id="msg" class="errors">Su sesi&oacute;n a caducado de precione F5 para continuar...</div>';
	
}

?>