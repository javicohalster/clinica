...
//----------------------------------
//$fie_tituloscamposgrid
//$campos_validaciongrid
$titulos_bloq=array();
for($ivals=0;$ivals<count($campos_dataedit);$ivals++)
{
  $titulos_bloq[$campos_dataedit[$ivals]]=$fie_tituloscamposgrid[$ivals];

}
//print_r($titulos_bloq);
//----------------------------------
...
if($('#atenc_id').val()=='')
{
 alert("Por favor guarde primero el registro principal para asignar una precuenta...");
 
 event.preventDefault();
 var new_position = jQuery('#un-guardar').offset();
	...
	for($i=0;$i<count($campos_validaciongrid);$i++)
	 {
		 echo "		 
		  ...
		  {
		   var titulo_data='".$titulos_bloq[$campos_validaciongrid[$i]]."';
...
$("#lista_detalles_<?php echo $this->fie_id;  ?>").load("<?php echo $this->formulario_path; ?>/grid_standarprecuenta.php",{
...
function edad_sexo_<?php echo $this->fie_id;  ?>()
{
   $("#edad_sexo_<?php echo $this->fie_id;  ?>").load("<?php echo $this->formulario_path; ?>edad_sexo.php",{
	clie_id:$('#clie_id').val()	
	 },function(result){      
	...
	$("#edad_sexo_<?php echo $this->fie_id;  ?>").html("Espere un momento...");
...
<div class="panel-body"><div style="color:#0000FF" id="edad_sexo_<?php echo $this->fie_id;  ?>"></div><b><span style="color:#0000FF"></span></b>
...
$rs_lcanp = $DB_gogess->executec($lista_campos,array());
 if($rs_lcanp)
 {
	  ...
	  {
		  $link_afecta=" onchange=afecta_listaespe_".$this->fie_id."('especipr_id','".$this->fie_id."',$('#".$rs_lcanp->fields["gridfield_nameid"]."').val(),''); ";				  
		  echo '<div class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].'"><label><b>'.$rs_lcanp->fields["gridfield_title"].'</b></label>';
...
function afecta_listaespe_<?php echo $this->fie_id;  ?>(esca_campoafecta,fie_id,valor,esca_campoenlace)
{

$("#recibe_"+esca_campoafecta+"x").load("<?php echo $this->formulario_path; ?>lista_dataespe.php",{
      fie_id:fie_id,
