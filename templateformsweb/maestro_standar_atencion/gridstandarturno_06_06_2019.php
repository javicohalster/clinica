...
$rs_lcanp = $DB_gogess->executec($lista_campos,array());
 if($rs_lcanp)
 {
	  ...
	  {
	      echo '<div class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].'"><label><b>'.utf8_encode($rs_lcanp->fields["gridfield_title"]).'</b></label>';
		  echo '<input placeholder="'.utf8_encode($rs_lcanp->fields["gridfield_title"]).'" name="'.$rs_lcanp->fields["gridfield_nameid"].'" type="text" id="'.$rs_lcanp->fields["gridfield_nameid"].'" class="form-control" value="'.date("Y-m-d").'" '.$rs_lcanp->fields["gridfield_extra"].' />';
		  echo '</div>';
	  }
