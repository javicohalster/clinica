...
<?php echo $campo_id; ?>x:id_grid
	...
	?>
	 ...
	 afecta_lista_5759('usuaat_id','5759',$('#especi_idx').val(),'');  
...
$("#lista_detalles_<?php echo $this->fie_id;  ?>").load("<?php echo $this->formulario_path; ?>/grid_standarturno.php",{

enlace:enlacep,
idgrid:id_grid,
opcion:opcionp,
enlace:enlacep,
clie_idx:$('#clie_id').val(),
...
sess_id:'<?php echo $_SESSION['datadarwin2679_sessid_inicio']; ?>'
    ...
    for($i=0;$i<count($campos_dataedit);$i++)
	 {
		if($campos_dataedit[$i]!='gridtur_fecha')
		{
...
{
  alert("Por favor para asignar un turno, guarde primero dando clic en guardar registro...");
}
...
function grid_extrasin_<?php echo $this->fie_id;  ?>(enlacep,id_grid,opcionp)
{
...
$rs_lcanp = $DB_gogess->executec($lista_campos,array());
 if($rs_lcanp)
 {
	  ...
	  //afecta
	  if($rs_lcanp->fields["gridfield_tipo"]=='selectafecta')
	  {
		  $link_afecta=" onchange=afecta_lista_".$this->fie_id."('usuaat_id','".$this->fie_id."',$('#".$rs_lcanp->fields["gridfield_nameid"]."').val(),''); ";				  
		  echo '<div class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].'"><label><b>'.$rs_lcanp->fields["gridfield_title"].'</b></label>';
		  echo '<select class="form-control" name="'.$rs_lcanp->fields["gridfield_nameid"].'" id="'.$rs_lcanp->fields["gridfield_nameid"].'"  '.$rs_lcanp->fields["gridfield_extra"].' '.$link_afecta.'  >';
		  echo '<option value="" >--Seleccionar--</option>';
		  $this->fill_cmb($rs_lcanp->fields["gridfield_tablecmb"],$rs_lcanp->fields["gridfield_camposcmb"],'',' '.$rs_lcanp->fields["gridfield_ordercmb"],$DB_gogess);
		  echo '</select>';
		  echo '</div>';
	  ...
	  //recibe
	  if($rs_lcanp->fields["gridfield_tipo"]=='selectrecibe')
	  {
		  echo '<div class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].'"><label><b>'.$rs_lcanp->fields["gridfield_title"].'</b></label>';
		  echo '<div id="recibe_'.$rs_lcanp->fields["gridfield_nameid"].'" >';
		  echo '<select class="form-control" name="'.$rs_lcanp->fields["gridfield_nameid"].'" id="'.$rs_lcanp->fields["gridfield_nameid"].'"  '.$rs_lcanp->fields["gridfield_extra"].' >';
		  echo '<option value="" >--Seleccionar--</option>';
		  $this->fill_cmb($rs_lcanp->fields["gridfield_tablecmb"],$rs_lcanp->fields["gridfield_camposcmb"],'',' '.$rs_lcanp->fields["gridfield_ordercmb"],$DB_gogess);
		  echo '</select>';
	   ...
	   if($rs_lcanp->fields["gridfield_tipo"]=='fecha')
	  {
	   ...
	   if($rs_lcanp->fields["gridfield_tipo"]=='fechahoy')
	  {
	      echo '<div class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].'"><label><b>'.$rs_lcanp->fields["gridfield_title"].'</b></label>';
		  echo '<input placeholder="'.$rs_lcanp->fields["gridfield_title"].'" name="'.$rs_lcanp->fields["gridfield_nameid"].'" type="text" id="'.$rs_lcanp->fields["gridfield_nameid"].'" class="form-control" value="'.date("Y-m-d").'" '.$rs_lcanp->fields["gridfield_extra"].' />';
		  echo '</div>';
	  }
...
{
?>
 grid_extrasin_<?php echo $this->fie_id;  ?>('<?php echo @$this->contenid[$campo_enlace]; ?>',0,0);
<?php
}
else
{
?>
 grid_extrasin_<?php echo $this->fie_id;  ?>('<?php echo @$this->sendvar[$campo_enlace."x"]; ?>',0,0);
<?php
}
?>
...
function afecta_lista_<?php echo $this->fie_id;  ?>(esca_campoafecta,fie_id,valor,esca_campoenlace)
{

$("#recibe_"+esca_campoafecta+"x").load("<?php echo $this->formulario_path; ?>lista_data.php",{
      fie_id:fie_id,
	  esca_campoafecta:esca_campoafecta,
	  valor:valor,
	  esca_campoenlace:esca_campoenlace
     },function(result){       
    	
    
      });  
    
$("#recibe_"+esca_campoafecta+"x").html("...");
