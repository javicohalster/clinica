...
$campos_datainserta=explode(",",$rs_fields->fields["fie_tablasubgridcampos"].",centro_id,clie_id");
...
if($_POST["opcion"]==2)
{
 $borra_reg="delete from ".$sub_tabla." where clie_id='".$clie_idx."' and ".$campo_id."=".$_POST["idgrid"];
...
if($_POST["opcion"]==1)
{
		...
		if($_POST[$campo_id."x"]>0)
			...
			else
			{
		     ...
		     //insertar turno
               $busca_siguiente="select gridtur_turno from pichinchahumana_extension.dns_gridturnos where especi_id=".$_POST["especi_idx"]." and gridtur_fecha='".date("Y-m-d")."' and centro_id='".$_SESSION['datadarwin2679_centro_id']."' and usuaat_id=".$_POST["usuaat_idx"]." order by gridtur_turno desc limit 1"; 
...
$lista_servicios="select * from ".$sub_tabla." where clie_id='".$clie_idx."' and gridtur_fecha='".date("Y-m-d")."' order by gridtur_fecha ,especi_id,gridtur_turno desc";
