...
//$saca_total="select ((docdet_cantidad*docdet_preciou)-docdet_descuento) as total_valor from beko_documentodetalle where doccab_id='".$_POST["id_detallep"]."'";
...
'<input name="total_linea" type="hidden" id="total_linea" value="'.number_format($rs_totallinea->fields["total_valor"], $objimpuestos->cgfe_decimales, '.', '').'">';
...
$total_apagar=$suma_general+@$sutotales[3]["conimpuesto"]-$descuento_gen;
...
$total_apagar=$suma_general+@$sutotales[2]["conimpuesto"]-$descuento_gen;
$guardar_doccab_subtotaliva=number_format(@$sutotales[2]["total"], $objimpuestos->cgfe_decimales, '.', '');
}

//echo $suma_general."<br>";
//echo @$sutotales[2]["conimpuesto"]."<br>";
//echo $descuento_gen;
