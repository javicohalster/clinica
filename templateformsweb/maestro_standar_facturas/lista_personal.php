...
if($_POST["prof_id"]==0)
{
$lista_servicios="select * from efacsistema_producto where prod_activo=1";
} 
 ...
 if($rs_data)
 {
	 ...
	 //ver precio convenio
	  ...
	  if($_POST["conve_id"]>0)
      {
	    $busca_valorconvenio="select * from pichinchahumana_extension.dns_gridconvenios conve inner join efacsistema_producto on conve.prod_enlace=efacsistema_producto.prod_enlace where prod_id='".$rs_data->fields["prod_id"]."' and gconve_convenio='".$_POST["conve_id"]."'";
	  ...
	  //ver precio convenio
       ?>
  <tr>
    <td onClick="agregar_servicio('<?php echo $rs_data->fields["usuat_id"]; ?>','<?php echo $rs_data->fields["prod_id"]; ?>','<?php echo $_POST["prof_id"]; ?>')" style="cursor:pointer" ><img src="images/bekosell.png"></td>
	<td><?php echo $cuenta; ?></td>
    <td><?php echo $rs_data->fields["usua_nombre"]." ".$rs_data->fields["usua_apellido"]; ?></td>
	<td><?php echo $rs_data->fields["prod_codigo"]; ?></td>  
	<td><?php echo $rs_dataespcialidad->fields["prof_nombre"]; ?>(<?php echo $rs_data->fields["prod_nombre"]; ?>)</td>    
	<td nowrap="nowrap">$ <?php echo $valor_precio; ?></td>     
  </tr>
  <?php
        $rs_data->MoveNext();
	  }
...
<!--
function agregar_servicio(idinsumo,prod_id,prof_id)
{

	
$("#agregar_servicio").load("templateformsweb/maestro_standar_facturas/agregar_servicio.php",{
usua_id:idinsumo,
prod_id:prod_id,
