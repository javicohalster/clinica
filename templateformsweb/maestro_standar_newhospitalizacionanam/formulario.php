...
if($csearch)
			{
			$busca_centroatencio="select * from dns_atencion where atenc_id='".$atenc_id."'";
			$rs_centrobu = $DB_gogess->executec($busca_centroatencio,array());
					...
					echo '<a href="javascript:abrir_protocolo();">';
					echo '<img src="archivo/gogess_data76963CKHA20210812.png"><span class="selected"></span></a></div>'; 					
					
					echo '<div class="col-sm-2">';
					echo '<a href="javascript:abrir_epicrisis();">';
					echo '<img src="archivo/gogess_data56077GIEQG20210812.png"><span class="selected"></span></a></div>'; 
					
					echo '<div class="col-sm-2">';
					echo '<a href="javascript:abrir_enfermeria();">';
					echo '<img src="archivo/gogess_data83934JUDT20181228.png"><span class="selected"></span></a></div>'; 
					//--------------------------------
...
<input type="button" name="btn_examenfisiconew_id" id="btn_examenfisiconew_id" value="EXAMEN FÍSICO"  onclick="ocultar_mostrar2('examenfisiconew_id')"  />
...
<div id="examenfisiconew_id">

<?php
$objformulario->generar_formulario_bootstrap_v2(@$submit,$table,26,$DB_gogess, $templateGridSectionAdditionals);
?>

<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td width="41%"><div id="mapa_lesione">
	


	</div></td>
    <td width="59%" valign="top">
	Seleccione una opci&oacute;n y de clic en agregar, en el dibujo aparece el n&uacute;mero que hace referencia a la opci&oacute;n seleccionada y con el mouse nueva el punto al lugar que dese en el gr&aacute;fico.
<?php
$objformulario->generar_formulario_bootstrap_v2(@$submit,$table,27,$DB_gogess, $templateGridSectionAdditionals);
?>
<center><input type="button" name="Submit" value="Agregar" onclick="agregar_lesiones()" style="width:150px" class="mb-sm btn btn-primary" /></center>
<br />
<div id="lista_lesione"></div>

<div id="agregar_lesione"></div>
...
$objformulario->generar_formulario_bootstrap(@$submit,$table,25,$DB_gogess);
?>
</div>

<?php
$objformulario->generar_formulario_bootstrap(@$submit,$table,28,$DB_gogess);
$objformulario->generar_formulario_bootstrap(@$submit,$table,29,$DB_gogess);
$objformulario->generar_formulario_bootstrap(@$submit,$table,30,$DB_gogess);
...
//busca cantidad de productos
   ...
   function ocultar_mostrar2(muestra)
  {
	...
	$('#examenfisiconew_id').hide();
	cambio_inactivo('examenfisiconew_id',0);
...
////////////////
function lista_lessiones()
  {
	  $("#lista_lesione").load("templateformsweb/maestro_standar_newhospitalizacionanam/lista_lesiones.php",{
	   anam_enlace:$('#anam_enlace').val()
	  ...
	  $("#lista_lesione").html("Espere un momento...");  
  
  }	
  
  lista_lessiones();
  
  function agregar_lesiones()
  {
  
      $("#agregar_lesione").load("templateformsweb/maestro_standar_newhospitalizacionanam/agregar_lesiones.php",{
	   anam_enlace:$('#anam_enlace').val(),
	   les_id:$('#les_id').val()
	  ...
	  $("#agregar_lesione").html("Espere un momento..."); 
  
  }
  
   
  function borrar_lesiones(less_id)
  {
  
      $("#agregar_lesione").load("templateformsweb/maestro_standar_newhospitalizacionanam/borrar_lesiones.php",{
	   anam_enlace:$('#anam_enlace').val(),
	   less_id:less_id
  ...
  function despliega_imagen()
  {
  
  
   $("#mapa_lesione").load("templateformsweb/maestro_standar_newhospitalizacionanam/mapa_lesiones.php",{
	   anam_enlace:$('#anam_enlace').val()
	  },function(result){  
	
	   
	  });
	  $("#mapa_lesione").html("Espere un momento..."); 
