...
<!--
 ...
 #contenedor_imp {
	...
	left:0px;
	position: relative;
	background-image:url(templateformsweb/maestro_standar_newhospitalizacionanam/images/diagramatopografico.png);
}


<?php
$listacamposcss="select * from dns_registrolesioneshospi where anam_enlace='".$_POST["anam_enlace"]."'"; 
...
$listacamposcss="select * from dns_registrolesioneshospi inner join pichinchahumana_extension.dns_lesioneshospi on dns_registrolesioneshospi.les_id=pichinchahumana_extension.dns_lesioneshospi.les_id where anam_enlace='".$_POST["anam_enlace"]."'"; 
 $rs_camposcss = $DB_gogess->executec($listacamposcss,array()); 
   if($rs_camposcss)
   {
			 while (!$rs_camposcss->EOF) {
              
			    echo '<div id=L'.strtolower($rs_camposcss->fields["less_id"]).'_div ><img src="templateformsweb/maestro_standar_newhospitalizacionanam/images/'.$rs_camposcss->fields["les_id"].'.png" width="13" height="13" /></div>';
...
function guardar_posicion(idcampo,x,y)
{
  ...
  $("#actualizando_div").load("templateformsweb/maestro_standar_newhospitalizacionanam/actualizar_xy.php",{
    pidcampo:idcampo,
	px:x,
	py:y,
