...
if (@$_SESSION['datadarwin2679_sessid_inicio']) {
  $crb_id = $_POST["crb_id"];
  $busca_compra = $_POST["busca_compra"];
  $usua_id = @$_SESSION['datadarwin2679_sessid_inicio'];
  $cliev_id = $_POST["cliev_id"];
  ...
  if ($cliev_id) {
    $sql1 = " proveevar_id = '" . trim($cliev_id) . "' and ";
  }

  if ($busca_compra) {
    $sql2 = " compra_nfactura = '" . trim($busca_compra) . "' and ";
  }

  $concatena_sql = $sql1 . $sql2;
  $concatena_sql = substr($concatena_sql, 0, -4);

  //$busca_compras = "select * from dns_compras_vista where (compra_nfactura like '%" . $busca_compra . "%' or proveevar_id like '%" . $busca_compra . "%' or provee_ruc like '%" . $busca_compra . "%' or provee_nombre  like '%" . $busca_compra . "%') and  ( saldo>0 and compra_id not in (select compracp_id from lpin_masivocobropagodetalle where compracp_id>0 and crb_enlace='" . $crb_enlace . "')) and tipdoc_id in (1,2,3) order by compra_fecha asc";

  $busca_compras = '';
  if ($concatena_sql) {
    $busca_compras = "select * from dns_compras_vista where " . $concatena_sql . " and  ( saldo>0 and compra_id not in (select compracp_id from lpin_masivocobropagodetalle where compracp_id>0 and crb_enlace='" . $crb_enlace . "')) and tipdoc_id in (1,2,3) order by compra_fecha asc";
  }

  //echo  $busca_compras;

  $rs_listadiariox = $DB_gogess->executec($busca_compras, array());
  if ($rs_listadiariox) {
?>
    <div>
      ...
      <table width="100%" border="1" cellspacing="0" cellpadding="0">
        ...
        ?>
          <tr>
            ...
            <td>
              <input name="check_asig[]" type="checkbox" id="check_asig" value="<?php echo  $rs_listadiariox->fields["compra_id"]; ?>">
            </td>
            <td nowrap><?php echo $rs_listadiariox->fields["compra_fecha"]; ?></td>
            <td nowrap><?php echo $rs_listadiariox->fields["compra_nfactura"]; ?></td>

            <td>
              <div align="center">
			  <?php echo $rs_listadiariox->fields["compra_descripcion"]; ?>
                <input name="valor_obs_<?php echo $rs_listadiariox->fields["compra_id"]; ?>" type="hidden" id="valor_obs_<?php echo $rs_listadiariox->fields["compra_id"]; ?>" value="" size="15">
              </div>
            </td>


            <td><?php echo $rs_listadiariox->fields["tipdoc_nombre"]; ?></td>
            <td><?php echo $rs_listadiariox->fields["provee_ruc"] . " / " . $rs_listadiariox->fields["provee_cedula"]; ?></td>
            <td><?php echo $rs_listadiariox->fields["provee_nombre"]; ?></td>
            <td><?php echo $rs_listadiariox->fields["saldo"]; ?></td>
            <td>
              <div align="center">
                <input name="valor_data_<?php echo $rs_listadiariox->fields["compra_id"]; ?>" type="text" id="valor_data_<?php echo $rs_listadiariox->fields["compra_id"]; ?>" value="<?php echo $rs_listadiariox->fields["saldo"]; ?>" size="10">
              </div>
            </td>
          </tr>
        <?php
    ...
    <script type="text/javascript">
      ...
      function enviar_registros() {
        ...
        $("#div_procesaentrega").load("templateformsweb/maestro_standar_macobropagopl/listasvc/asig_compras.php", {
