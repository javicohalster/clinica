...
if($centro_id==1)
{
$stockactualx="select sum(stock_cantidad*stock_signo) as stactual from dns_principalstockactual where centro_id='55' and cuadrobm_id='".$cuadrobm_id."'";
$rs_stactuax = $DB_gogess->executec($stockactualx);

}
else
{
$stockactualx="select sum(stock_cantidad*stock_signo) as stactual from dns_stockactual where centro_id='".$centro_id."' and cuadrobm_id='".$cuadrobm_id."'";
$rs_stactuax = $DB_gogess->executec($stockactualx);

}

$verifica_acstock=$rs_stactuax->fields["stactual"];


if($ajuspr_creal==$verifica_acstock)
{
 $ajuspr_diferencia=0;
 echo "Ya porcesado:".$cuadrobm_id." verifica=".$verifica_acstock." -> Real".$ajuspr_creal."<br>";
...
$busca_movimientoult="select * from dns_principalmovimientoinventario where cuadrobm_id='".$cuadrobm_id."' and tipom_id='1' and centro_id='".$centro_id."' order by moviin_id desc limit 1";
...
$file = fopen("log/e_asiento".date("Y-m-d")."_".@$_SESSION['datadarwin2679_centro_id'].".txt", "a+");
fwrite($file,$_SESSION['datadarwin2679_sessid_inicio']."-->".$inserta_datas."-->".date("Y-m-d H:i:s"). PHP_EOL);
fclose($file);
...
$busca_paraentrega="select * from dns_compras RIGHT join dns_principalmovimientoinventario on dns_compras.compra_id=dns_principalmovimientoinventario.compra_id where dns_principalmovimientoinventario.tipom_id=1  and cuadrobm_id='".$cuadrobm_id."' order by moviin_fechadecaducidad asc";
  ...
  if($rs_paraentrega)
   {
	  ...
	  $moviin_redpublica=$rs_paraentrega->fields["moviin_redpublica"];
	  $moviin_laboratorio=$rs_paraentrega->fields["moviin_laboratorio"];
	  ...
	  //moviintranscent_id
						...
						$lista_ent="select sum(moviin_totalenunidadconsumo) as entregadot from  dns_principalmovimientoinventario where (moviintranscent_id='".$rs_paraentrega->fields["moviin_id"]."') and centro_id='".$centro_id."' and tipom_id=2 and tomfis_id>0 and cuadrobm_id='".$cuadrobm_id."'";
						
						$rs_ent = $DB_gogess->executec($lista_ent);
						$entregadot=0;
						$entregadot=$rs_ent->fields["entregadot"];
						
						$lista_ent2="select sum(moviin_totalenunidadconsumo) as entregadot from  dns_principalmovimientoinventario where (moviintranscent_id='".$rs_paraentrega->fields["moviin_id"]."') and centro_id='".$centro_id."' and tipom_id=2 and tomfis_id=0 and cuadrobm_id='".$cuadrobm_id."'";
						
						$rs_ent2 = $DB_gogess->executec($lista_ent2);
						$entregadot2=0;
						$entregadot2=$rs_ent2->fields["entregadot"];						
						
		//busca_movimientos_fuera
		
		//busca lo ingresado en comprobante
						
						$busca_asig="select sum(cantidad_val) as totalegreso from dns_temporaldespacho inner join dns_egresocentros on dns_temporaldespacho.egrec_id=dns_egresocentros.egrec_id where moviin_id='".$rs_paraentrega->fields["moviin_id"]."' and egrec_anulado=0 and  egrec_procesado=0";
						...
						$valor_movimiento=0;
                        $valor_movimiento=$rs_paraentrega->fields["moviin_totalenunidadconsumo"];
						
						$restante_valor=$valor_movimiento-$cantidad_asig-$entregadot-$entregadot2;
		
          // echo "Dataor:".$restante_valor."<br>"; 		

       	 //busca lo ingresado en comprobante
		 
	  
	  $actual_porlote=$restante_valor;
	 
	 //echo $actual_porlote."<br>";
	  ...
	  if($actual_porlote>0)
	  {
		...
		//echo "<br>".$actual_porlote."->".$moviin_idvalor."<br>";
...
$file = fopen("log/e_asiento".date("Y-m-d")."_".@$_SESSION['datadarwin2679_centro_id'].".txt", "a+");
fwrite($file,$_SESSION['datadarwin2679_sessid_inicio']."-->".$inserta_movimiento."-->".date("Y-m-d H:i:s"). PHP_EOL);
fclose($file);
