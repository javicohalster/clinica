...
<?php
			...
			$datos_examenfisicoregional="select tb.gexamfis_observacion as Resumen_Clinico FROM pichinchahumana_original.dns_traumatologiaanamesis aet LEFT JOIN pichinchahumana_extension.dns_traumatologiagridexamenfisico tb on aet.anam_enlace=tb.anam_enlace where aet.clie_id=".$clie_id." and tb.tiporg_nombre='CP'";
...
{
     $busca_diag="select count(*) as total from dns_diagnosticoimagensolicitud where imgag_enlace='".$objformulario->contenid["imgag_enlace"]."'";
...
{
     $busca_diag="select count(*) as total from dns_diagnosticoimagensolicitud where imgag_enlace='".$objformulario->sendvar["imgag_enlacex"]."'";
     $rs_diag = $DB_gogess->executec($busca_diag,array());
