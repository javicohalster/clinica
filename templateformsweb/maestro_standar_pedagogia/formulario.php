...
<script>
         ...
         $(function() {
            ...
            $( "#pedago_cie" ).autocomplete({

               source: "templateformsweb/maestro_standar_pedagogia/searchcie.php",

               minLength: 1,

			   select: function( event, ui ) {

				  $('#pedago_cie_visorcie').val(ui.item.descripcion);
...
function pedago_cie_agregar() {

var datos;

datos=$('#pedago_impresiondiagnostica').val() + $('#pedago_cie_visorcie').val()+ ",";

$('#pedago_impresiondiagnostica').val(datos);
...
$('#pedago_testcomplementario').ckeditor(config1);
$('#pedago_impresiondiagnostica').ckeditor(config1);
$('#pedago_recomterapeutica').ckeditor(config1);	 
$('#pedago_recomfamiliares').ckeditor(config1);	 
$('#pedago_recomescolares').ckeditor(config1);	
$('#pedago_recommultidiciplinarias').ckeditor(config1);
