...
if($tabla_combo)
{
$lista_servicios="select * from ".$sub_tabla." inner join  ".$tabla_combo." on ".$sub_tabla.".".$idtabla_combo."=".$tabla_combo.".".$idtabla_combo." where ".$campo_enlace."='".$_POST['enlace']."' order by areev_orden asc ";
}
else
{
 ...
 if($rs_data)
 {
  ...
  <tr>
	...
	for($i=0;$i<count($fie_camposgridselect);$i++)
	 {
		 if($fie_camposgridselect[$i]!='areev_nombre')
		 {
		 //echo '<td><input name="val_datos'.$rs_data->fields[$campo_id].'" type="text" id="val_datos'.$rs_data->fields[$campo_id].'" value="'.$rs_data->fields[$fie_camposgridselect[$i]].'" /></td>';
		 ...
		 echo '<td><textarea onblur="'.$link_guarda.'" class="form-control" name="'.$fie_camposgridselect[$i].$rs_data->fields[$campo_id].$i.'" cols="30" rows="3" id="'.$fie_camposgridselect[$i].$rs_data->fields[$campo_id].$i.'">'.$rs_data->fields[$fie_camposgridselect[$i]].'</textarea></td>';
	...
	?>
	<td><input type="button" name="Submit" value="Guardar fila"  /></td>
  </tr>
  <?php
   $rs_data->MoveNext();	   
	  }
