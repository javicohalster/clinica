...
<table width="800" border="1" align="center" cellpadding="0" cellspacing="2">
  ...
  <tr>
	...
	?>
<input name="<?php echo $objformulario->tab_campoprimario; ?>" type="hidden" id="<?php echo $objformulario->tab_campoprimario; ?>" value="<?php echo $objformulario->contenid[$objformulario->tab_campoprimario] ?>" /></td>
  </tr>
</table>
<p>&nbsp;</p>
				...
				if($rs_buscag)
                   {
					  ...
					  if($rs_bbcampos)
					  {
									 ...
									 else
									 {
										...
										else
										{
											   if($rs_bbcampos->fields["fie_typeweb"]=='checkbox_peke')
											   {
													...
													{
													$rmp='<img src="images/visto_dns.png" width="20" height="18" />';
													}
													else
													{
													$rmp='';
						   ...
						   else
						   {
								  ...
								  {
								    $saca_nom=array();
									$saca_nom=explode(".",$rs_bbcampos->fields["fie_archivogrid"]);
								    include("../../".$template_reemplazo."".$saca_nom[0]."_block.php");
								  }
						          else
								  {
