...
<?php echo $campo_id; ?>x:id_grid
	...
	$('#<?php echo $campo_id; ?>x<?php echo $this->subtabla; ?>').val($('#<?php echo $campo_id; ?>xval').val());
	<?php
	for($i=0;$i<count($campos_dataedit);$i++)
	 {
		 echo "$('#".$campos_dataedit[$i]."x".$this->subtabla."').val($('#".$campos_dataedit[$i]."xval').val());";
	 }
	?>
...
//validaciones
   <?php
	for($i=0;$i<count($campos_validaciongrid);$i++)
	 {
		 echo "		 
		  if($('#".$campos_validaciongrid[$i]."x".$this->subtabla."').val()=='')
		  {
...
$("#lista_detalles_<?php echo $this->fie_id;  ?>").load("<?php echo $this->formulario_path; ?>/grid_standardiagnostico.php",{

enlace:enlacep,
idgrid:id_grid,
opcion:opcionp,
enlace:enlacep,
<?php echo $campo_id; ?>x:$('#<?php echo $campo_id; ?>x<?php echo $this->subtabla; ?>').val(),
<?php
for($i=0;$i<count($campos_dataedit);$i++)
	 {
	    echo $campos_dataedit[$i]."x:$('#".$campos_dataedit[$i]."x".$this->subtabla."').val(),
		";
	 }
?>
fie_id:'<?php echo $this->fie_id;  ?>',
sess_id:'<?php echo $_SESSION['datadarwin2679_sessid_inicio']; ?>',
subtabla:'<?php echo $this->subtabla; ?>'
 },function(result){       
	<?php
	echo " $('#".$campo_id."x".$this->subtabla."').val(''); 
	";
    for($i=0;$i<count($campos_dataedit);$i++)
	 {
		echo " $('#".$campos_dataedit[$i]."x".$this->subtabla."').val(''); 
		";
	 }
     ?>	
...
$rs_lcanp = $DB_gogess->executec($lista_campos,array());
 if($rs_lcanp)
 {
	  ...
	  if($rs_lcanp->fields["gridfield_tipo"]=='select')
	  {
		  ...
		  echo '<select class="form-control" name="'.$rs_lcanp->fields["gridfield_nameid"].$this->subtabla.'" id="'.$rs_lcanp->fields["gridfield_nameid"].$this->subtabla.'"  '.$rs_lcanp->fields["gridfield_extra"].' >';
	  ...
	  {
	      echo '<div class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].'"><label><b>'.$rs_lcanp->fields["gridfield_title"].'</b></label>';
		  echo '<input placeholder="'.$rs_lcanp->fields["gridfield_title"].'" name="'.$rs_lcanp->fields["gridfield_nameid"].$this->subtabla.'" type="text" id="'.$rs_lcanp->fields["gridfield_nameid"].$this->subtabla.'" class="form-control" value="" '.$rs_lcanp->fields["gridfield_extra"].' />';
		  echo '</div>';
	  }
	  ...
	  {
	      echo '<div class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].'"><label><b>'.$rs_lcanp->fields["gridfield_title"].'</b></label>';
		  echo '<textarea placeholder="'.$rs_lcanp->fields["gridfield_title"].'" name="'.$rs_lcanp->fields["gridfield_nameid"].$this->subtabla.'" id="'.$rs_lcanp->fields["gridfield_nameid"].$this->subtabla.'"  class="form-control"  '.$rs_lcanp->fields["gridfield_extra"].' ></textarea>';
		  echo '</div>';
	  }
	  ...
	  {  
		  echo '<input name="'.$rs_lcanp->fields["gridfield_nameid"].$this->subtabla.'" type="hidden" id="'.$rs_lcanp->fields["gridfield_nameid"].$this->subtabla.'" value="" '.$rs_lcanp->fields["gridfield_extra"].' />';
	  ...
	  {
	      echo '<div class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].'"><label><b>'.$rs_lcanp->fields["gridfield_title"].'</b></label>';
		  echo '<input placeholder="'.$rs_lcanp->fields["gridfield_title"].'" name="'.$rs_lcanp->fields["gridfield_nameid"].$this->subtabla.'" id="'.$rs_lcanp->fields["gridfield_nameid"].$this->subtabla.'" class="form-control timepicker" value="" type="text" '.$rs_lcanp->fields["gridfield_extra"].' />';
		  echo '</div>';
...
?>
<input name="<?php echo $campo_id; ?>x<?php echo $this->subtabla; ?>" type="hidden" id="<?php echo $campo_id; ?>x<?php echo $this->subtabla; ?>" value="0" />  
<?php
}
?>
</div>
...
$rs_lcanp = $DB_gogess->executec($lista_campos,array());
 if($rs_lcanp)
 {
	  while (!$rs_lcanp->EOF) {
		 ...
		 echo "$('#".$rs_lcanp->fields["gridfield_nameid"].$this->subtabla."').datepicker({dateFormat: 'yy-mm-dd'});
		 " ;
...
$rs_lcanp = $DB_gogess->executec($lista_campos,array());
 if($rs_lcanp)
 {
	  while (!$rs_lcanp->EOF) {
		 ...
		 echo "$('#".$rs_lcanp->fields["gridfield_nameid"].$this->subtabla."').wickedpicker({twentyFour: true});
		 " ;
