...
//plantilla receta
$busca_receta="select * from dns_formatoreceta where forrect_id=2";
...
$lee_grafico1='<img src="/pichinchahumana/archivo/'.$rs_receta->fields["forrect_grafico1"].'" />';
$lee_grafico2='<img src="/pichinchahumana/archivo/'.$rs_receta->fields["forrect_grafico2"].'" />';
$lee_grafico3='<img src="/pichinchahumana/archivo/'.$rs_receta->fields["forrect_grafico3"].'" />';
$lee_grafico4='<img src="/pichinchahumana/archivo/'.$rs_receta->fields["forrect_grafico4"].'" />';
...
$lee_plantilla=str_replace("-fecha-",date("Y-m-d"),$lee_plantilla);
...
$fie_tituloscamposgrid=explode(",",str_replace($quitar_t,"",$rs_fields->fields["fie_tituloscamposgrid"]));

//lista
$fie_camposgridselect=array();
$fie_camposgridselect=explode(",",str_replace($quitar_lista,"",$rs_fields->fields["fie_camposgridselect"]));
...
for($i=0;$i<count($fie_tituloscamposgrid);$i++)
	{
      $tbl_listareceta.='<td bgcolor="#DFE9EE" style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:7px" >'.utf8_encode($fie_tituloscamposgrid[$i]).'</td>';
	}
$tbl_listareceta.='<td bgcolor="#DFE9EE" style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:7px" >FIRMA</td>';	
$tbl_listareceta.='</tr>';

$cuenta=0;
$lista_servicios="select * from ".$sub_tabla." where ".$campo_enlace."='".$_GET['enlace']."' and (regterapia_fecha >= '".$_GET["fechai"]."' and  regterapia_fecha <= '".$_GET["fechaf"]."')";
		...
		for($i=0;$i<count($fie_camposgridselect);$i++)
		 {
			 ...
			 switch (trim($rs_dataxc->fields["gridfield_tipo"])) {
					...
					default:
					   {
						  $tbl_listareceta.='<td style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:8px" >'.utf8_decode($rs_data->fields[$fie_camposgridselect[$i]]).'</td>';
						}
				}
			 
		 }

   $tbl_listareceta.='<td style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:8px" height="40" ><br><br></td>';		  
   $tbl_listareceta.='</tr>';
   $rs_data->MoveNext();	   
	  }
  }
