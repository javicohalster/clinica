...
$rs_ingresodta = $DB_gogess->executec($busca_ingresosdata,array());
 ...
 if($rs_ingresodta)
 {
	  while (!$rs_ingresodta->EOF) {
		...
		$recomendaciones_rt.=$rs_ingresodta->fields["infevolucion_rterapeuticas"];
		$recomendaciones_rf.=$rs_ingresodta->fields["infevolucion_rfamiliares"];
		$recomendaciones_re.=$rs_ingresodta->fields["infevolucion_rescolares"];
		$recomendaciones_rm.=$rs_ingresodta->fields["infevolucion_rmultidiciplinarias"];
...
$lee_plantilla=str_replace("-diagnosticoi-",'<div align="justify" >'.$diagnositcoinicial.'</div>',$lee_plantilla);
$lee_plantilla=str_replace("-situacionac-",'<div align="justify" >'.$situacion_ac.'</div>',$lee_plantilla);

$recomendaciones_concatenada='';

if($recomendaciones_rt)
{
$recomendaciones_concatenada.="<b>Terap&eacute;uticas</b>".$recomendaciones_rt;
}

if($recomendaciones_rf)
{
$recomendaciones_concatenada.="<b>Familiares</b>".$recomendaciones_rf;
}

if($recomendaciones_re)
{
$recomendaciones_concatenada.="<b>Escolares</b>".$recomendaciones_re;
}

if($recomendaciones_rm)
{
$recomendaciones_concatenada.="<b>Multidisciplinarias</b>".$recomendaciones_rm;
}


$lee_plantilla=str_replace("-recomendacionesac-",$recomendaciones_concatenada,$lee_plantilla);
...
if($_GET["dhoja"]==1)
{
  $pie_responsable.='<p class="saltodepagina" />';
}
  $pie_responsable.=utf8_encode("<p><strong>RESPONSABLES </strong> </p><p></p><p>&nbsp;</p><div align='center'>".$responsables_cuadro."</div>");
