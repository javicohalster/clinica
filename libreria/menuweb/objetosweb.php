      ...
      echo "<div id='slider1' class='contentslide'><div class='opacitylayer'>";
  		while($row = mysql_fetch_array($resultado)) 
			{	
				...
				{
				   //articulos
				   $selecTabla1="select * from sysc_contenido where con_id=".$sli_articulo." order by con_id desc";    
				...
				else
				{
				  ...
				  {				  
				   //aplicacion
				   $selecTabla2="select * from sysc_aplication where ap_id=".$sli_apl;    
      ...
      echo "<div id='slider1' class='contentslide'><div class='opacitylayer'>";
  		while($row = mysql_fetch_array($resultado)) 
			{	
				...
				{
				   //articulos
				   $selecTabla1="select * from sysc_contenido where con_id=".$sli_articulo;				
				   $resultado1 = mysql_query($selecTabla1);
							while($row1 = mysql_fetch_array($resultado1)) 
								{	
								echo "<div class='contentdiv'>";								
...
function mp3playerhome($system)
{
  ...
  $sql = "SELECT * FROM sysc_sonido WHERE ((('$fecha_hoy' >= son_fechai) && ('$fecha_hoy' <= son_fechaf))) and sys_id=$system";  
    ...
    if ($seccion)
	{
    	  ...
    	  while($row = mysql_fetch_array($result))
	    	{  
			   ...
			   {
			     ////////////////////////////////////////////////////////////////
				   $selecTabla1="select * from sysc_contenido where con_id=".$row["hom_articulo"];				
				   $resultado1 = mysql_query($selecTabla1);
...
function desplegarcarrucel($nregistros,$borde,$cellpadding,$cellspacing,$columnas,$ancho,$alto,$system,$sessid,$DB_gogess)
{
	if(!($nregistros))
	{
  	...
  	//$buscalista="select * from sysc_fotoscontenido where fto_activo=1 limit ".$nregistros;
	$buscalista="select * from sysc_fotoscontenido where fto_activo=1";
...
//-->
	 ...
	 if($zona)
	 {
	    $listanoticas="select * from sysc_bloquenoticias where blnot_zona=".$zona." limit 1";
		...
		if($resultnot)
	    {
		   while($rownot = mysql_fetch_array($resultnot)) 
			{
		        $sacaarticulost="select * from sysc_seccp,sysc_contenido where sysc_seccp.secp_id=sysc_contenido.secp_id and sysc_seccp.secp_id=".$rownot["secp_id"];
				...
				$sacaarticulos="select * from sysc_seccp,sysc_contenido where sysc_seccp.secp_id=sysc_contenido.secp_id and sysc_seccp.secp_id=".$rownot["secp_id"]." order by con_id desc limit ".$inicioreg.",".$finreg;
...
//-->
	 ...
	 if($zona)
	 {
	    $listanoticas="select * from sysc_seccp where secp_id=".$zona."";
