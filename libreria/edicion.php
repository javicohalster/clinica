    ...
    var options = {
		...
		eval(result);
		   ...
		   ?>
			...
			{
			     if($objtableform->tab_historialmedico==1)
				 {
				 ?>
...
function tiempo_bloqueasigna(dia,mes,anio,campo)
{
    var fechallega;
    fechallega=$('#'+anio).val()+"-"+$('#'+mes).val();
    $('#'+campo).val(fechallega);

}


function fecha_bloqueasignaedad(dia,mes,anio,campo)
{
    var fechallega;
    fechallega=$('#'+anio).val()+"-"+$('#'+mes).val()+"-"+$('#'+dia).val();
	if(fechallega!='00000-00-00')
	{
    $('#'+campo).val(fechallega);   
    generar_edad(fechallega);
	}
}
