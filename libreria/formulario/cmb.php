...
function busca_conarbolcmp($cuenta,$DB_gogess)
{
...
function fill_cmbcuenta($tablecmb,$fieldcmb,$vbus,$orden,$DB_gogess)
{
...
$rs_listadiariox = $DB_gogess->executec($listadiariox,array());
 if($rs_listadiariox)
 {
     while (!$rs_listadiariox->EOF) {	    
		$cantidadd_valord=0;
	    $cantidadd_valord=$this->busca_conarbolcmp($rs_listadiariox->fields["planc_codigoc"],$DB_gogess);		
...
if(@$this->campoejecuta==@$this->ncamponombre)
 {
 $orden=str_replace(@$this->remplazarpor,@$this->codigofiltro,$orden);
 $selecTabla="select distinct ".$fieldcmb." from ".$tablecmb." ".$consulta_valor.$orden;
 }
 else
 {
 $selecTabla="select distinct ".$fieldcmb." from ".$tablecmb." ".$consulta_valor.$orden;
