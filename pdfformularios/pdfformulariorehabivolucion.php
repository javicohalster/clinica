...
for($i=0;$i<$numero_entero;$i++)
{
   ...
   $url="plantillas/evoform002reversorehabilitacion.php";
   ...
   if($rs_aevolucion)
	{
		while (!$rs_aevolucion->EOF) {
		  ...
		  $tipo_terapia='';
		  $tipo_terapia=$objformulario->replace_cmb("pichinchahumana_extension.dns_tipoterapia","tipotera_id,tipotera_nombre","where tipotera_id=",$rs_aevolucion->fields["tipotera_id"],$DB_gogess); 	  
		  ...
		  $lee_plantilla=str_replace("-notas".$cuenta_val."-","<b>".$tipo_terapia."</b>: ".$rs_aevolucion->fields["conext_notasdeevolucion"],$lee_plantilla);
