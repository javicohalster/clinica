...
$nomb_medico=$objformulario->replace_cmb("app_usuario","usua_id,usua_nombre,usua_apellido","where usua_id=",$rs_sihaydata->fields["usua_id"],$DB_gogess);
  ...
  if($t_div1>=0)
  {	   
  $t_div1=number_format($t_div1, 2, '.', '');
  $lee_plantilla=str_replace("-total1-",$t_div1,$lee_plantilla);
  }
  else
  {
  $lee_plantilla=str_replace("-total1-",'',$lee_plantilla);
  }

  $t_div2=$cal/$cal_t;  
  if($t_div2>=0)
  {
  $t_div2=number_format($t_div2, 2, '.', '');
  $lee_plantilla=str_replace("-total2-",$t_div2,$lee_plantilla);
  }
  else
  {  
  $lee_plantilla=str_replace("-total2-",'',$lee_plantilla);
  }
 
  $t_div3=$gin/$gin_t;  
  if($t_div3>=0)
  {
  $t_div3=number_format($t_div3, 2, '.', '');
  $lee_plantilla=str_replace("-total3-",$t_div3,$lee_plantilla);	
  }
  else
  {
  $lee_plantilla=str_replace("-total3-",'',$lee_plantilla);  
  }
  ...
  $odonto_indicec=$rs_sihaydata->fields["odonto_indicec"];
...
//paginar
$table_lista='dns_subsecuenteodontologia';

$numxpagina=11;
$busca_cuenta="select count(*) as total from ".$table_lista." where odonto_id=?";
$rs_cuenta = $DB_gogess->executec($busca_cuenta,array($valor_id));
$numero_datav=0;
$numero_datav=$rs_cuenta->fields["total"];
...
if($numero_datav==0)
{
  ...
  $lee_plantilla_lista=$objvarios->leer_contenido_completo($url); 
   
  $lee_plantilla_lista=str_replace("-nombre-",$nombre_paciente,$lee_plantilla_lista);
  $lee_plantilla_lista=str_replace("-apellido-",$apellido_paciente,$lee_plantilla_lista);
  $lee_plantilla_lista=str_replace("-sexo-",$clie_genero,$lee_plantilla_lista);
  $lee_plantilla_lista=str_replace("-hc-",$hc,$lee_plantilla_lista);  
  
  for($z=1;$z<=11;$z++)
  {
          $lee_plantilla_lista=str_replace("-fecha".$z."-","",$lee_plantilla_lista); 
		  $lee_plantilla_lista=str_replace("-hora".$z."-","",$lee_plantilla_lista);
		  $lee_plantilla_lista=str_replace("-notas".$z."-","",$lee_plantilla_lista);
		  $lee_plantilla_lista=str_replace("-farmaco".$z."-","",$lee_plantilla_lista);
		  $lee_plantilla_lista=str_replace("-otros".$z."-","",$lee_plantilla_lista);
  
  }
  
  $concatena_paginas.=$lee_plantilla_lista;
...
for($i=0;$i<$numero_entero;$i++)
{
  ...
  $lee_plantilla_lista=str_replace("-hc-",$hc,$lee_plantilla_lista);   
   ...
   $cuenta_val=0;
   $rs_aevolucion="select * from ".$table_lista." where  odonto_id=? limit ".$incio_reg.",".$fin_reg;
  ...
  //PLANES DE DIAGNÓSTICO, TERAPÉUTICO Y EDUCACIONAL
	...
	//dns_recetasodontologia	  
		  ...
		  $lee_plantilla_lista=str_replace("-fecha".$cuenta_val."-",$numsess.$rs_aevolucion->fields["subsecodont_fecharegistro"],$lee_plantilla_lista); 
//		  $lee_plantilla=str_replace("-hora".$cuenta_val."-",$rs_aevolucion->fields["conext_horar"],$lee_plantilla);
		  $lee_plantilla_lista=str_replace("-notas".$cuenta_val."-",$datos_rs_dns_odontologiaplandiganostico." ".$rs_aevolucion->fields["subsecodont_diagnosticoscomplicaciones"],$lee_plantilla_lista);
		  $lee_plantilla_lista=str_replace("-farmaco".$cuenta_val."-",$rs_aevolucion->fields["subsecodont_procedimientos"],$lee_plantilla_lista);
		  $lee_plantilla_lista=str_replace("-otros".$cuenta_val."-",$prescripcion_val,$lee_plantilla_lista);
  ...
  for($z=1;$z<=11;$z++)
  {
   ...
   $concatena_paginas.=$lee_plantilla_lista.$salto_pagina;	
}
...
$comprobantepdf=$lee_plantilla.'<div style="page-break-after:always;"></div>'.$concatena_paginas;
