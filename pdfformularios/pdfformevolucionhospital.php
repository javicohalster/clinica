   ...
   if($rs_aevolucion)
	{
		while (!$rs_aevolucion->EOF) {
		  ...
		  $lista_prescrip='<table width="100%" border="0" cellpadding="0" cellspacing="0">';
		  if($rs_aevolucion->fields["conext_prescripciones"])
		  {
		  $lista_prescrip.='<tr><td class="supinfelinea">'.$rs_aevolucion->fields["conext_prescripciones"].'</td><td class="supinfelinea" style="font-size:9px" >'.$nomb_medico.'</td></tr>';
		  }
		  ...
		  if($rs_notasevolista)
	      {
			while (!$rs_notasevolista->EOF) {
			  ...
			  $lista_prescrip.='<tr><td class="supinfelinea">'.$rs_notasevolista->fields["gridhpres_descripcion"].'</td><td class="supinfelinea" style="font-size:9px" >'.$usua_prescr.'<br>'.$rs_notasevolista->fields["gridhpres_fecharegistro"].'</td></tr>';
