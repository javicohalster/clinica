...
if ($_SESSION['datadarwin2679_sessid_inicio']) {
  ...
  //numero hojas
  $conteo_pg = 0;
  $busca_listapg = "select * from " . $table . " where  clie_id=" . $clie_id . " and atenc_id='" . $atenc_id . "' order by anam_id asc";
  ...
  if ($rs_blistapg) {
    while (!$rs_blistapg->EOF) {
      ...
      $conteo_pg++;
      $actualizpg = "update " . $table . " set anam_nhoja='" . $conteo_pg . "' where anam_id='" . $rs_blistapg->fields["anam_id"] . "'";
  ...
  $anam_nhoja = $rs_sihaydata->fields["anam_nhoja"];


  //=========================================================================


  $url = "plantillas/form003anversonewtraumatologia.php";
  ...
  $lee_plantilla = str_replace("-numera-", $anam_nhoja, $lee_plantilla);
...
*/
  ...
  */
  ...
  /*$datos_ro='';
  $rebicion_organos="select * from pichinchahumana_extension.dns_newtraumatologiagridorgano a inner join pichinchahumana_combos.dns_opcionorganonewconext b on a.opcorg_id=b.opcorg_id where tiporg_nombre='CP' and anam_enlace='".$anam_enlace."'";
  ...
  $datos_ro = '';
  $rebicion_organos = "select * from pichinchahumana_combos.dns_opcionorganonewconext";
  $rs_ro = $DB_gogess->executec($rebicion_organos, array());
  if ($rs_ro) {
    while (!$rs_ro->EOF) {

      $rebicion_buorganos = "select * from pichinchahumana_extension.dns_newtraumatologiagridorgano a  where a.opcorg_id='" . $rs_ro->fields["opcorg_id"] . "' and anam_enlace='" . $anam_enlace . "'";
  ...
  // $sig_vitales="select * from dns_signosvitales where atenc_enlace='".$atenc_enlace."' order by signovita_fecharegistro desc limit 1";
  ...
  /*$datos_freg='';
 $fisco_regional="select * from pichinchahumana_extension.dns_newtraumatologiagridexamenfisico a inner join pichinchahumana_combos.dns_opcionexamenfisico b on a.opcexa_id=b.opcexa_id where tiporg_nombre='CP' and anam_enlace='".$anam_enlace."'";
  ...
  $datos_freg = '';
  $fisco_regional = "select * from pichinchahumana_combos.dns_opcionexamenfisico order by 	opcexa_orden asc";
  $rs_freg = $DB_gogess->executec($fisco_regional, array());
  if ($rs_freg) {
    while (!$rs_freg->EOF) {

      $fisco_regionalbusca = "select * from pichinchahumana_extension.dns_newtraumatologiagridexamenfisico a inner join pichinchahumana_combos.dns_opcionexamenfisico b on a.opcexa_id=b.opcexa_id where  a.opcexa_id='" . $rs_freg->fields["opcexa_id"] . "' and anam_enlace='" . $anam_enlace . "'";

      $rs_bufreg = $DB_gogess->executec($fisco_regionalbusca, array());

      if ($rs_bufreg->fields["tiporg_nombre"] == 'CP') {

        $datos_freg .= $rs_bufreg->fields["opcexa_orden"] . " " . $rs_bufreg->fields["opcexa_tipo"] . ".- " . $rs_bufreg->fields["gexamfis_observacion"] . ", ";
        $lee_plantilla = str_replace("-" . $rs_freg->fields["opcexa_orden"] . strtolower($rs_freg->fields["opcexa_tipo"]) . "cp-", 'X', $lee_plantilla);
        $lee_plantilla = str_replace("-" . $rs_freg->fields["opcexa_orden"] . strtolower($rs_freg->fields["opcexa_tipo"]) . "sp-", '', $lee_plantilla);
      } else {
        $datos_freg .= $rs_bufreg->fields["opcexa_orden"] . " " . $rs_bufreg->fields["opcexa_tipo"] . ".- SIN EVIDENCIA DE PATOLOGIA, ";
      }
  ...
  $datos_diagnostico = '';
  $cuentase = 0;
  $diagosticos_rv = "select * from pichinchahumana_extension.dns_newtraumatologiadiagnosticoanamnesis where anam_enlace='" . $anam_enlace . "' limit 6";
  ...
  $anam_planes = str_replace(['<', '>'], ['&lt;', '&gt;'], $anam_planes);
