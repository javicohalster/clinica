...
function envia_documentos($tipocmp_tabla,$tipocmp_campofirmado,$tipocmp_sinfirma,$tipocmp_campoxmlfirmado,$tipocmp_campoid,$tipocmp_estadosri,$tipocmp_motivodev,$tipocmp_codigo,$link_envio,$debug,$valcentro,$tipocmp_campofecha,$compretcab_id,$DB_gogess)
{

$fechah = date('Y-m-d');
$nuevaFechah = date("Y-m-d",strtotime ( '-1 day' , strtotime ( $fechah ) ) );

$lista_fac="select * from ".$tipocmp_tabla." where ".$tipocmp_campofirmado."='SI' and (".$tipocmp_estadosri."='' or ".$tipocmp_estadosri." not in('RECIBIDA','AUTORIZADO','NO AUTORIZADO')) and compretcab_anulado!='1' and compretcab_id='".$compretcab_id."' limit 1 ";
...
if($resultlist_ldocumentos)
		{
			while (!$resultlist_ldocumentos->EOF) {			
			   ...
			   envia_documentos($tipocmp_tabla,$tipocmp_campofirmado,$tipocmp_sinfirma,$tipocmp_campoxmlfirmado,$tipocmp_campoid,$tipocmp_estadosri,$tipocmp_motivodev,$tipocmp_codigo,$link_envio,$debug,$valcentro,$tipocmp_campofecha,$compretcab_id,$DB_gogess);
