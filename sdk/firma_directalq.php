...
//firma
function firma_documentos($tipocmp_tabla, $tipocmp_campofirmado, $tipocmp_sinfirma, $tipocmp_campoxmlfirmado, $tipocmp_campoid, $valcentro, $tipocmp_anulado, $compra_id, $DB_gogess)
{
	//===============================================================================

	$lista_fac = "select * from " . $tipocmp_tabla . " where " . $tipocmp_campofirmado . "=''  and " . $tipocmp_anulado . "!='1'  and compra_id='" . $compra_id . "' limit 1 ";
	...
	if ($resultlist_fac) {
		while (!$resultlist_fac->EOF) {
			...
			//echo $doc['usuario']."<br>";			  
			//echo @$resultlist_fac->fields["compra_nfactura"]."<br>";		  
			...
			if ($resultado["motivo"]) {
				...
				if ($resultado["resultado"] != '') {
					...
					echo "<br>" . @$resultlist_fac->fields["compra_nfactura"] . " Firmado con Exito...";
				}
			}
...
if ($resultlist_ldocumentos) {
	while (!$resultlist_ldocumentos->EOF) {
		...
		firma_documentos($tipocmp_tabla, $tipocmp_campofirmado, $tipocmp_sinfirma, $tipocmp_campoxmlfirmado, $tipocmp_campoid, $valcentro, $tipocmp_anulado, $compra_id, $DB_gogess);
