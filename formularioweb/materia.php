...
function selecciona_alumnos()
{

$("#div_alumnos").load("variables_sess.php",{
  carr_id:'<?php echo $_SESSION['formularioweb_carr_id']; ?>',
  nivl_id:'<?php echo $_SESSION['formularioweb_nivl_id']; ?>',
  matprof_id:$('#matprof_id').val()
 },function(result){  

     
  });  

  $("#div_alumnos").html("Espere un momento...");  
...
if(@$_SESSION['formularioweb_peri_id'])
			...
			$sql='select distinct 	mate_id,matprof_id from inc_materiaprofesor,inc_profesorperiodo,inc_periodo where inc_materiaprofesor.proper_id=inc_profesorperiodo.proper_id and inc_periodo.perio_id=inc_profesorperiodo.perio_id  and  inc_profesorperiodo.peri_id='.$_SESSION['formularioweb_peri_id']." and inc_periodo.perio_id in(".$periodosactivos.") and carr_id=".$_SESSION['formularioweb_carr_id']." and nivl_id=".$_SESSION['formularioweb_nivl_id'];
	$resultadocarr = mysql_query($sql);
			
	?>					
<label for="select-choice-1" class="select" >Materia</label>
<select name="matprof_id" id="matprof_id"  onChange="selecciona_alumnos()">
			...
			while($rowcarr = mysql_fetch_array($resultadocarr)) 
				{
				   ...
				   $nombremateria=$objformulario->replace_cmb("inc_materia","mate_id,mate_nombre","where mate_id=",$rowcarr["mate_id"]);
   echo '<option value="'.$rowcarr["matprof_id"].'">'.$nombremateria.'</option>';
				}
			}	
	?>
</select>
<a href="alumnos.php"  data-role="button" rel="external" >Siguiente</a>
<div id="div_alumnos" >
