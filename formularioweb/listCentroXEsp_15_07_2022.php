...
$query = <<<QUERY
    SELECT UE.usua_enlace, APPU.usua_id, APPU.usua_nombre, APPU.usua_apellido, APPU.usua_piefirma, CS.centro_nombre, CS.centro_direccion,
    CH.horario_hora, CH.horario_horafin, CS.centro_id
    FROM dns_gridfuncionprofesional UE INNER JOIN dns_gridcentros DNSGC 
    ON DNSGC.usua_enlace = UE.usua_enlace 
    AND DNSGC.gridcent_activo = 1 INNER JOIN dns_centrosalud CS
    ON CS.centro_id = DNSGC.centro_id INNER JOIN app_usuario APPU
    ON APPU.usua_enlace = DNSGC.usua_enlace INNER JOIN clinica_horarios CH
    ON CH.usua_id = APPU.usua_id 
    AND CH.dia_id = '{$daySelectedF}' 
    WHERE UE.prof_id = '{$_REQUEST["esp"]}' 
    AND UE.grdifun_activo = 1 
QUERY;
    ...
    <tbody style="overflow-y: auto; "><?php
        ...
        while(!$rsDisp->EOF){
            ...
            $lstBtnAgenda = calcAtencion($dateSelected, $horaIni, $rsDisp->fields["horario_horafin"], $intervalBetween, "12:00", "12:30", [
                ...
                "profID" => $rsDisp->fields["usua_enlace"],
                "espID" => $_REQUEST["esp"],
            ], $DB_gogess);
            $stiloRow = "#EEE;";
            if($cont % 2 == 0){
...
<script>
    function verif_datos($date, $hour, $centroID, $profID, $espID){
        ...
        function(result){
            ...
            if(res.action=="registrar"){
                pedir_formulario($date, $hour, $centroID, $profID, $espID);
            } else {
                //agendar y notif
    ...
    function pedir_formulario($date, $hour, $centroID, $profID,$espID){
