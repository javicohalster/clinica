...
if ($resul_campos)
        {
			 while (!$resul_campos->EOF) {	
			  ...
			  if (@$resul_campos->fields["fie_value"]=="replace")
              {
			     
				$selecTabla='';
				$selecTabla="select ".$resul_campos->fields["fie_datadb"]." from ".$resul_campos->fields["fie_tabledb"]." ".$resul_campos->fields["fie_sql"].$resul_datosenvia->fields[$resul_campos->fields["fie_name"]]; 
				
				$resul_bcampo = $DB_gogess->executec($selecTabla,array());
				
				$separa_d=explode(",",$resul_campos->fields["fie_datadb"]);
				
				$rmp=$resul_bcampo->fields[trim($separa_d[1])];
					
			  }
			  else
			  {
			    $rmp=$resul_datosenvia->fields[$resul_campos->fields["fie_name"]];
			  }
			 $concatena_contenido.=' '.$resul_campos->fields["fie_title"]." ".$rmp."<br>";
