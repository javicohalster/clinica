...
//Fin datos de cada campo
	...
	function gridtabla($listacampos,$sqldatos,$sqldatoscompleto,$link_sqlserver,$DB_gogess)
	{
	 ...
	 //print_r($listacampos);
	   $i=0;
				   while ($i < count($listacampos))   
						...
						//print_r($campotabla);
						$parte1='';
						$parte1=str_replace("(","",trim($campotabla[0]));
						$parte1=str_replace(")","",$parte1);
						
						$parte2='';
						$parte2=str_replace("(","",trim($campotabla[1]));
						$parte2=str_replace(")","",$parte2);
						
						//echo $parte1."<br>";
						
						//$this->form_format_field(trim($parte1),trim($parte2),$DB_gogess);							
						...
						{
							$campo_asdata=array();
							$campo_asdata=explode(" as ",trim($campotabla[1]));
							 
							
							
							if(count($campo_asdata)>1)
							{
								
								$this->form_format_field(trim($campotabla[0]),trim($campo_asdata[0]),$DB_gogess);
								if($this->fie_alias)
								{							
								$this->arrcampos[]=trim($campo_asdata[1]);							
								$titulocampo=str_replace(":","",$this->fie_alias);
								...
								else
								{
									...
									$this->arrcampos[]=trim(@$campo_asdata[1]);							
									$this->arrcamposn[]=trim(@$campo_asdata[1]);
		...
		///////////////lista datos///////////////////////
			  ...
			  // mssql_query('use LotTracker_cli');
			   
			   $resultdatos=mssql_query($sqldatos,$link_sqlserver);
			  ...
			  //echo $sqldatoscompleto;
			   ...
			   $resultdatoscompleto=mssql_query($sqldatoscompleto,$link_sqlserver);
			   //echo $sqldatoscompleto;
			   ...
			   if ($resultdatos)
			   {
				 while ($fields=mssql_fetch_array($resultdatos))
					{
