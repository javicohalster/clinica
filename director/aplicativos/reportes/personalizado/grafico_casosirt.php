...
$ingresadom='';
$porcentaje='';
	  $listac_idm="select count(*) as total,tipoing_id,mate_id from media_causasdet inner join media_causascab on media_causasdet.caucab_id=media_causascab.caucab_id where 	year(caudet_fechaingreso)='".trim($datos_val[0])."' and tipoing_id=3 group by tipoing_id,mate_id";
...
if(is_array($ingresadom))
{
  ...
  $rs_lmateria = $DB_gogess->Execute($lista_mat);
	...
	$datos_g[]=array('',number_format($porcentaje_vg, 2, '.', ''));
...
$p->SetPlotAreaPixels(240, 20, 595, 295);
$p->SetLegendPixels(5, 40);
$p->SetLegend($legend);

$p->SetTitle("No. Casos ingresados por remisión de tránsito");
