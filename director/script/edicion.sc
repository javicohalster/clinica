...
function nuevo_<?php echo $table; ?>()
{
<?php
...
function ejecutar_formulario_<?php echo $table; ?>(){
    var options = {
		...
		eval(result);
            if(result_global=="1") 
			{
			   ...
			   if($('#opcion_<?php echo $table ?>').val()=='guardar')
			   {
   ...
   $("#div_<?php echo $table ?>").html("<table border=0><tr><td></td><td><span style='font-size:11px; color:green;'>Ejecutando...</span></td></tr></table>");
...
$.validator.setDefaults({
	...
	ejecutar_formulario_<?php echo $table; ?>();
	}
