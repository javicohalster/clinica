...
if ($table)
{
 ...
 if (@$objformulario->tab_datosf)
 {
	  if (!($opcion))
	  {
		...
		//**************************************************************
		$selecf="select * from `".$table."`";
	    $resultadof = mysql_query($selecf);
		if ($resultadof)
			{
			   $ncamposf = mysql_num_fields($resultadof);
			   $n_registrosf  = mysql_num_rows($resultadof);  
			   $if=0;
			 } 
		...
		while ($if < $ncamposf) 
         {
				...
				$nombre_campof  = mysql_field_name($resultadof, $if); 		
				$objformulario->form_format_field($table,$nombre_campof);  
		
				$flags = mysql_field_flags($resultadof, $if);
		...
		//**************************************************************
		//echo $sqlf; 
        $resultadof = mysql_query($sqlf);
  		while($rowf = mysql_fetch_array($resultadof)) 
			{	
			  $csearch=$rowf[$ordernamef];
			}
...
<table  border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td bgcolor="#003366" class="titulotemplate">&nbsp;&nbsp;<?php echo $objtableform->titulo_tabla ?></td>
    <td  bgcolor="#003366" class="titulotemplate">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
  </tr>
</table>
<?php
}	
...
if (@$tablelista)
{
...
else
 {
     ...
     switch ($opcp) 
	{
     ...
     case 2:
      {
          include("modules/laboratorio.php");
       }
      break;
	  	  case 3:
      {
          include("modules/informacion.php");
       }
      break;
	  	  case 4:
      {
          include("modules/interes/loan-calculator.php");
       }
      break;
	  	  	  case 5:
      {
