...
function fecha_bloque_desc($formato,$ncampo,$valor,$obl)
{
   ...
   //dia
   $select["d"]='<div class="input-group input-group-static my-3"><select class="form-control" name="dia_'.$ncampo.'" id="dia_'.$ncampo.'" onClick="fecha_bloqueasigna(\'dia_'.$ncampo.'\',\'mes_'.$ncampo.'\',\'anio_'.$ncampo.'\',\''.$ncampo.'\')" ><option value="00">-dia-</option>';
   for($i=1;$i<=31;$i++)
   {
   ...
   //mes
   $select["m"]='<div class="input-group input-group-static my-3"><select class="form-control" name="mes_'.$ncampo.'" id="mes_'.$ncampo.'" onClick="fecha_bloqueasigna(\'dia_'.$ncampo.'\',\'mes_'.$ncampo.'\',\'anio_'.$ncampo.'\',\''.$ncampo.'\')"  ><option value="00">-mes-</option>';
   for($i=1;$i<=12;$i++)
   {
   ...
   //anio
   $select["Y"]='<div class="input-group input-group-static my-3"><select class="form-control" name="anio_'.$ncampo.'" id="anio_'.$ncampo.'" onClick="fecha_bloqueasigna(\'dia_'.$ncampo.'\',\'mes_'.$ncampo.'\',\'anio_'.$ncampo.'\',\''.$ncampo.'\')"  ><option value="00000">-a&ntilde;o-</option>';
...
function fecha_bloque($formato,$ncampo,$valor,$obl)
{
   ...
   //dia
   $select["d"]='<select name="dia_'.$ncampo.'" id="dia_'.$ncampo.'" onClick="fecha_bloqueasigna(\'dia_'.$ncampo.'\',\'mes_'.$ncampo.'\',\'anio_'.$ncampo.'\',\''.$ncampo.'\')" ><option value="00">-dia-</option>';
   for($i=1;$i<=31;$i++)
   {
    $valord=str_pad($i,2, "0", STR_PAD_LEFT);
	if(@$valor_biene[2]==$valord)
   ...
   //mes
   $select["m"]='<select name="mes_'.$ncampo.'" id="mes_'.$ncampo.'" onClick="fecha_bloqueasigna(\'dia_'.$ncampo.'\',\'mes_'.$ncampo.'\',\'anio_'.$ncampo.'\',\''.$ncampo.'\')"  ><option value="00">-mes-</option>';
   for($i=1;$i<=12;$i++)
   {
    $valorm=str_pad($i,2, "0", STR_PAD_LEFT);
	if(@$valor_biene[1]==$valorm)
   ...
   //anio
   $select["Y"]='<select name="anio_'.$ncampo.'" id="anio_'.$ncampo.'" onClick="fecha_bloqueasigna(\'dia_'.$ncampo.'\',\'mes_'.$ncampo.'\',\'anio_'.$ncampo.'\',\''.$ncampo.'\')"  ><option value="00000">-a&ntilde;o-</option>';
   for($i=$rango_inicio;$i<=$fechaac;$i++)
   {
...
$file = fopen("sistemainserta".date("Y-m-d").".txt", "a+");
...
fclose($file);
   ...
   $this->nuevoid=$DB_gogess->Insert_ID();
...
$file = fopen("sistemaactualiza".date("Y-m-d").".txt", "a+");
fwrite($file,$sql_1 . PHP_EOL);
fclose($file);
		...
		case 5:
        {
		  while ($i < $ncampos) 
          {
			...
			////		
				...
				function borrar_".str_replace(".","_",$table)."() \n{\n
					  ...
					  abrir_pantalla('libreria/dialogo/borrar.php','BORRAR','divBody_borrar_".str_replace(".","_",$table)."','divDialog_borrar_".str_replace(".","_",$table)."',400,200,'".$table."',buscadata,$('#opcion_".str_replace(".","_",$table)."').val(),0,0,0,0);
