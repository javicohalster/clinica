...
if($rs_tblpla)
	{
		...
		//------------------------------------------------------------		
		if($rs_tcodigo->fields["plantrai_codigo"]!='')
		{
		
		$busca_precio="select * from dns_cuadrobasicomedicamentos where cuadrobm_codigoatc='".$rs_tcodigo->fields["plantrai_codigo"]."'";
		$rs_precio = $DB_gogess->executec($busca_precio,array());
		
		echo $busca_precio."<br>";
		
				//
		$actualiza_descarga="update ".$rs_tblpla->fields["fie_tablasubgrid"]." set plantrai_preciocompra='".$rs_precio->fields["cuadrobm_preciodispositivo"]."',plantrai_porcentajeadm='10',plantrai_porcentajeiva='12',plantrai_valorplanilla='".$rs_precio->fields["cuadrobm_valorplanilladispositivos"]."' where plantrai_id='".$rs_tcodigo->fields["plantrai_id"]."'";
