...
$contenido = file_get_contents(@$director."jason_files/estructura/beko_recibodetalle.json");
...
if($script_busca)
{
  ...
  if($rscampos_valor->fields["tippo_id"]==4)
  {  
    $forma_pago='IESS';    
  }
  if($rscampos_valor->fields["tippo_id"]==2)
  {  
    $forma_pago='CONVENIOS';    
  }
  
   if($rscampos_valor->fields["tippo_id"]==5)
  {  
    $forma_pago='MOVILIDAD';    
  }
  
  if($rscampos_valor->fields["tippo_id"]==6)
  {  
...
$lista_eati="select distinct usua_nombre,usua_apellido from beko_recibodetalle inner join app_usuario on app_usuario.usua_id=beko_recibodetalle.usuaat_id where doccab_id='".$rscampos_valor->fields["doccab_id"]."'";
...
$lista_espe="select distinct prof_nombre from beko_recibodetalle inner join pichinchahumana_extension.dns_profesion profe on beko_recibodetalle.prof_id=profe.prof_id inner join dns_especialidad on dns_especialidad.especi_id=profe.especienc_id where doccab_id='".$rscampos_valor->fields["doccab_id"]."'";
