...
{
$lista_empleados="select * from conco_roles inner join app_usuario on conco_roles.usua_id=app_usuario.usua_id where usua_estado=1 and genr_id='".$genr_id."' order by usua_apellido asc";
}
      ...
      while (!$resultl_listaroles->EOF)
		{
		  ...
		  $comprobantepdf_condata=str_replace("-subtitulo-",$resultl_empresa->fields["emp_departamento"],$comprobantepdf);
		  ...
		  $comprobantepdf_condata=str_replace("-cedula-",utf8_encode($resultl_empleado->fields["usua_ciruc"]),$comprobantepdf_condata);
...
grid_infolaboral3.info_unidad=app_tipounidad.tipouni_id inner join cmb_puestoinstitucional on grid_infolaboral3.info_puestoinstitucional=cmb_puestoinstitucional.tipoinst_id where info_fechadesalida='0000-00-00' and app_usuario.usua_id='".$resultl_listaroles->fields["usua_id"]."' order by info_id desc limit 1";
		  ...
		  if($resultl_ingresos)
           {
		       while (!$resultl_ingresos->EOF)
		       {
				  ...
				  $ingreso_val.='<tr><td class="css_txt" ><b>'.utf8_decode($resultl_ingresos->fields["rubrg_nombre"]).':</b></td> <td class="css_txts" >'.number_format($resultl_ingresos->fields["rubrg_valor"], 2, '.', '').'</td></tr>';
				  $total_ingresos=$total_ingresos+number_format($resultl_ingresos->fields["rubrg_valor"], 2, '.', '');
		  ...
		  if($resultl_egresos)
		   ...
		   $busca_fondosreservas="select conco_acumuladetalleroles.detlroll_id,conco_acumuladetalleroles.roles_id,conco_acumuladetalleroles.rubrg_id,conco_acumuladetalleroles.rubrg_nombre,conco_acumuladetalleroles.rubrg_ingresoegreso,conco_acumuladetalleroles.rubrg_valor,conco_acumuladetalleroles.rubrg_formula,conco_acumuladetalleroles.rubrg_salariominimo,conco_acumuladetalleroles.tipprub_id,conco_acumuladetalleroles.genr_id,conco_acumuladetalleroles.usua_id from conco_acumuladetalleroles inner join conco_rubrosg on conco_acumuladetalleroles.rubrg_id=conco_rubrosg.rubrg_id where roles_id='".$resultl_listaroles->fields["roles_id"]."' and conco_acumuladetalleroles.rubrg_ingresoegreso=1 and tiporub_id=2";
		   
		   $resultl_frs= $DB_gogess->executec($busca_fondosreservas,array());
		   if($resultl_frs)
           {
		       while (!$resultl_frs->EOF)
		       {
			   
			    $egresos_val.='<tr><td class="css_txt" ><b>DEPOSITO '.$resultl_frs->fields["rubrg_nombre"].':</b></td> <td class="css_txts" >'.number_format($resultl_frs->fields["rubrg_valor"], 2, '.', '').'</td></tr>';
				$total_egresos=$total_egresos+number_format($resultl_frs->fields["rubrg_valor"], 2, '.', '');
			   
			   $resultl_frs->MoveNext();
			   }
		   }
