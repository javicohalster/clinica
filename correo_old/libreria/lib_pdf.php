<?php
function generacodebarra_pdf($idnumerografico,$idfac,$grafbarra)
{
  ...
  if ( isset($font) ){
   ...
   $local="c:/xampp/htdocs/dibeal/correo/codigodebarra/";
...
function leer_xml($estructurado,$xml,$tcomp,$nauto,$fechaaut,$logotipo_imd,$code_barra,$griddata,$valor_total,$valorsiniva_total,$valornograbado,$pathextrap,$offline_valor)
{
	//echo $pathextrap;
     ...
     //echo $pathextrap."plantillas/factura.php";
	switch ($tcomp) {
    case "01":
	    //FACTURA
		if($offline_valor==1)
	  ...
	  // $fechaEmision=$estructura->infoFactura->fechaEmision->__toString();
	   ...
	   //Lista
		...
		$condatos=str_replace("-logo-",$logotipo_imd,$condatos);
			...
			generacodebarra_pdf($claveAcceso,$claveAcceso,"FAC");
		$code_barra= '<img src="c:/xampp/htdocs/dibeal/correo/codigodebarra/FAC'.$claveAcceso.".gif".'"  width="320" height="60"  />';
...
if($id==0)
	   ...
	   //Lista
		...
		$condatos=str_replace("-total-",number_format($importeTotal, 2, ".", ","),$condatos);
        $condatos=str_replace("-logo-",$logotipo_imd,$condatos);
		generacodebarra_pdf($claveAcceso,$claveAcceso,"CRE");
		$code_barra= '<img src="c:/xampp/htdocs/dibeal/correo/codigodebarra/CRE'.$claveAcceso.".gif".'"  width="320" height="60"  />';
		$condatos=str_replace("-barraclave-",$code_barra,$condatos);
...
function lee_notadedebito($estructura,$plantilla,$numeroAutorizacion,$fechaAutorizacion,$logotipo_imd,$code_barra,$griddata,$valor_total,$valorsiniva_total)
{
	  ...
	  //$fechaEmisionDocSustento=$estructura->infoNotaDebito->fechaEmisionDocSustento->__toString();
	   ...
	   //Lista
		...
		$condatos=str_replace("-descuento-",number_format($totalDescuento, 2, ".", ","),$condatos);
        $condatos=str_replace("-logo-",$logotipo_imd,$condatos);
		generacodebarra_pdf($claveAcceso,$claveAcceso,"DEB");
		$code_barra= '<img src="c:/xampp/htdocs/dibeal/correo/codigodebarra/DEB'.$claveAcceso.".gif".'"  width="320" height="60"   />';
		$condatos=str_replace("-barraclave-",$code_barra,$condatos);
...
$fechaemidos=$estructura->impuestos->impuesto[$id]->fechaEmisionDocSustento->__toString();			
	 ...
	 <tr>
	   ...
	   //Lista
		...
		$condatos=str_replace("-descuento-",number_format($totalDescuento, 2, ".", ","),$condatos);
        $condatos=str_replace("-logo-",$logotipo_imd,$condatos);
		generacodebarra_pdf($claveAcceso,$claveAcceso,"RET");
		$code_barra= '<img src="c:/xampp/htdocs/dibeal/correo/codigodebarra/RET'.$claveAcceso.".gif".'"  width="320" height="60"  />';
		$condatos=str_replace("-barraclave-",$code_barra,$condatos);
