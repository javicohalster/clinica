...
if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS') {
	...
	//$userid =103;
	$req_data = $db->query("SELECT 	requ_id,usua_id,requ_observacion,requ_categoria,tiempres_id,pais_id,cant_id,requ_sector,requ_barrio,requ_paracuando,fpagcl_id,provee_id,requ_estado,requ_fecharegistro,IF(requ_grafico='','logo_img.png', REPLACE(SUBSTRING(SUBSTRING_INDEX(requ_grafico, ',', 2), LENGTH(SUBSTRING_INDEX(requ_grafico, ',', 2-1))+1), ',', '')) as requ_grafico  FROM app_requerimiento WHERE requ_id=$requ_id");
